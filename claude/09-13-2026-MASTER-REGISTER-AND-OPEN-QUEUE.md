...
| A2 | ROUND 21.1 Accounting tabs — item 1 (counts + dollar totals in tab labels) **DONE, see Part 1** (#21968); aging-bucket clickable filter tiles **DONE, see Part 1** (#22001, AP side only — A/R equivalent not yet built, no confirmed target surface). Items 2-3 (validation errors as a counted column with hover reason, split `Paid` from `Deposited`) remain OPEN — tracked as A5, not duplicated here. | PARTIAL — items 1 + aging-tiles DONE, items 2-3 OPEN (=A5) |
...
| A5 | Behaviour-only research items remaining: validation-errors counted column (no confirmed data source yet — no `validation_error`/`issues`/`warnings` field exists today on any bill/invoice row or API response; needs a defined backend concept before a UI column can be built, not yet investigated), Paid/Deposited split (mechanism confirmed feasible via `accounting.payments.source_bank_transaction_id`/`cleared_date`/`deposited_to_account_id`, but USMCA currently has 0 paid invoices so live verification is blocked by data volume — still buildable as forward-looking infra per owner's explicit ask), reconciliation Difference (AP Aging already shows a per-page QBO-mirror Δ strip; unclear if that satisfies "one pinned Difference that must reach zero" or a new dedicated surface is wanted — needs Lead/owner confirmation of intent before building) | OPEN, next cycle — items 2/3/5 need scope clarification before build, see notes |
