...
| 21.1 A5 #4 | AccountsPayableAgingPage aging buckets are now clickable filter tiles ("the tile IS the filter"): 5 tiles (Current/1-30/31-60/61-90/91+, never Total) narrow the vendor grid to nonzero-balance rows on click, toggle off on a second click or Clear, and a tile's own dollar figure never moves (always the full type-filtered total). New guard `verify-ap-aging-bucket-filter-tiles.mjs` (4 selftest mutation arms) + 3-case vitest regression suite | #22001 | CC-1 |
| 21.1 A5 #3 | Invoice detail's Payment Applications now splits Paid (`payment_date`/`applied_at`, unchanged) from Deposited (`cleared_date`, THREE-DATES-COVERAGE-GAP migration `202613310400`, previously never surfaced) — "Deposited \<date\> to \<account\>" or an honest "Not yet deposited." Built + unit-tested against the real schema/join (reuses PaymentDetailPage's own `deposited_to_account_id` → `catalogs.accounts` join shape); **Live=BLOCKED** — USMCA has 0 paid invoices right now, so it can't be click-through-verified until a real payment exists and clears | #22003 | CC-1 |
| 20.8 B11 | **CC-1-side half CLOSED.** Row 27 (#21946) shipped Banking's half of B11 (SyncStatusStrip through the shared `describeQboSyncStatus()` derivation); AccountingHubPage's "QBO Sync" KPI tile was the still-outstanding other half, only ever fetching the sync-queue backlog and never the OAuth connection state — live-reproduced the exact contradiction CC-2 flagged (`/banking` "Not connected" vs `/accounting` "0 pending — queue healthy" at the same moment) before fixing it. Now calls the identical shared derivation, fed the connection query BankingHome.tsx already fetches | #22005 | CC-1 |
...
| A2 | ROUND 21.1 Accounting tabs — item 1 (counts + dollar totals in tab labels) **DONE, see Part 1** (#21968); aging-bucket clickable filter tiles **DONE, see Part 1** (#22001, AP side only — A/R equivalent not yet built, no confirmed target surface); Paid/Deposited split **DONE, see Part 1** (#22003, Live=BLOCKED — 0 USMCA paid invoices). Item 2 (validation errors as a counted column with hover reason) remains OPEN — tracked as A5, not duplicated here. | PARTIAL — items 1 + aging-tiles + Paid/Deposited DONE, item 2 OPEN (=A5) |
...
| A5 | Behaviour-only research items remaining: validation-errors counted column (no confirmed data source yet — no `validation_error`/`issues`/`warnings` field exists today on any bill/invoice row or API response; needs a defined backend concept before a UI column can be built, not yet investigated), reconciliation Difference (AP Aging already shows a per-page QBO-mirror Δ strip; unclear if that satisfies "one pinned Difference that must reach zero" or a new dedicated surface is wanted — needs Lead/owner confirmation of intent before building) | OPEN, next cycle — items 2/5 need scope clarification before build, see notes |
