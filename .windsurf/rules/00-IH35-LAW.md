...
- Queue = `claude/GO-23-BUILD-SEQUENCE-STRICT-2026-09-02.md`. STRICT ORDER. Waves run in order; seats inside a wave run in parallel. You do not start a later wave while a row in your lane is open in an earlier one. A defect not on that board does not exist — never open a new register, a new file is never the answer. `INBOX-<SEAT>.md` carries your current row only; GO-23 decides what that row is. **Never** `Downloads/final`, `Downloads/20 blocks`, `Downloads/abb`, `Downloads/ab`, `Downloads/sdf`. **Never** start from GUARD-WORKORDERS sweep rows (SWEEP-A, SORT-03, BOOKLOAD-ALWAYSTRACK). Cursor lead **coordinates seats**, does not sweep. Boards the owner publishes also live in `~/Downloads/IH35-BOARDS/` — that is a copy, not a second queue.
...
Read first: `claude/00-IH35-CURRENT-STATE-AND-LAW-READ-FIRST.md` then `claude/GO-23-BUILD-SEQUENCE-STRICT-2026-09-02.md` (the only queue).
