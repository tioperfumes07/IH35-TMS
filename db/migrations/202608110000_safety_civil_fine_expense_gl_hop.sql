...
--      'civil_fines_expense'.
--   §2 Additive JE-linkage columns on safety.civil_fines so a posted fine is drill-throughable in both
--      directions (fine -> JE, JE -> fine) per the Total-Connectivity law.
...
-- ORDERING NOTE: the safety.civil_fines ADD COLUMN section is deliberately placed BEFORE the CoA-role
-- CHECK widening. scripts/verify-schema-parity.mjs attributes ADD COLUMN tokens by scanning a fixed
-- 4096-char window forward from each `ALTER TABLE schema.table`, so a role-CHECK ALTER sitting above
-- these columns would mis-attribute expense_je_id/expense_posted_at/expense_posted_by_user_id to
-- accounting.chart_of_accounts_roles and catalogs.account_role_bindings. Keeping the column DDL first
-- (and the CHECK-only ALTERs last, where the window sees only ADD CONSTRAINT) keeps the baseline honest
-- without weakening that shared guard.
...
-- ===========================================================================================
-- §2 — Fine -> JE linkage (Total-Connectivity law: forward AND reverse drill-through).
--      Additive, nullable, no default backfill. Written ONLY by the poster, ONLY when the flag is ON.
--      expense_je_id is also the poster's idempotency latch: a fine that already carries one is never
--      posted twice.
-- ===========================================================================================
DO $$
BEGIN
  IF to_regclass('safety.civil_fines') IS NOT NULL THEN
    ALTER TABLE safety.civil_fines
      ADD COLUMN IF NOT EXISTS expense_je_id            uuid NULL,
      ADD COLUMN IF NOT EXISTS expense_posted_at        timestamptz NULL,
      ADD COLUMN IF NOT EXISTS expense_posted_by_user_id uuid NULL;

    -- FK to the ledger, added separately + guarded so a fresh DB that has not yet built
    -- accounting.journal_entries cannot fail the migration.
    IF to_regclass('accounting.journal_entries') IS NOT NULL
       AND NOT EXISTS (
         ...
         WHERE nsp.nspname = 'safety'
           ...
           AND con.conname = 'fk_civil_fines_expense_je'
       )
    THEN
      ALTER TABLE safety.civil_fines
        ADD CONSTRAINT fk_civil_fines_expense_je
        FOREIGN KEY (expense_je_id) REFERENCES accounting.journal_entries(id);
    END IF;

    -- Reverse drill-through: JE id -> the fine that produced it.
    CREATE INDEX IF NOT EXISTS ix_civil_fines_expense_je
      ON safety.civil_fines (expense_je_id)
      WHERE expense_je_id IS NOT NULL;

    -- Forward: the "company-paid, posted?" worklist.
    CREATE INDEX IF NOT EXISTS ix_civil_fines_company_paid_unposted
      ON safety.civil_fines (operating_company_id, status)
      WHERE expense_je_id IS NULL AND voided_at IS NULL;

    COMMENT ON COLUMN safety.civil_fines.expense_je_id IS
      'accounting.journal_entries.id of the COMPANY-PAID expense JE (Dr civil_fines_expense / Cr cash_clearing). '
      'NULL for driver-recovery fines (those use converted_to_liability_id) and for any fine posted while '
      'SAFETY_FINE_GL_POSTING_ENABLED is OFF. Also the poster idempotency latch.';
  END IF;
END
$$;
...
--
--   -- 3) linkage columns present:
--   SELECT column_name FROM information_schema.columns
--    WHERE table_schema='safety' AND table_name='civil_fines'
--      AND column_name IN ('expense_je_id','expense_posted_at','expense_posted_by_user_id');
--
...
--   BEGIN;
--     ALTER TABLE safety.civil_fines DROP CONSTRAINT IF EXISTS fk_civil_fines_expense_je;
--     -- leave the three columns in place (additive, nullable, harmless); re-narrow both CHECKs;
