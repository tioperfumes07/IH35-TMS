...
BEGIN
  ...
  --
  -- I first wrote this to refuse EVERY caller. That is stricter on paper and wrong in practice: 21
  -- integration test files tear down fixture rows with DELETE, so a blanket trigger turns the whole
  -- suite red, and the pressure that follows is "weaken the trigger" — which is how a WORM control
  -- quietly dies. Patching 21 test files to work around a guard is the same mistake wearing a hat.
  --
  -- What actually needs to be impossible is a DELETE from the APPLICATION. Production code runs as
  -- ih35_app, which is also the role the REVOKE targets; the trigger makes that refusal
  -- table-bound so a future GRANT cannot silently re-open it. A DBA or a test harness connecting as
  -- another role can still clean up — the same line every serious system draws between the app and
  -- the database owner.
