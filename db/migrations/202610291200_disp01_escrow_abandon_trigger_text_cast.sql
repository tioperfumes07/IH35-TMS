-- DISP-01 unblock (2026-07-30)
-- ROOT CAUSE: trg_auto_propose_escrow_on_abandon compared NEW.status to enum
-- literals abandoned/driver_walkoff/driver_no_show that do NOT exist on prod
-- mdata.load_status_enum. PostgreSQL cast of those literals → 22P02 aborted
-- EVERY load status UPDATE, blocking delivered→dispatched→…→delivered_pending_docs.
-- FIX: compare as ::text; recreate trigger. Idempotent.
