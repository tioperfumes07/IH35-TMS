# Module completion — Accounting (Module 3)

**PROGRESS: 8 of 25** · complete: `false` · as_of: 2026-07-25T16:29:00.000Z · live_sha: `a3f9c12`

| Status | Count |
|---|---:|
| PASS | 8 |
| HOLD | 0 |
| OPEN | 0 |
| FAIL | 8 |
| UNVERIFIED | 9 |

| ID | Status | Title | Evidence | PR |
|---|---|---|---|---|
| `ACCT-ECON-01` | **PASS** | bill_lines dense vs bills (header+lines) | Neon lucia: bills=16212 bill_lines=155049 (2026-07-24) | #3382 |
| `ACCT-ECON-02` | **PASS** | bill_payments live density > 0 after QBO pull+project | Neon lucia 2026-07-25: bill_payments=6479 (all qbo-sourced); qbo_ap_bill_payments=6313; QBO_AP_BILL_PAYMENTS_PROJECTION_ENABLED=true TRANSP; sync_runs ap_bill_payments_inbound_mirror success×2 last 23:23Z | #3398 |
| `ACCT-ECON-03` | **FAIL** | AR payments live density > 0 after QBO pull+project | FAIL-BY-DESIGN KEEP 2026-07-25: QBO_AR_PAYMENTS_PROJECTION_ENABLED default_enabled=false on Neon (lucia). Owner decision: projection flags stay OFF (parallel books, reconcile-only). Schema for AR payment mirrors may exist; do NOT flip flag. payments=0 correct-at-0 until CPA/owner unlock. | #3433 |
| `ACCT-ECON-04` | **FAIL** | expenses live density > 0 after Purchase pull+project | FAIL-BY-DESIGN KEEP 2026-07-25: QBO_EXPENSES_PROJECTION_ENABLED default_enabled=false on Neon (lucia). Owner decision: projection flags stay OFF. expenses=0 correct-at-0 until CPA/owner unlock. | #3433 |
| `ACCT-ECON-05` | **FAIL** | vendor_credits live path exercised or honest empty | Neon lucia 2026-07-25: vendor_credits=0, vendor_credit_applications=0. Path was NOT usable: bills.vendor_id holds the QBO vendor id on 16211 of 16212 bills while a credit holds an mdata.vendors uuid, so the apply picker resolved 0 bills (mdata.vendors.qbo_vendor_id bridges 16211). Bridge + cross-vendor refusal + canonical VC numbering shipped and proven on a migrated throwaway Postgres (9 lifecycle assertions incl. planted-failure); guard verify-vendor-credit-live-path (step 1436). Still FAIL: no live prod row — PASS needs one operator credit applied after deploy, or owner acceptance of honest-empty. (ACCT-F01/F06 note 2026-07-25: this FAIL is a SEPARATE root cause from ECON-03/04 — the vendor-bridge picker gap, NOT the projection-flags-off design — do not conflate the two.) | #3426 |
| `ACCT-PULL-01` | **PASS** | QBO AR Payment mirror pull writes rows + qbo.sync_runs | Neon lucia 2026-07-25: qbo_ar_payments=23830; qbo.sync_runs kind=ar_payments_inbound_mirror status=success n=1 completed 2026-07-24T23:12:22Z | #3411 |
| `ACCT-PULL-02` | **PASS** | QBO BillPayment mirror pull writes rows | Neon lucia 2026-07-25: qbo_ap_bill_payments=6313; sync_runs ap_bill_payments_inbound_mirror success×2 last 2026-07-24T23:23:41Z | #3398 |
| `ACCT-PULL-03` | **PASS** | QBO Purchase mirror pull writes rows | Neon lucia 2026-07-25: qbo_purchases=27984; sync_runs qbo_purchases_inbound_mirror success×2 last 2026-07-24T23:25:26Z | #3399 |
| `ACCT-LINK-01` | **UNVERIFIED** | journal_entry_types inbound FK from journal_entries (not island) | SCHEMA-LIVE 2026-07-25: journal_entries.journal_entry_type_id + FK journal_entries_journal_entry_type_id_fkey present (mig 202607960000 applied+ledgered). inbound typed density still 0/8 JEs — needs ops backfill/picker use + browser DoD. NOT PASS (Rule 23). | #3440 |
| `ACCT-LINK-02` | **FAIL** | detail_types canonical FK from catalogs.accounts (not text subtype only) | FROZEN pending owner LINK-02: WIRE/LOCK — 144 detail_types / 0 inbound FKs; accounts.account_subtype text only. Do not wire FK without unlock (ACCT-02). / 2026-07-25: owner decision STILL OPEN — detail_types WIRE/LOCK (keep text subtype); no code. | — |
| `ACCT-LINK-03` | **FAIL** | Bill unit_id / insurance_claim_id / linked_work_order_uuid density > 0 where applicable | vendor_id=16212; unit/claim/wo = 0 | #3425 |
| `ACCT-LINK-04` | **UNVERIFIED** | expense_categories inbound FK from expense lines | SCHEMA-LIVE 2026-07-25: expense_lines_expense_category_same_entity_fkey present (mig 202608020000; column expense_category_uuid). Needs inbound density + browser. NOT PASS. | #3446 |
| `ACCT-LINK-05` | **UNVERIFIED** | posting_templates inbound FK from consumers (WF-053) | SCHEMA-LIVE 2026-07-25: posting_batches.posting_template_id + source_template_code + FK present (mig 202607950000). catalogs.posting_templates still 0 rows — needs seed/ops + browser. NOT PASS. | #3444 |
| `ACCT-SURF-01` | **UNVERIFIED** | Bills family — DoD A–E + VERIFY 1–8 on live surfaces | Frozen map docs/trackers/ACCT-08-SURF-SURFACE-MAP-2026-07-25.md. Structural DoD guard scripts/verify-acct-surf-01-bills.mjs (route+ParityDrawer+submit payload+canonical accounting.bills). Desktop Expected vs Actual: ~/Desktop/IH35-CURSOR-AUDIT/modules/accounting-surf-dod-2026-07-25.md. Stays UNVERIFIED until TRANSP+USMCA browser re-click + reverse density unit/claim/WO > 0 (Rule 23 — no structural PASS flip). | #3477 |
| `ACCT-SURF-02` | **FAIL** | Expenses — DoD A–E + VERIFY 1–8 with live rows | expenses=0 with QBO_EXPENSES_PROJECTION_ENABLED OFF by owner decision — correct-at-0 (not a flag-flip chase). Surface chrome may be structurally wired; economics stay FAIL until owner unlocks projection + Neon density. | #3433 |
| `ACCT-SURF-03` | **UNVERIFIED** | Bill payment — DoD A–E + VERIFY 1–8 with live rows | Frozen map docs/trackers/ACCT-08-SURF-SURFACE-MAP-2026-07-25.md. Structural DoD guard scripts/verify-acct-surf-03-bill-payment.mjs (step 1461): route+ParityDrawer+pay payload+payVendorBill→accounting.bill_payments+JE/bank reverse EntityLinks. Neon bill_payments density live; stays UNVERIFIED until TRANSP+USMCA browser re-click (Rule 23). | #PENDING |
| `ACCT-SURF-04` | **FAIL** | Receive Payment — DoD A–E + VERIFY 1–8 with live rows | FAIL-BY-DESIGN KEEP 2026-07-25: payments surface empty while QBO_AR_PAYMENTS_PROJECTION_ENABLED OFF by owner. Not a schema gap. Browser still required before any PASS claim. | #3433 |
| `ACCT-SURF-05` | **UNVERIFIED** | Journal Entries — type catalog wired + reverse drill | SCHEMA-LIVE 2026-07-25 (LINK-01/JE type FK). Browser DoD TRANSP+USMCA still blocked (session gate). Do NOT PASS without authenticated click-through (Rule 23). No new structural SURF guards. | — |
| `ACCT-SURF-06` | **FAIL** | Chart of Accounts / Detail Types — canonical detail_type link | text subtype only; ACCT-LINK-02 FROZEN — owner WIRE vs LOCK. SURF DoD FAIL documented; no FK PR. | — |
| `ACCT-SURF-07` | **UNVERIFIED** | Account Register + All Transactions reverse to money docs | Routes mounted (account-register + transactions); sourceRoute + detail_path reverse drill guarded (verify-acct-surf-07-register step 1462); Desktop SURF DoD UNVERIFIED browser TRANSP+USMCA click-through. | — |
| `ACCT-SURF-08` | **UNVERIFIED** | Period close / Audit trail / Posting lineage — active path + evidence | period-close→month-close Navigate alias + MonthClosePage + AccountingAuditTrailPage + PostingLineagePage mounted; guard 1463 structural DoD; Desktop SURF DoD UNVERIFIED click-through. | — |
| `ACCT-SURF-09` | **UNVERIFIED** | Factoring / Escrow / Settlements cross-links from Accounting More ▾ | Frozen map docs/trackers/ACCT-08-SURF-SURFACE-MAP-2026-07-25.md. Structural DoD guard scripts/verify-acct-surf-09-crosslinks.mjs (routes+More▾+EntityLink+canonical factoring_advances/escrow_*/driver_settlements). Desktop Expected vs Actual: ~/Desktop/IH35-CURSOR-AUDIT/modules/accounting-surf-dod-2026-07-25.md. Stays UNVERIFIED until TRANSP+USMCA browser cross-link economics + Neon density proof (Rule 23 — no structural PASS flip). LINK-02 detail_types FK FROZEN. | — |
| `ACCT-CTRL-01` | **PASS** | Parallel books — refuse invented TMS JE for qbo-sourced money | Refuse-GL paths merged; flags default OFF | #3386 |
| `ACCT-CTRL-02` | **PASS** | CoA roles resolvable under lucia + company GUC (not false-empty) | chart_of_accounts_roles=89 with lucia | #3400 |
| `ACCT-GATE-01` | **PASS** | Every money PR uses EVERY-PR checklist (VERIFY-1..8) — process closed | #3430 merged on main (sha fc1cf13); verify-steps 1430+1431 + Rule 24 live; money PRs require MODULE_PROGRESS trailers | #3430 |

Desktop audit: ~/Desktop/IH35-CURSOR-AUDIT/modules/accounting.md
