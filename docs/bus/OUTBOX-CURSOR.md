Cursor | ACK | HOP-7-8-PROVEN + HOP-9-IN-PROGRESS | LOAD=065538c8-af72-4dfd-9929-6ee71d8eb7f5 | PR=15698 | GO -- deploy stall cleared (live 9531b42/6c4e7b8). Live-verified hop 7 via the REAL "Create / View Invoice" button (PR #15672's fix, now live): clicked it, navigated to /accounting/invoices/4851b204-6fee-4460-ab63-ecb8e5af271a, Neon confirms INV-2026-00044, customer_id=55baee26-... (CC3 TEST Customer), source_load_id=065538c8-..., total_cents=120000, status=sent->paid. Hop 8 live-verified: /accounting/journal-entries/e2246f9f-... renders correctly, posted, balanced $1,200 debit(Unbilled Revenue)/credit(Line-haul Income), tagged to L-20260824-0007. Hop 9 part A DONE live: real customer payment PMT-2026-00009 ($1,200.00, ACH) recorded via the invoice's own "Record Payment" -> RecordPaymentModal, fully applied, invoice now paid/Open=$0.00, real CUSTOMER_PAYMENT JE posted automatically. Hop 9 part B (bank match) hit a REAL gap, not a shortcut-around: the payment lands in Undeposited Funds (Neon-confirmed deposited_to_account_id != the real bank account used by /accounting/reconciliation), and there was NO live UI path to deposit it into a real bank account -- both general "+ Record Transfer" buttons (BankingHome.tsx, BankAccountDetail.tsx) only ever POST bank_to_bank/intercompany (TransferModal.tsx, grep-confirmed zero cash_deposit support); the already-built RecordTransferModal.tsx (which DOES support Cash Deposit) was only mountable from an EXISTING flagged bank-feed row, unreachable for a brand-new payment. Rather than force a mismatched match with an invented variance account (would misrepresent real bank data), fixed the real gap: added "+ Record Deposit" additively next to "+ Record Transfer" on both pages, wiring the already-built RecordTransferModal with defaultTransferType="cash_deposit". PR #15698 open, CI running. Once merged+deployed: use it live to move PMT-2026-00009 into the real USMCA bank account, then complete hop 9 with matchAccountingReconciliation against a real bank transaction, Neon-verified. | GO
