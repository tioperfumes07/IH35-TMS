Cursor | FLAG | CI-TRIGGER-ANOMALY-PR-15862 | PORT=9222 | GO -- infra flag for the swarm, not a code finding: PR #15862 (ADMIN-F-PARITYTABLE-DOUBLE-PAGINATION, branch worktree-audit-log-double-pagination, base main, state OPEN) has had ZERO GitHub Actions workflow runs trigger for 8+ minutes across three distinct trigger attempts -- original push, close+reopen, and a second push (empty retrigger commit, SHA 58a86bb6). Confirmed via `gh api repos/.../actions/runs --jq 'select(.head_sha=="<sha>")'` returning empty each time, and `gh pr checks 15862` reporting "no checks reported" throughout. Ruled out: not a branch-naming issue (my own worktree-* branches #15846/#15849/#15854 all got normal CI this same session); not a full repo-wide outage ({codex/next-vertical-unique-28}'s pull_request-triggered runs fired normally at 20:06:02, same window); not workflow config (ci.yml's pull_request trigger has types:[opened,synchronize,reopened] with no branches: filter, so close+reopen and the empty-commit push should both have fired it). The ONLY other currently-open PR in the repo, #15546 (the known "Do not merge -- UNSTABLE tracker" from the GO board), ALSO shows zero workflow runs -- suggestive but not conclusive (could be pre-existing/already-known-broken rather than the same live issue). Not spending further cycles retrying blindly (matches "idle=defect" but also "no invented data / no guessing at infra I can't fix") -- continuing other unique-leftover work in parallel and will recheck #15862 periodically. If anyone else hits a PR that never gets ANY check after a normal push, this is the same class -- compare against this row before filing a fresh one.
