DEVIN-A→CC-2 | F-BAND-DATA-HONESTY-01 — exhaustive F2 sweep + F1 is_sample_data baseline ready for your health checks | GO — full reconciliation at docs/audit/WORKORDER-F-BAND-DATA-HONESTY-SWEEP-2026-09-01.md (merged on main). F2: register baseline was 6 test-named accounts; exhaustive live Neon sweep finds 24 accounts + 20 drivers + 20 customers + 47 vendors + 23 units = 134 test-named master data records. 1 account (DRIVERCASHAD896665-023) carries $1,200 GL balance with 5 postings — BLOCKING, must be reversed before deactivation. 17 active accounts with 0 balance, 5 already deactivated, 1 false positive (Antidoping-Drug Test Services is real business). F1: 14 tables carry is_sample_data (NOT NULL DEFAULT false, zero NULLs). USMCA has MORE sample-tagged than real records in 5 of 8 financial tables (expenses 151/81, bills 54/26, journal_entries 460/338, bill_payments 16/7, driver_settlements 28/19). 2 tables (mdata.equipment, mdata.units) have is_sample_data but NO create-path writer — gap for CC-1 to fix. Create-path citations with file:line for all 14 tables in the workorder. F2 check design: scan for TEST/DEMO/SEED/SAMPLE/FAKE/CODEX-TEST/CC3TEST/ARCHIVED-TEST/ZZ-SAMPLE patterns, exclude legitimate "Drug Test" business names, per-entity, shadow baseline=134, flip to blocking at zero. F1 check design: count is_sample_data=false records with test-like names per table, per-entity, shadow first. BLOCKED: owner rules on disposition of 134 records and $1,200 GL reversal.
