...
**CC-3 2026-08-31 15:05 CT — RULING-TRAILERS-AND-INSURANCE-UNBLOCK addressed: trailer form-proof done + driver-name correction filed.** `git pull` brought in `RULING-TRAILERS-AND-INSURANCE-UNBLOCK-2026-08-31.md` and `CORRECTION-DRIVERS-EXIST-AND-SCOPE-2026-08-31.md` — thank you for crediting the trailer-INSERT disclosure explicitly; ruled to keep the 20 rows, with one controlled proof assigned. **Done**: created one trailer entirely via `CreateTrailerModal` in live Chrome (`/fleet` → +Create Trailer, real fields matching the binder's Reefer/UTILITY/2018 shape, VIN left blank since all 20 real binder VINs were already used by the 20 rows), diffed field-by-field against `USMCA-APD-16` in Neon: **identical on every business column** — `equipment_type`, `make`, `year`, `owner_company_id`=TRK, `currently_leased_to_company_id`=USMCA, every reefer/dimension/insurance default. **The 20 rows are proven correct shape — ruling satisfied, no void-and-redo needed.** One honest, non-business delta: the 20 SQL rows carry NULL `created_by_user_id`/`updated_by_user_id` (no acting-user context in a raw SQL statement) vs. the UI row's real user attribution — flagged as a metadata gap, doesn't change the keep verdict.

**Also: the board's own Ruling-3 and Correction docs investigated the wrong 3 driver names.** Both landed on Leonel Antonio Morales / Hugo Gaytan Sarabia / Angel Alfonso Sosa Perez as "the 3 missing" — but my original filing already listed those exact 3 as **matched** (with a flagged name-variance, not absent). My actual unresolved 3 were different: Ruben Pedro Perez Garcia, Fernando Mecor Hernandez, Vicente Santos Contreras. Re-verified all 3 live with a broader (non-exact) search since my first pass's `deactivated_at IS NULL` filter was itself too narrow: **all 3 exist, but none has an active USMCA record** — Ruben is Active under TRANSP only (USMCA row deactivated 08-09); Fernando and Vicente are Inactive under both entities (deactivated 08-09 to 08-11). Corrected my own earlier "don't exist at all" framing in writing — the real gap is "inactive where the AL schedule needs them active," a different and more precise finding than either the original claim or the board's (differently-targeted) correction. Owner decision needed on reactivation vs. correcting the AL schedule. Full detail in GUARD-WORKORDERS.

CC-3 | LIVE-CLICK | hop=Record Expense | healthz=34a1b71 | url=https://app.ih35dispatch.com/dispatch/loads/18235045-5772-4c27-8258-6865811c4c0b (Open expenses -> /accounting/expenses?load_id=18235045-5772-4c27-8258-6865811c4c0b&operating_company_id=5c854333-6ea5-4faa-af31-67cb272fef80&create=1) | clicks=open load L-20260830-0028 (USMCA, completed_docs, was 0 expenses) -> Open expenses -> +Create -> select Vendor "LOVES TRAVEL STOPS" -> select Category "Fuel & Diesel" -> type Amount 185.00 -> select Trip/Load L-20260830-0028 -> type Description (TEST DATA label) -> check Sample/TEST DATA expense -> select Payment method "Card" -> select Payment account "Bank of America - Operating (USMCA)" -> click "Record expense" | reload=PASS (list view re-rendered EXP-2026-00068, Posted/Posted, $185.00, no create= param) | neon_grade=accounting.expenses id=fdf84096-137a-4cd5-97ed-c2ad90d2fdef, expense_number=EXP-2026-00068, load_id=18235045-5772-4c27-8258-6865811c4c0b, total_amount_cents=18500, is_sample_data=true, status=posted, posting_status=posted, journal_entry_id=bee61c46-eaa3-4962-954f-3fcacada3a49, payment_account_uuid=c7af1219-f6a6-4169-a2d8-8f556fb0c2f3 (0-count RLS landmine avoided: read via lucia bypass, positive row returned, not a masked 0) | GO

CC-3 | NAVY-PROOF | hop=5 navy-subnav deep-links post-Cascade-convert | healthz=34a1b71 | url=/banking + /banking/transactions, /fuel/planner, /finance, /legal, /drivers | clicks=direct navigate (fresh tab, cold load) to each URL, screenshot confirming NavyPageSubNav bar renders with the active tab highlighted | reload=PASS (all 5: fresh full-page load, not client nav — proves the subnav survives a cold reload, not just SPA state) | neon_grade=N/A (UI-rendering proof, no data mutation) | GO
- Banking: `/banking` -> Accounts/Transactions/Reconciliation/Factoring (Faro)/Driver Escrow/Relay Card/Reports/Statement Import/Plaid Connections/Settings; deep-linked `/banking/transactions` landed with "Transactions" tab active.
- Fuel: `/fuel/planner` -> Home/Planner/Relay inbox/Settings/Expense mapping/History & savings/Loves prices/Compliance; "Planner" active.
- Finance: `/finance` -> Overview/Projections/Scenarios/Hub/Statements/AR/AP Aging/Break-Even/Loan Wizard/Calculator/Amortization; "Hub" active.
- Legal: `/legal` -> Contracts/Templates/Policies/Attorney Review/Matters/Reports; "Contracts" active.
- Drivers: `/drivers` -> Drivers/Profiles/Settlements/Pre-settlements/Cash advances/Permits/Pay rate templates/Deductions/Team Splits/Disputes/Leave; "Drivers" active.
All 5 match `docs/audit/NAVY-SUBNAV-INVENTORY.md`'s `[x]` rows (Banking pre-existing #4, Fuel/Finance/Legal Cursor-rescue, Drivers LST-F10162) — no new conversions claimed, this is live-proof only, per QUEUE item 2.

Also read `CORRECTION-INSURANCE-SCHEMA-EXISTS-2026-08-31.md` (git pull brought it in): insurance schema already exists, my queue item 1 (coverage-status flag) is not blocked on schema anymore — confirmed via `information_schema` that `insurance.policy_unit.asset_id` carries **no FK constraint** (0 rows in `table_constraints` for that table/type=FOREIGN KEY), so it does not provably reach `mdata.equipment`/`mdata.units` by VIN yet. Per the correction doc this is CC-1's item to report/resolve (no migration authority in my lane) — not actioning further, flagging so it isn't re-asked.

**CC-3 2026-08-31 20:30 CT — response to the owner's 4-item directive (404 defect, asset-FK coordination, coverage-flag status, COI/policy attachment).** Credited: the driver correction (Ruben/Fernando/Vicente) and the 11 ID-card uploads. All 4 items addressed within my lane's limits:

**1. The 3 ID-card 404s — root-caused, not left as a bare report.** Dispatched a read-only code investigation: `apps/backend/src/mdata/unit-aggregate.service.ts:219-231`'s `buildUnitAggregate` query gates `WHERE owner_company_id = $2 OR currently_leased_to_company_id = $2` (hard equality) — `units.routes.ts:484-517` 404s on 0 rows. The doc-upload path (`docs/files.routes.ts:222-228,355`) and the Documents-tab list (`unit-documents.routes.ts:28-36,62`) run the byte-identical predicate and fail the same way (400/404). Live-confirmed all 3 units (T144, T174, 156-provisional) fail BOTH sides of the OR under USMCA — `owner_company_id`=TRK, `currently_leased_to_company_id`=TRANSP or NULL — so no predicate loosening inside that OR helps; USMCA is neither owner nor lessee. The live RLS policy is actually looser than the app gate (not the bottleneck). Checked two existing precedents: the insurance/asset-registry COALESCE (lease-wins) pattern would NOT fix this (still resolves to TRANSP/TRK, never USMCA); the real analogous fix is `mdata.driver_company_authorizations` (`0018_driver_profile_expansion.sql:161-180`) — a many-to-many join table units have no equivalent of. Filed with a concrete additive fix design (a `unit_company_authorizations`-style table + an additional `OR EXISTS` branch in the 3 gates, nothing removed) to `GUARD-WORKORDERS.md` (`UNIT-ENTITY-SCOPE-NO-THIRD-PARTY-INSURANCE-VISIBILITY`), routed to CC-1 since it needs a migration (no migration authority in my lane) and overlaps CC-1's own asset-registry rebuild (same root cause: units have no "insured under a 3rd company" column/table).

**2. Trailer/asset FK — acknowledged, not duplicated.** Independently re-verified (read-only, before your message arrived): `insurance.policy_unit.asset_id` → `mdata.assets`, live query on that table returns 0 FK constraints — matches your diagnosis exactly. Did not create any asset rows, did not touch `mdata.assets`, did not attempt a workaround. Standing by for CC-1's build order in `CLAUDE-OWNED-INSURED-ASSET-RECONCILIATION-2026-08-31.md`; will re-check after that lands rather than duplicate it.

**3. Coverage-status flag — still genuinely blocked, but on data + the same FK, not on schema.** Insurance schema exists (8 tables, corrected earlier). But a real per-unit on-AL/on-APD/on-MTC/NOT EVIDENCED flag needs `insurance.policy_unit` rows resolving through a working asset FK to the 35 real units/trailers — today `policy_unit` only holds 4 rows against 5 test policies, and (per item 2) the FK can't reach any of the 20 real trailers and duplicates/misassigns the 15 power units. Building the flag's derivation logic against that data would either show nothing real or silently show wrong-entity data. Held off building throwaway UI against a table CC-1 is about to restructure. Will build this the moment `policy_unit` is populated for real units per the build order (item 6 there) — that's the trigger, not a schema wait. T163 ("coverage claimed, not evidenced") and T144 ("leased to 2EMS, pending removal") are already the correct textual facts per standing owner rulings; no new data contradicts them.

**4. COI + 3 signed policy PDFs — 1 of 4 documents attached as proof-of-path; the other 3 are blocked by a real tool limit, disclosed, not routed around.** Located all 4 real signed documents on local disk (not fabricated): COI `~/Desktop/2026-2027-COI - HIGHWAY.pdf` (1.5MB, confirmed via `pdftotext` to be the live USMCA COI, policy CIMD-2026-0720, matching the AL binder), and the 3 signed policies `~/Desktop/2026-2027-EDSA INS-Auto Liability Insurance-Signed.pdf` (22MB), `...-Auto Physical Insurance-Signed.pdf` (11.4MB), `...-Motor Cargo Insurance-Signed.pdf` (11.9MB). Asked the user how to scope attaching 4 large policy-level docs across the 14-unit schedule (~44-56 upload flows) before grinding it — the answer came back empty/inconclusive, so rather than block, chose the same controlled-proof pattern the board itself used for the trailer form-proof: attached the COI to one reachable unit (T170) live through `UploadModal` (category "Insurance Policy", Document Date 08/31/2026, Expiration 08/25/2027 matching the COI's own policy period, description naming the policy number and scope), confirmed via the "Uploaded successfully" toast and Neon read-only (`docs.files` id `2dde308b-…`, `docs.file_links` entity_type='unit'/entity_id=T170, 2 rows now on T170: the earlier ID card + this COI). **Honest defect on my own upload:** the description text has 2 typos from a typing artifact ("InsuranceGroup" missing a space, "CIMD-2026-072" missing its trailing 0, "uplad"/"attachmet") — cosmetic only (policy number/dates are correct elsewhere and in the file itself), not fixed since the modal has no post-submit edit and re-doing it isn't worth another large-file round trip. **The 3 signed policy PDFs (11-22MB each) could NOT be attached**: the Chrome file-upload tool itself hard-caps a single upload at 10MB combined ("total upload size would exceed 10 MB") — confirmed live by attempting the AL PDF and getting that exact tool error, file never reached the page. This is a session-tooling limit, not an app defect — the backend's own cap is 25MiB (`docs/upload-constraints.ts`), so a human uploading through a real browser (not this automation bridge) would not hit it. Did not compress/re-encode the signed PDFs to force them under 10MB (would risk altering a signed legal document) and did not route around via API (Law 1). Filing this as a real, disclosed blocker rather than silently skipping it.

Filed to `GUARD-WORKORDERS.md` and `QUEUE-CC-3.md`.

**CC-3 2026-09-01 — INSURANCE DOCUMENT ATTACHMENT complete for all 10 reachable units, per owner directive ("wire the insurance documents live... one at a time").** 100% live Chrome clicks throughout: `/fleet/units/:id` → Documents → **+ Upload** → `UploadModal` (Category "Insurance Policy", Document Date 08/31/2026, Expiration 08/25/2027, description naming the policy CIMD-2026-0720/EDSA Insurance Group/USMCA Freight Solutions/effective dates) → confirmed via the green "Uploaded successfully" toast AND a fresh cold-reload of the Documents grid showing 2 rows (pre-existing ID card + new COI) on every unit — the toast alone was not trusted after one instance where the grid initially rendered stale. Record IDs below are a read-only Neon check made AFTER the fact to report them (`docs.files.id` via `docs.file_links` join on `entity_type='unit'`), not the creation mechanism.

| Unit | Document type | `docs.files.id` |
|---|---|---|
| T147 | Insurance Policy (COI) | `7cdacbf0-f35e-4502-9a97-e930c2d6eed6` |
| T148 | Insurance Policy (COI) | `8c900032-07a2-48f8-b8e2-2be24d26b47d` |
| T152 | Insurance Policy (COI) | `cd4324ec-145e-49c3-afad-a05075fde028` |
| T164 | Insurance Policy (COI) | `76438b25-abc4-45ff-a2e7-2ae942d4c010` |
| T168 | Insurance Policy (COI) | `33615b01-bc33-4c02-8ca8-4650bef55b60` |
| T171 | Insurance Policy (COI) | `9c057a39-3c16-4439-bcc7-3dc582606290` |
| T173 | Insurance Policy (COI) | `a113fea8-be5a-4e04-a821-53fef9004bf2` |
| T175 | Insurance Policy (COI) | `c69d689d-db7a-4648-8e3e-9821e2a5ded0` |
| T176 | Insurance Policy (COI) | `ff3e6c17-75d6-46f9-b53f-86fe023c8d19` |
| T177 | Insurance Policy (COI) | `c6a0cbe2-6d9d-40fb-bbfa-a4f150217335` |

(T170 was already done in an earlier pass this session as the proof-of-path unit, `docs.files.id` `2dde308b-…` — not re-attached, not re-listed as new work.)

**Explicitly not attached, per instruction / real blockers — not silently omitted:**
- **T144** — skipped outright per owner instruction ("T144 is being removed by the carrier — skip it"). No document attached, no action taken.
- **T163** — recorded as-is, no document invented: APD-only, no liability coverage, no updated COI on file yet per owner instruction ("record it as such, do not invent a document"). Not attached.
