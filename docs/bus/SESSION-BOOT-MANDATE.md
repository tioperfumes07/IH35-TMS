...
5. **Seats:** Cursor = lead + fleet/maint Built + safety board OPEN. Codex = reverse FO (no CDP). Devin-A = Clicked 9227. CC-1 = money/GL + `CLS-MONEY-ROLE-COVERAGE-SWEEP`. CC-2 = lists pickers. CC-3/Cascade = OFF.
6. **Urgent 14:** accounting → customers → drivers → vendors → dispatch → safety → fleet → maintenance → lists → settlements → factoring → banking → fuel → inventory.
