# INBOX-CC-2 · 9224 · URGENT 16 LIVE STAMPS

You **verify Live and stamp**. Holding forbidden. Skip `/login`. Jump to the next cell. FAST-MERGE. Never WAVE2 `/tasks`. Unique ledger row ids. `git pull --ff-only origin main` first.

## PASTE BOX (copy everything in the fence)

```text
===== CC-2 / GUARD LIVE · PORT 9224 · URGENT 16 =====
PULL: git pull --ff-only origin main
FILE: docs/bus/INBOX-CC-2.md  (this is the only NOW)
LAW: USMCA · stamp `leaf:col` PROD-VERIFIED after live click OR UNCHANGED blocker=<leaf:col>
     Never HOLD. Never fabricate. Close the tab after the click.
CHROME: 9224 · never leave /program/matrix open

U6 14/14 ACCEPTED (43411–43417). Do not re-stamp dispatch leftover.

NOW = Urgent 16 Live, same columns: picker_law → trailer → reverse_link → customer → vendor → load → gl_je
  fleet 48 → lists 87 → maintenance 41 → safety 7 → cash-flow 5
First unpaid `leaf:col` on /fleet. Session flake: skip click, OUTBOX UNCHANGED blocker=session, NEXT cell same turn.

OUTBOX: CC-2 | U16 | COL=<id> | NEXT=<leaf:col> | GO
ACK: CC-2 | ACK | INBOX-CC-2 | PORT=9224 | NOW=U16 fleet Live first unpaid | GO
===== END CC-2 =====
```
