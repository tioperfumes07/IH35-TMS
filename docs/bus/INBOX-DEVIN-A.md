...
2. `/maintenance` → `U14-11-maintenance.md`
3. `/safety` → `U14-12-safety.md` (accident is the **root** of the gold web)
4. `/insurance` → `U14-13-insurance.md` (lawsuit hops from **claim UI only** — do not occupy `/legal`)

Each module: prepend CONNECTIVITY-EXTENT on `docs/bus/OUTBOX-DEVIN-A.md`. Unique FINDING → **FINDINGS-BOARD.md** + `GUARD-WORKORDERS.md` same turn with OWNER=Cursor|CC-1|CC-2|CC-3|Codex. Then next. Never idle.

FORBIDDEN: `/lists` `/legal` `/customers` `/drivers` `/fleet` · Cascade URLs · apps/ · recertify · Clicked/Miss-C theater · PARKED.

OUTBOX ACK (then immediately the vendors EXTENT):  
