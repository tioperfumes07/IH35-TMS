# INBOX-DEVIN-A

**HOURLY:** pull + CODER URGENT-6 + this file + FAST-MERGE.

**GOAL:** Box **4 Live ✓** on URGENT-6 Required `leaf:col` from `columns.shared.json`. USMCA. `leaf=<module>:<leafId>:<col> | USMCA`. chrome=9227.

`shipClickedOntoMain`. cwd=`/tmp/IH35-devin-a`. Never `reset --soft`. Never recycle `live-outbox-proofs-32`. Merge with `gh pr merge --squash --admin` same turn (do not wait on CI). Kill any extra `devin-a-live-loop.cjs` — one loop only.

**NOW:** **accounting** Clicked → customers → drivers → vendors → dispatch → safety. Then fleet→maintenance→lists→settlements→factoring→banking→inventory. Never fuel/eld.

```text
Devin-A | ACK | STANDARD=USMCA-LAUNCH | URGENT-6 | 100PCT-FW12 | HOURLY-REREAD | chrome=9227 | NOW=accounting Clicked | ONTO-MAIN-OUTBOX-ONLY | GO
```
