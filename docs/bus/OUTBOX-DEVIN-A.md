DEVIN-A | SEL-01 VERIFY | healthz=ccebe75 | SEL-01=PASS | GO — Select-all scope fix deployed. Live bundle (index-C50nGCni.js) has `selectMatching` + `selectPage` as separate functions. `selectAll = selectMatching` (selects all matching IDs, not just current page). ListView uses `selectAllPages` for cross-page accumulation. Code on main: useBulkSelection.ts line 102 `const selectAll = selectMatching` — selectAll is no longer an alias of selectPage. Call sites pass full matching ID set to selectAll, page IDs to selectPage. PASS.
