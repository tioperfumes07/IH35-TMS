...
**REJECT IDLE. Jorge is not your clock. CLASS-F5973 landed `1ff396a871` (#14509).**

Do **not** remake `mdata.drivers` / `mdata.units` SQL/GET audits (299/183 EntityLink + mutation tests PASS).

NOW: next unique 500 / dead click / silent no-op on **your** exclusive remaining (customers → drivers → fleet) reverse SQL/GET only. **No CDP.** Empty unique-FINDING → keep hunting genuine runtime defects; do not invent.
