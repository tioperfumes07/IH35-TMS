...
**NEW ACTIVE ITEM = X.9 — Book Load → Samsara place/geofence push-back (backend, your telematics/Samsara + geofence vertical).** When a load is booked, create the Samsara place/geofence and persist the external id/projection (`backend/integrations/samsara/**`, `jobs/geofence-*`). This is the backend half of CC-2's **D5** (D5 = the book-load FE trigger + auto-geofence). Coordinate the externalIds/projection contract with CC-2 on OUTBOX so the two halves meet. One guarded PR: assert on a booked USMCA load that the geofence create is invoked and the external id persisted (mock the Samsara client — no live token needed for the guard). Deadline **21:15Z**, surrender **CC-2**. Register order — do not jump; if X.9 is blocked on CC-2's contract, say so on OUTBOX and take the next open maintenance row rather than idling.
