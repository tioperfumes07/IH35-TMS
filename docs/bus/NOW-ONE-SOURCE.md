...
**22:18 CT GO — SPINE NOT CLOSED. Owner: fix to perfection now. Split: FIXERS vs SCENARIO TESTERS. Program tracker must move on the same TEST UUIDs.** Live API until deploy lands = `9531b42`. In-flight API `dep-da6gklu7bikc738hkn4g` tip `20c02fde` (#15695 trailer swap). Hard-reload when healthz=`20c02fd`. Never `trigger_deploy`. U14 never restamp.

**PROFORMA vs CASH FLOW (measured, not guessed):** A **proforma invoice is not cash and is not A/R.** `/reports/cash-flow-overview` expected AR is `status IN ('sent','partial')` — proforma **excluded**. `/cash-flow` Daily Prediction / Actual-vs-Projected uses **load `rate_total_cents`**, not the invoice document — the load can still show as **projected income**. **`/accounting` cash forecast** currently **does include** open proformas (no status filter): USMCA `INV-2026-00046` $2,500 open due 2026-09-24, plus `INV-2026-00036`/`00035`. That is **CASH-FORECAST-INCLUDES-PROFORMA** → CC-1. Relay invoice `INV-2026-00044` (`4851b204-…`) on load `L-20260824-0007` is now **`paid` / open $0** — it is **not a proforma anymore**; it must not sit on Open A/R.

**FIXERS (code, FAST-MERGE, name JE/UUID):** Cursor = `BOOK-LOAD-NOOP` (form click → POST → `mdata.loads`; geocode must write lat/lng). CC-1 = expense `57cabbab-…` JE; invoice-send vs Event-2 A/R JE; cash-forecast proforma exclusion. CC-3 = `parts_receive` on WO `850e2cc4-…`. Codex = prove accident spawn-WO + trailer swap **live** after this deploy, then next unique FK. Do not remake BILL-2026-00015 / Close / `/425c`.

**TESTERS (walk + Program cards green, no idle):** Cascade + Devin-A walk hops 1–9 + breakdown battery; file unique FINDING only. CC-2 bind `/cash-flow` `/reports` `/finance` to the same TESTs; prove proforma **not** on A/R aging; prove Program hop Now: states. Every hop OUTBOX must name `TABLE` + `UUID` + Program key. If a card stays `--` after the write, that is a FINDING.
