# NOW — ONE SOURCE (owner 2026-08-24 07:33 CT)

**This file is the only NOW.** Every session, every seat: `git pull --ff-only origin main` then this page + `INBOX-<SEAT>.md` TOP.

**LEFTOVER-6 leftover-CERTIFIED** at live `6585a66`. Do not recertify.  
**LEFTOVER WAVE 2 leftover-CERTIFIED** at live `6585a66`. Do not recertify.  
**LEFTOVER WAVE 3 leftover-CERTIFIED** at `6585a66` (`/customers` `/drivers` `/fleet`). Do not recertify. Not U14 restamp.  
**LEFTOVER WAVE 4 leftover-CERTIFIED** at live `189dd4c` (`/425c` `/vendors` `/lists` `/legal` `/safety`). Pack `docs/bus/LEFTOVER-WAVE-4-CERTIFY-NOW-2026-08-24.md` is a stamp table. Do not recertify. Not U14 restamp.  
**THIS TURN:** next unique leftover FINDING only (500 / dead click / silent no-op). CC-1 next unique money FINDING. Never restamp U14. Do not loop `/425c`.  
If a seat claims “no instructions” / poll-mode, they did not pull. Idle = defect.

**U14 14/14 CERTIFIED** = exclusive 14-module pass finished — not stop. Never restamp. Never recertify.

**AFTER URGENT 14:** `docs/lockdown/POST-URGENT-14-MODULE-SEQUENCE-2026-08-23.md`  
Paste pack: `docs/bus/PASTE-ALL-SEATS-NOW.md`.

If any other bus file disagrees — **this file + POST table win.**

Live SHA: `curl -sS https://api.ih35dispatch.com/api/v1/healthz/shallow` → `version` (**re-curl**).

---

## Law that is still in force (do not re-ask)

- USMCA only. No TRANSP/TRK. No TMS→QBO write-back.
- U14 exclusive table is **14/14 CERTIFIED**. **Never restamp. Never recertify.**
- CERTIFIED ≠ Rule 24 `complete` ≠ matrix Box 4 Live.
- Fully-Wired **1–12**, Live Chrome last. Unique 500 / dead click / silent no-op. One small PR per finding.
- CREATE-TEST-THEN-VOID. Do not remake proven TESTs / Close / Book Load.
- FAST-MERGE ~4 min. Never `gh pr checks --watch`. Deploy: Cursor lead only, every **10 minutes AND 10 PRs**, one in-flight. CC never `trigger_deploy`.
- HOLD / idle / “awaiting next order” / “no instructions” = defect.

---

## Chrome (one NOW per seat)

| Seat | Port | NOW (one URL / one id) | Role |
|------|------|------------------------|------|
| **Cursor** | **9222** | lead + FAST-MERGE | WAVE 3 leftover-CERTIFIED. Never leftover-6/W2/U14 restamp. |
| **CC-1** | **9223** | next unique money FINDING | L1–L2 leftover-CERTIFIED. STOP `/425c`. |
| **CC-2** | **9224** | next unique leftover FINDING | W1 `/fuel` leftover-CERTIFIED. No Close remake. |
| **CC-3** | **9225** | next unique leftover FINDING | W2 `/maintenance` leftover-CERTIFIED. lists+legal omit. |
| **Codex** | **9226** | next unique leftover FINDING | W11–W13 leftover-CERTIFIED. Not U14 restamp. |
| **Cascade** | audit Chrome | leftover unique FINDING only | AUDIT ONLY. WAVE 3 stamped. |
| **Devin-A** | audit Chrome | leftover unique FINDING only | AUDIT ONLY. Not PARKED. |

Forbidden steal: do not occupy another seat’s NOW. Do not remake Close / F6301 / fleet #15291–#15310 / fuel phantom #15335.

---

## Superceded — do not take orders from

Stale Desktop INBOX that says leftover-6 / WAVE 2 / WAVE 3 is OPEN, or that U14 is OPEN.
