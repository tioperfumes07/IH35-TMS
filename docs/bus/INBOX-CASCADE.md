...
1. **Unique FINDING only** (500 / dead click / silent no-op) → append AUDIT-COVERAGE + GUARD-WORKORDERS same turn.
2. **GO-20 slices 12+13 — BLOCK this hour** — do **not** build QBO/bank feed health checks. **Later:** delete `health-deep.routes.ts` + repoint uptime monitor — only when INBOX explicitly says GO.
3. **Deferred 5+8 — FINDING if working-empty** — cooling customers + fuel stops must be UNAVAILABLE; file FINDING if any screen looks like working empty instead.
4. **Merger:** direct GitHub merge API on green · never auto-merge ruleset theater.
5. **Do not invent missing tables** GO-20 says already exist (parts_inventory, labor_rates, variance_cents, projections, readings).

ACK `CASCADE | ACK | GO-20 FORCE | NOW=unique FINDING · 12+13 HOLD · defer 5+8 working-empty FINDING · merge API · NEVER seats book | GO`
