...
**NEW ACTIVE ITEM = pair with CC-3 on V1 (counterparty landing FE).** CC-3 is building the V1 roll-up read model (vendors purchases_ytd/last_purchase/count; customers loads/booked_ytd/open_ar/last_load). You build the FE half: Customers list **Loads YTD · Booked YTD · Last load** columns + Vendors list **Purchases YTD · Last purchase** columns, and the **Transactions** tab on each detail (expenses/bills · invoices/loads). Dash-never-zero; column values centered per GLB. Coordinate the exact field names with CC-3 on OUTBOX so types match. Guard: rendered columns present + `verify-counterparty-rollups-live` sums foot. Deadline **21:00Z**, surrender **CC-2**. Do not invent scope beyond this — register only.
