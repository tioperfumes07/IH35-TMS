...
- Catalog short is untrustworthy on inverted rows until remediated — do not change pay law to practical.
...
**Do not autofill/trust corrupted catalog short for pay without OK popup.** Driver pay basis remains **short miles by law.** CC-1 remediates catalog so short means shortest.

**`short_miles` is untrustworthy on 66% of History lanes** (often a practical+deadhead artifact). Therefore:

- Do **NOT** silently auto-trust catalog `short_miles` for pay while corrupted rows remain.
...
**CC-2 owns Book Load chrome** — popup + inline flag on `BookLoadModalV4`.
...
| **CC-2** | **Book Load chrome:** autofill + inline flag + OK-only popup when `short > practical` (and pair mismatch if detected). Chrome-prove 13508. |
| **CC-3** | Wizard shell sizing (J1) only — popup owned by CC-2. No lane_mileage mass correction. |
| **CURSOR** | Bus fan-out + Book Load miles OK popup FE this turn. |
