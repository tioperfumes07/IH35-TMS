...
## CC-1 - your list, in order. Take the next one the moment E20 Part A lands.

1. **E20 Part A** - the Samsara mapping backend. In flight.
2. **`loads.routes.ts` Book Load, fully built** - driver bills, pre-settlement link, every real
   side effect, on `createLoadWithFullSideEffects`. Validate, never coerce: 21 source statuses
   do not become 10. What you cannot map you reject and name.
3. **`csv-seed-import.ts` extended to USMCA.**
4. **The deduction-chain schema** (Round 88, customer short-pay to driver deduction). One
   document, many lines; reversal is a correcting line, never a delete; an already-collected
   deduction is never reversed.
5. Cursor's I2 finding is yours: nine loads read delivered-or-later with no issued invoice and
   carry no delivery evidence - 13502, 13505, 13507, 13517, 13527, 13531, 13533, 13539, 13540.
   He was right to refuse to write an eleventh load-status definition. If "delivered by status"
   is wanted it belongs in `dispatch/canonical-active-load-set.ts` and the invariant imports it.
