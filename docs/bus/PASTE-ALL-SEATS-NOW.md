# PASTE ALL SEATS · NO DEVIATE · CONTINUOUS · HOURLY · FAST-MERGE

`git pull --ff-only origin main`

**Every hour:** re-read `CODER-INSTRUCTIONS-NOW.md` + your INBOX + `FAST-MERGE-4MIN-LAW.md`. Re-ACK. Keep working.

**WAVE 1:** banking → factoring → accounting → settlements → customers → drivers  
**WAVE 2:** insurance → legal → lists → safety → fleet → vendors → maintenance → dispatch  
**WAVE 3:** inventory → compliance → reports → cash-flow → finance → form_425 → users → docs → home → tasks → program  

Complete = Fully-Wired **1–12** (Live Chrome last = Devin). Devin Clicked = OUTBOX-only onto **current** `origin/main` (`shipClickedOntoMain` — no stale-branch revert). Gate → push → create → `gh pr merge --admin` same turn. Never `gh pr checks --watch`.

ACK: `STANDARD=USMCA-LAUNCH | WAVE1-THEN-2 | HOURLY-REREAD | NOW=<module> <col> | GO`
