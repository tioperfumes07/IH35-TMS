...
8.2 **Clone-once, then reconcile-only.** A one-time full backfill imports QBO data — **master data (customers/vendors) + AR (invoices/payments) + AP (bills/bill-payments) + GL** — into the TMS database (store-once). After backfill the QBO connection exists **only to reconcile/compare**: a twice-daily module (the two Jorge-locked scheduled passes, `TMS-QBO-RECONCILIATION.md` §2) flags rows added / voided / changed in either system. Specs: `docs/specs/QBO-CLONE-PROGRAM.md` (master data + AR/AP clone, MD-1..MD-RECON), `docs/specs/TMS-QBO-RECONCILIATION.md`, and the QBO-IMPORT GL program (IMPORT-0..4v2).
