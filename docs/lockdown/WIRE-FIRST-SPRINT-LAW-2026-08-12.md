...
> **WIRE FIRST · TEST LATER.** Complete total wiring on **all 10 priority modules** (Box 3 **Built** on every Required cell). **No Chrome audit, no system test, no Box 4 Live** until **all three boxes are green** on those modules — then we test.
...
|------|-------------------------|
| **Cursor** | Bus · deploy/CI unblock · guards · scoreboard · overflow |
| **Codex** | Primary Wave A+B FE wiring (lists · customers · vendors · dispatch · pickers · EntityLink) |
| **CC-1** | Money · GL · migrations · Neon apply (serial) |
| **CC-2** | Merge-sample checks only — **no full Live certify until 3-box gate** |
