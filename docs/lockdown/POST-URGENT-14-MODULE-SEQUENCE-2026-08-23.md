...
**THIS IS THE SEQUENCE AFTER URGENT 14. EVERY CODER. EVERY SESSION.**

U14 exclusive certify is **CLOSED — 14/14 CERTIFIED**. **Never restamp. Never recertify. Never reopen customers/drivers/fleet/lists/legal as U14 work.**

If your Urgent-14 rows are CERTIFIED, **you do not idle**. You run **this leftover table** (or your claimed NOW on it). Do not wait for Jorge. Do not watch INBOX. Do not invent a 17th queue.

Same bar: Fully-Wired **1–12** + Live Chrome last. USMCA only. CREATE-TEST-THEN-VOID. Unique FINDING only (500 / dead click / silent no-op). Grep `origin/main` before remaking. Do not steal another seat’s NOW.

Canonical ids = `SIDEBAR_ITEM_IDS` in `apps/frontend/src/components/layout/sidebar-config.ts`.

Seat NOW (one source): `docs/bus/NOW-ONE-SOURCE.md`.
...
## After-U14 NOW by seat (paste this)
