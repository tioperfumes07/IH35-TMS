...
| 4 | 425C | `form_425` | `/425c` | **Cursor** DONE -- all 5 tabs (Profiles & Defaults/Deposit Import/Form 425C/Merge & Export/History) + Exhibits A-F live-Chrome-confirmed genuinely wired. Found+fixed real defects this pass: F425C-ATTACHMENTS-CHECKBOX-NO-UPLOAD (#15053, Part 8 checkbox had no real attach path, wired the existing backend endpoint), F425C-PROFILES-BANK-ACCOUNTS-NO-REMOVE (#15059, created rows were permanent), F425C-EXHIBIT-D-NOT-A-REAL-QUARTER (#15076, U.S. Trustee 28 U.S.C. § 1930(a)(6) fee computed over the shared 1-month exhibits period instead of a real calendar quarter -- live-reproduced the understated-fee bug on the deployed app before fixing). Plus a long tail of prior-tick leftover FINDING commits this same session (save-draft silent, dirty-generate-skips-save, exhibits export/build silent, history month UTC, etc. -- see git log grep F425C). Live Chrome round-trip run: created + removed a bank account, built exhibits A/D live, opened the real OS file picker on Attach file, parsed a Deposit Import row and confirmed the honest no-match toast. Remaining known gap, not touched by this pass: verify-form425c-econ-link-pack.mjs pre-existing FAIL (ExhibitsViewer missing forward links to /accounting/reconciliation, /finance/statements, /legal/reports) -- separate leftover item, logged in #15076's REMAINING line. |
