...
> **SESSION LAW (auto-loaded every Cursor session):** Rule #0 `docs/specs/QUALITY-STANDARD-LOCKED.md` + Law of the Land `docs/specs/ARCHITECTURE-BLUEPRINT-2026-07-05.md` + always-apply `.cursor/rules/00`–`07`. Trust over speed. Verify live. No guess / patch / defer. Never delete modules — only add.
...
- **SUPERSEDED (2026-07-02) — the T11.20.6.2 write-back cuts above (customer/vendor/account/invoice/bill, all six `tms.*.push_requested` handlers) are gated OFF under the parallel-books architecture: TMS does NOT write back to QBO. Enforced by IMPORT-P0b (`QBO_ENTITY_PUSH_ENABLED` default OFF, per-entity-only, + clone-origin refusal). Historical cut records kept for reference (append-only). See `docs/specs/ACCOUNTING-ARCHITECTURE.md` + `docs/lockdown/00_LOCKED_DECISIONS.md` §8.**
