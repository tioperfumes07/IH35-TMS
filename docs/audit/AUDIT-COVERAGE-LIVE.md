...
| Driver Hub | C | TRANSP | PASS (honest-empty) | Driver Hub's financial primitive is `driver_finance.cash_advance_requests`→ same posting chain already code-verified sound in `LAYERC-settlements-GL-2026-08-02.md`. 0 rows company-wide — consistent with the same data-scarcity pattern as Settlements/Driver Profile/Safety/Compliance | 2026-08-02 | Cascade | | |
...
| Fleet | C | TRANSP | PASS (honest-empty) | Chain traced: `accounting.fixed_assets`→`accounting.depreciation_schedule_rows.posted_journal_entry_id`→`accounting.journal_entries` (clean FK-enforced, `posted` boolean gate). TRANSP has 0 rows in `accounting.fixed_assets` — data scarcity, not a defect. NOTE: while tracing this chain discovered a THIRD operating company with real activity here — see cross-cutting finding below | 2026-08-02 | Cascade | | |
| Fleet | C | IH 35 Trucking LLC (3rd entity, newly discovered) | PARTIAL — real, partially-posted | This company (`b49a737b-6cf0-43bb-8758-a6c8ff8a2c4e`) has 1 real `accounting.fixed_assets` row with 13 `depreciation_schedule_rows`; 2 of 13 periods are `posted=true` with a real `posted_journal_entry_id`, 11 of 13 are unposted (`posted=false`, null JE). The poster works when it runs, but the schedule is not being fully worked through — 11/13 periods sitting unposted is itself worth a Layer-B-style FAIL note for THIS entity once it's formally in scope | 2026-08-02 | Cascade | | |
...
| Cash Flow | C | TRANSP | PARTIAL — nuanced, not a clean derivative of Banking | `apps/backend/src/cash-flow/cash-flow.service.ts` deliberately does NOT read `accounting.journal_entries` for its historical actuals — a code comment explicitly says to source from `banking.bank_transactions` directly, "never re-sum bank_transactions for the Plaid-mixed population." Historical cash-flow also independently reads `accounting.bills`, `accounting.payments`, `accounting.bill_payments`, `driver_finance.driver_settlements`, and `accounting.cash_flow_adjustments`; projected/forward cash-flow reads `mdata.loads`/`load_stops`. Net effect: the bank-side numbers ARE reliable (Banking Layer C already proved bank_categorization/transfer↔JE is a clean 1:1, so bank_transactions itself is trustworthy), but the bills/payments/driver_settlements legs it also folds in are the SAME tables already found empty (`driver_settlements`=0) or posting-flag-gated (bills/payments — see `CLS-SUBLEDGER-GL-DARK` reclassification). Cash Flow's own math is internally consistent with its inputs, but those inputs are incomplete for the same reason already documented elsewhere — not a new defect, but worth tracking as its own re-verify item once posting flags flip | 2026-08-02 | Cascade | | |
...
| **AUDIT-SCOPE GAP: a THIRD operating company exists and has never been audited** | IH 35 Trucking LLC | OPEN (scope gap, not a code defect) | `org.companies` id `b49a737b-6cf0-43bb-8758-a6c8ff8a2c4e`, legal_name "IH 35 Trucking LLC" — discovered while tracing Fleet Layer C (fixed-assets/depreciation). This entity has REAL, partially-posted GL activity (1 fixed asset, 13 depreciation_schedule_rows, 2 of 13 posted to real journal_entries) and is also one of the 3 entities with `INVOICE_AR_GL_POSTING_ENABLED` explicitly overridden ON (see the subledger-GL-dark correction file). Every verdict in this audit to date has covered TRANSP and USMCA only — this third entity's own A/B/C/D/E layers have never been run. Flagging for owner decision: is this a real third tenant that needs its own audit pass, or a sandbox/demo company that should be explicitly excluded from scope? | 2026-08-02 | Cascade | | |
