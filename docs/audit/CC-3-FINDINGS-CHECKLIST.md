...
| **☑ fixed & signed off by a coder** | **1** (`LV-USMCA-ACTIVE-DRIVERS-HAVE-NO-PAY-RATE` — owner-assigned to CC-3 and executed by CC-3, so maker=checker here BY OWNER INSTRUCTION; every other row still awaits its lane) |
