-- GEOFENCE-ENGINE-REBUILD migration #4 (09-05-2026-Claude-Coder-3-GEOFENCE-ENGINE-REBUILD spec §2).
-- READY-TO-APPLY DRAFT, not committed by CC-3 (no-migrations lane). Handoff target: CC-1 (or any
-- authorMigrations:true seat) as his STEP 0 per the 2026-09-05 CC-3/Cursor resequencing directive.
--
-- Renumber to the next real migration number before applying (checked live at draft time; confirm
-- the actual next-free number in db/migrations/ at apply time -- do not trust this comment's
-- filename, it is not a reserved number). Idempotent throughout (DO/IF NOT EXISTS/ADD COLUMN IF
-- NOT EXISTS). Nothing deleted, nothing renamed -- void-not-delete / append-only law.
--
