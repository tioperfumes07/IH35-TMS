...
## LV-063  BANK-F10 is marked PASS on a populated FIELD, not on recovered MONEY: $316.34 of fuel-card overage is flagged `overage_recovered_cents` while every deduction table in the system is empty — nothing ever took it from a driver
...
- severity:  major (an acceptance item marked PASS while the money it describes was never recovered; $316.34 live)
