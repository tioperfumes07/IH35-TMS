...
| **FIXED — NEEDS BEHAVIOR VERIFY** `MDATA-F03` cross-entity safety-event WRITE | — | C | CC-3 built · **CC-2 to verify live** | `scopeToRelatedEntity()` derived the operating company from a **caller-supplied** `related_load_id` / `related_driver_id` / `related_customer_id`, set the GUC to it, and INSERTed a dispatcher safety event under it — with **NO membership assertion anywhere in the file** (zero matches for `assertCompanyMembership` on main before the fix). A user in company A could write a safety event into company B. **Cross-entity WRITE, not a read leak.** Fixed by asserting membership INSIDE the resolver so no call path can skip it (throws 403 `forbidden_company_membership`). **NOT behavior-verified — no two-company fixtures exist**, so this is typecheck+read verified only. **CC-2 live check: POST a dispatcher safety event with ANOTHER entity's `related_load_id` → expect 403.** | **#4541** | **FIXED — awaiting CC-2 live verify** |
| **RESOLVER RULE** (guard-scoping law, proved by MDATA-F03) | — | C | CC-3 | Sites that **DERIVE** the operating company from a supplied id (e.g. `scopeToRelatedEntity`) must be **EXEMPTED** from `verify-join-entity-scoped`, NOT given a predicate — scoping them by the value they are deriving is **circular**. The correct control there is a **membership assertion on the derived company**. A baseline entry is a pointer to LOOK, never a diagnosis: these three reads are in the 446-offender baseline and reading them as 'unscoped reads' would have produced a circular no-op while the real defect (missing authorization) survived. | #4541 | **LAW — apply when wiring 2648** |
