...
| — | F | DRIVER-SUBACCOUNT-AUTO-PROVISION — ASSET side | Per-driver advance sub-account on hire | MED | M | A3 series | 🟢 BUILT — PR __DSPR__ · on driver create, auto-create "Driver Cash Advance- <Name>" under the canonical Driver Cash Advance parent (resolved by NAME, never UUID); idempotent; best-effort (TRK chart lacks the parent → graceful no-op). LIABILITY/escrow side gated on STOP-DECISION #1; BULK backfill gated on STOP-DECISION #2. Live-CoA verified: catalogs.accounts is global=TRANSP chart; TRK has no Driver-Cash-Advance/escrow parents; USMCA excluded. |
