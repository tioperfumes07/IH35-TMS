...
- [ ] **AF-7-money-controls** (platform) — Owner per-flag OK to flip MONEY_CONTROL_* after CPA tie-out; independently confirm period-close UX + financial-RLS-per-role scope built.
...
| flow5-dual-deduction-systems-consolidate | settlements | LOCKED_DECISIONS §9.1 'Canonical deduction store = driver_finance.driver_settlement_deductions … Retire the payroll.* copy + the settlement_lines auto_deduction path.' | Consolidate the capped-recovery + legacy deductions paths onto the canonical store behind an OFF flag; owner+CPA verify before flipping (same work as 0285-df-gap2). |
