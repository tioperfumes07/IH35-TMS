# MODULE DEEP-AUDIT SCOREBOARD (30 sidebar modules)

**Law:** `docs/trackers/OWNER-EXECUTION-PLAN-2026-07-22.md`  
**Depth:** click-through + wizard chrome + reverse/forward links + economics/CoA (Jorge definition).  
**Statuses:** `NOT_STARTED` · `IN_PROGRESS` · `AUDITED` · `FIXING` · `DONE` · `DEFERRED`

**Owner-locked sequence (2026-07-22):**

| Seq | Module | Status | Desktop audit | Notes |
|-----|--------|--------|---------------|-------|
| 1 | accounting | **DONE** | `~/Desktop/IH35-CURSOR-AUDIT/modules/accounting.md` | **START** |
| 2 | bank | **IN_PROGRESS** | `~/Desktop/IH35-CURSOR-AUDIT/modules/banking.md` | accounting 31/31 closed; density + transfer smoke |
| 3 | safety | NOT_STARTED | — | |
| 4 | lists | NOT_STARTED | — | |
| 5 | maintenance | NOT_STARTED | — | |
| 6 | insurance | IN_PROGRESS | partial A2 | #3228 HOLD + entity-scope fix |
| 7 | legal | IN_PROGRESS | partial | #3221 merged FE |
| 8 | dispatch | NOT_STARTED | — | |
| 9 | settlements | NOT_STARTED | — | #3224 reverse merged; full audit still due |
| 10 | factoring | NOT_STARTED | — | |
| 11 | vendors | NOT_STARTED | — | |
| 12 | customers | NOT_STARTED | — | |
| 13 | drivers | NOT_STARTED | — | |
| 14 | driver-hub | NOT_STARTED | — | |
| 15 | fleet | NOT_STARTED | — | |
| 16 | cash-flow | NOT_STARTED | — | |
| 17 | finance | NOT_STARTED | — | |
| 18 | home | NOT_STARTED | — | Last wave |
| 19 | fuel | NOT_STARTED | — | Last wave |
| 20 | form_425 | NOT_STARTED | — | Last wave |
| 21 | reports | NOT_STARTED | — | Last wave |
| 22 | tasks | NOT_STARTED | — | Last wave |
| 23 | inventory | NOT_STARTED | — | Last wave |
| 24 | docs | NOT_STARTED | — | Last wave |
| 25 | users | NOT_STARTED | — | Last wave |
| 26 | help | NOT_STARTED | — | Last wave |
| 27 | program | NOT_STARTED | — | Last wave |
| 28 | system | NOT_STARTED | — | Last wave |
| 29 | eld | NOT_STARTED | — | Last wave |
| 30 | **compliance** | NOT_STARTED | — | **Owner: absolute end** |

**Control-plane:**

| Item | Status |
|------|--------|
| verify-reverse-drill-required (marker contract, not keyword) | DESIGNED — build next |
| held-migration ↔ code recon | NOT_STARTED |
| hold-merge-gate v2 | NOT_STARTED |
| entity-scope burn-down (USMCA) | ACTIVE debt — #3228 proved new leaks still appear |
| sidebar docs 28→30 | NOT_STARTED |

Last updated: 2026-07-30 — accounting DONE (PR #3834); bank IN_PROGRESS
