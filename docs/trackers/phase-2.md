...
| 2026-05-05 | BT-2-PWA-RLS-DRIVER-INSERT (P2-T3.1.1): Driver PWA upload returned HTTP 500 (RLS violation) on production despite P2-T3.1 entity_id fix. Root cause: docs.files INSERT RLS behavior in production was not explicitly granting Driver writes for self-linked uploads. Added migration 0030 with explicit driver INSERT and SELECT policies, scoped to company access via org.user_company_access and self identity via mdata.drivers.identity_user_id. | Jorge | Resolved | Hot-fix from production smoke test |
| 2026-05-05 | BT-2-FMCSA-VERIFICATION (P2-T4): Free FMCSA SAFER lookup integration. Office UI button on Customer create/edit to verify broker authority by USDOT or MC#. Result cached 7 days in catalogs.fmcsa_lookups. Customer record stores fmcsa_verified_at, fmcsa_lookup_id, fmcsa_authority_status_at_verification. UI shows green badge when verified. Critical for vetting brokers before contract — federal violation to work with non-authority brokers. NO automatic re-verification yet (Phase 6). NO carrier lookup (only broker — Phase 6). | Jorge | Resolved | Phase 2 task 4 of 5 |
| 2026-05-05 | BT-2-PHASE-2-GATE (P2-T5): Phase 2 Documents Module CLOSED. 7 planned tasks + 4 hot-fixes + 1 outbox processor cycle (unplanned, P1 gap discovery). Migration 0032 formalized file_links driver policies (had been hot-patched directly on production). All 20 expected verify scripts pass. Audit events 92→97. Migrations 28→32. Phase 1 retroactive note added re: outbox processor gap. SCHEMA_DEPRECATIONS.md created for outbox.outbox_queue. Production-blocking tasks captured for Phase 3: driver onboarding flow, backend operating_company_id resolution, verify-script fixture cleanup. | Jorge | Resolved | Phase 2 Gate CLOSED — Phase 3 begins May 6 |
