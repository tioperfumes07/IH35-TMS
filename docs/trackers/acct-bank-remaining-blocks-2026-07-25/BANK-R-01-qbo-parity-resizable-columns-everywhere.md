# BANK-R-01 — qbo-parity-resizable-columns-everywhere · Customers/Vendors/Drivers list pages (and most of the remaining 28-module catalog beyond t
**FINDING:** BANK-R-01 / pile `qbo-parity-resizable-columns-everywhere` (P2) · **Lane:** NON-FINANCIAL · **Module:** banking.
...
Spec sources reviewed: docs/specs/CURSOR-PERMANENT-RULES.md · docs/specs/IH35_MASTER_BLUEPRINT_v3_FULL.md (module banking) · docs/specs/IH35_UNIFIED_BLUEPRINT_ADDITIONS.md · docs/specs/IH35_ARCHITECTURAL_DESIGN.md (module banking) · docs/lockdown/00_LOCKED_DECISIONS.md (if financial) · docs/specs/LAW-OF-THE-LAND-COMPLETE-2026-07-25.md (index; source rule wins on conflict)
Approved screens reviewed: docs/approved-screens/4Banking_Homepage.png
Tab count check (Rule 05): design says N tabs for banking · this block changes count to same N unless Fix explicitly adds a design-approved leaf (then update design same commit)
...
OPEN: Customers/Vendors/Drivers list pages (and most of the remaining 28-module catalog beyond the 9 confirmed files) do not use TableHeaderCell sort+resize.
...
1. Canonical target: `banking.* (declare exact table at build)` — verify `to_regclass` on prod before write/FK.
...
Root-cause Fix for `qbo-parity-resizable-columns-everywhere` — not a patch:
...
`scripts/verify-qbo-parity-resizable-columns-everywhere.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
MODULE_PROGRESS: banking 4 of 13 (M may grow if this FAIL is new leaf — Rule 21)
ITEMS_TOUCHED: qbo-parity-resizable-columns-everywhere
...
ROOT CAUSE: qbo-parity-resizable-columns-everywhere still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
