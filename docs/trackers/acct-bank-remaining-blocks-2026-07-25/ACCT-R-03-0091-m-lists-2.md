...
**FINDING:** ACCT-R-03 / pile `0091-m-lists-2` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: 'Merge accounts' does not merge — handleMerge (CoaBatchActions.tsx:55-80) only deactivates the source accounts; catalogs.accounts GL references (accounting.journal_entry_postings lines, child accounts, catalogs.items.def
...
Root-cause Fix for `0091-m-lists-2` — not a patch:
...
`scripts/verify-0091-m-lists-2.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ROOT CAUSE: 0091-m-lists-2 still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
