...
**FINDING:** ACCT-R-23 / pile `flow3-cancellation-billing-deduction-linkage` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: No cancellation_id FK column on accounting.invoices or driver_finance.driver_settlement_deductions; no trigger wiring driver-fault cancellations to either. [closure needs live proof]
...
Root-cause Fix for `flow3-cancellation-billing-deduction-linkage` — not a patch:
...
`scripts/verify-flow3-cancellation-billing-deduction-lin.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: flow3-cancellation-billing-deduction-linkage
...
ROOT CAUSE: flow3-cancellation-billing-deduction-linkage still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
