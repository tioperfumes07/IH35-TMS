# ACCT-R-16 — audit8-revenue-leakage-detection · Revenue leakage detection + unbilled-revenue tracking/forecasting/variance views — confirm
**FINDING:** ACCT-R-16 / pile `audit8-revenue-leakage-detection` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: Revenue leakage detection + unbilled-revenue tracking/forecasting/variance views — confirmed absent.
...
Root-cause Fix for `audit8-revenue-leakage-detection` — not a patch:
...
`scripts/verify-audit8-revenue-leakage-detection.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: audit8-revenue-leakage-detection
...
ROOT CAUSE: audit8-revenue-leakage-detection still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
