...
**FINDING:** ACCT-R-41 / pile `flow2-customer-chargeback-driver-expense` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
Root-cause Fix for `flow2-customer-chargeback-driver-expense` — not a patch:
...
`scripts/verify-flow2-customer-chargeback-driver-expense.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: flow2-customer-chargeback-driver-expense
MIGRATE: N/A until owner unlock
ROOT CAUSE: flow2-customer-chargeback-driver-expense still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
