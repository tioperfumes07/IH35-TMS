...
**FINDING:** ACCT-R-07 / pile `0280-42-wo-to-expense-flow` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: WO status-count widget has no join to accounting.bills to show expense/bill linkage per work order
...
Root-cause Fix for `0280-42-wo-to-expense-flow` — not a patch:
...
`scripts/verify-0280-42-wo-to-expense-flow.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0280-42-wo-to-expense-flow
...
ROOT CAUSE: 0280-42-wo-to-expense-flow still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
