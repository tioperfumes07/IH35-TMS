...
**FINDING:** ACCT-R-33 / pile `0473-1-8-tk-transp-lease-asc842` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
Root-cause Fix for `0473-1-8-tk-transp-lease-asc842` — not a patch:
...
`scripts/verify-0473-1-8-tk-transp-lease-asc842.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0473-1-8-tk-transp-lease-asc842
MIGRATE: N/A until owner unlock
ROOT CAUSE: 0473-1-8-tk-transp-lease-asc842 still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
