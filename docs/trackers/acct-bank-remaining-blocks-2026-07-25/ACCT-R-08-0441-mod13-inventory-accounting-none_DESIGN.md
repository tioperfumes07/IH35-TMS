...
**FINDING:** ACCT-R-08 / pile `0441-mod13-inventory-accounting-none_DESIGN` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: parts-inventory has no FK/write path to accounting.expenses, accounting.bills, or any GL account — total absence of accounting linkage
...
Root-cause Fix for `0441-mod13-inventory-accounting-none_DESIGN` — not a patch:
...
`scripts/verify-0441-mod13-inventory-accounting-none-des.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0441-mod13-inventory-accounting-none_DESIGN
...
ROOT CAUSE: 0441-mod13-inventory-accounting-none_DESIGN still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
