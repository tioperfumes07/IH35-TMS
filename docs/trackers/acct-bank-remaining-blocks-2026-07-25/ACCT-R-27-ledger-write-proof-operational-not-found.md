...
**FINDING:** ACCT-R-27 / pile `ledger-write-proof-operational-not-found` (P2) · **Lane:** DOCS · **Module:** accounting.
...
OPEN: apps/backend/src/accounting/__proofs__/core-ledger-write-proof.spec.ts does not exist; docs/proofs/CORE-LEDGER-WRITE-PROOF-operational.md does not exist; no Neon-branch TB/BS/P&L tie-out proof for the 5 operational poste
...
Root-cause Fix for `ledger-write-proof-operational-not-found` — not a patch:
...
`scripts/verify-ledger-write-proof-operational-not-found.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: ledger-write-proof-operational-not-found
...
ROOT CAUSE: ledger-write-proof-operational-not-found still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
