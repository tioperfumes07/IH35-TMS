...
**FINDING:** ACCT-R-34 / pile `0519-at2-no-db-enforced-sod` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
Root-cause Fix for `0519-at2-no-db-enforced-sod` — not a patch:
...
`scripts/verify-0519-at2-no-db-enforced-sod.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0519-at2-no-db-enforced-sod
MIGRATE: N/A until owner unlock
ROOT CAUSE: 0519-at2-no-db-enforced-sod still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
