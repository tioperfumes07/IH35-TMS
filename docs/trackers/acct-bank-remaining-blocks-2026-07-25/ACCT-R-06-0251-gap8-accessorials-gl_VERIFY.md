...
**FINDING:** ACCT-R-06 / pile `0251-gap8-accessorials-gl_VERIFY` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: catalogs.charge_codes table with revenue_account_id FK to catalogs.accounts does not exist; accessorial codes have no GL mapping
...
1. Canonical target: `catalogs.charge_codes (TO ADD) → catalogs.accounts` — verify `to_regclass` on prod before write/FK.
...
Root-cause Fix for `0251-gap8-accessorials-gl_VERIFY` — not a patch:
...
`scripts/verify-0251-gap8-accessorials-gl-verify.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0251-gap8-accessorials-gl_VERIFY
...
ROOT CAUSE: 0251-gap8-accessorials-gl_VERIFY still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
