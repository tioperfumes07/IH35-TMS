# ACCT-R-12 — 0519-es1-58-unscoped-tables · No CI guard verifies that any of the 58 unscoped tables actually inherit scope via an enfo
**FINDING:** ACCT-R-12 / pile `0519-es1-58-unscoped-tables` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: No CI guard verifies that any of the 58 unscoped tables actually inherit scope via an enforced parent FK; accounting.bill_lines.bill_id (and accounting.expense_lines.expense_id per the same migration) have no FK constrai
...
Root-cause Fix for `0519-es1-58-unscoped-tables` — not a patch:
...
`scripts/verify-0519-es1-58-unscoped-tables.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0519-es1-58-unscoped-tables
...
ROOT CAUSE: 0519-es1-58-unscoped-tables still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
