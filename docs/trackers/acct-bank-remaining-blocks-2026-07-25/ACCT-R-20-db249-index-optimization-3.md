# ACCT-R-20 — db249-index-optimization-3 · 3 composite indexes as specified do not exist on any of the 3 tables; this is a real, unbu
**FINDING:** ACCT-R-20 / pile `db249-index-optimization-3` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: 3 composite indexes as specified do not exist on any of the 3 tables; this is a real, unbuilt DB migration (perf-only, non-financial-math but still touches accounting.invoices DDL so still financial-cluster per §1.4). [closure needs live proof]
...
Root-cause Fix for `db249-index-optimization-3` — not a patch:
...
`scripts/verify-db249-index-optimization-3.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: db249-index-optimization-3
...
ROOT CAUSE: db249-index-optimization-3 still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
