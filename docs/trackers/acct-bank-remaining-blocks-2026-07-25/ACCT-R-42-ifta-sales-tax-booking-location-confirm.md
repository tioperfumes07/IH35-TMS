...
**FINDING:** ACCT-R-42 / pile `ifta-sales-tax-booking-location-confirm` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
Root-cause Fix for `ifta-sales-tax-booking-location-confirm` — not a patch:
...
`scripts/verify-ifta-sales-tax-booking-location-confirm.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: ifta-sales-tax-booking-location-confirm
MIGRATE: N/A until owner unlock
ROOT CAUSE: ifta-sales-tax-booking-location-confirm still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
