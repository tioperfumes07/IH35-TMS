...
**FINDING:** ACCT-R-01 / pile `0007-pattern-5-split-brain-engines` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
Root-cause Fix for `0007-pattern-5-split-brain-engines` — not a patch:
...
`scripts/verify-0007-pattern-5-split-brain-engines.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0007-pattern-5-split-brain-engines
...
ROOT CAUSE: 0007-pattern-5-split-brain-engines still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
