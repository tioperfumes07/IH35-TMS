...
**FINDING:** ACCT-R-04 / pile `0243-g4-deploy-smoke-fixed-unit-test-owner` (P2) · **Lane:** NON-FINANCIAL · **Module:** accounting.
...
OPEN: IH35_SMOKE_UNIT_ID (and IH35_SMOKE_OPERATING_COMPANY_ID) are not set in render.yaml, so prod preDeploy still resolves a live unit by recency rather than a fixed synthetic id. Test-owner-email exclusion from prod identity [closure needs live proof]
...
Root-cause Fix for `0243-g4-deploy-smoke-fixed-unit-test-owner` — not a patch:
...
`scripts/verify-0243-g4-deploy-smoke-fixed-unit-test-own.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0243-g4-deploy-smoke-fixed-unit-test-owner
...
ROOT CAUSE: 0243-g4-deploy-smoke-fixed-unit-test-owner still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
