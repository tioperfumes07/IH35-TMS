...
**FINDING:** ACCT-R-19 / pile `db249-finance-schema-naming-drift` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: The finance.* vs accounting.* naming drift is real but scoped to 2 live/wired tables (loans, loan_amortization_rows), not 10. This is a rename/migration decision already correctly deferred to the tracked schema-canonical
...
Root-cause Fix for `db249-finance-schema-naming-drift` — not a patch:
...
`scripts/verify-db249-finance-schema-naming-drift.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: db249-finance-schema-naming-drift
...
ROOT CAUSE: db249-finance-schema-naming-drift still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
