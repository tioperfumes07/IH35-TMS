# ACCT-R-14 — audit4-tax-return-automation · 1) The sales-tax return module (prepare/file/mark-paid) is built but never registered into
**FINDING:** ACCT-R-14 / pile `audit4-tax-return-automation` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: 1) The sales-tax return module (prepare/file/mark-paid) is built but never registered into the app (apps/backend/src/index.ts has no import of accounting/sales-tax/sales-tax.routes.ts) — orphaned, needs wiring or an expl
...
Root-cause Fix for `audit4-tax-return-automation` — not a patch:
...
`scripts/verify-audit4-tax-return-automation.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: audit4-tax-return-automation
...
ROOT CAUSE: audit4-tax-return-automation still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
