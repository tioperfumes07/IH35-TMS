...
**FINDING:** BANK-R-03 / pile `phase13-audit216-banking-industry` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** banking.
**Pile source:** `docs/trackers/block-audit-piles-2026-07-21.json` post-purge 2026-07-25 · **pile:** `NEEDS-OWNER` · **status_reconcile:** `NEEDS-VERIFY`.
...
Root-cause Fix for `phase13-audit216-banking-industry` — not a patch:
...
`scripts/verify-phase13-audit216-banking-industry.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: phase13-audit216-banking-industry
MIGRATE: N/A until owner unlock
ROOT CAUSE: phase13-audit216-banking-industry still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
