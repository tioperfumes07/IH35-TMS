...
**FINDING:** ACCT-R-29 / pile `0251-gap2-vendor-gl-linkage` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
Root-cause Fix for `0251-gap2-vendor-gl-linkage` — not a patch:
...
`scripts/verify-0251-gap2-vendor-gl-linkage.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0251-gap2-vendor-gl-linkage
MIGRATE: N/A until owner unlock
ROOT CAUSE: 0251-gap2-vendor-gl-linkage still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
