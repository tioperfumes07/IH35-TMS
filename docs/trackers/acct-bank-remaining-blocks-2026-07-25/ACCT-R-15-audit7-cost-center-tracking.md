# ACCT-R-15 — audit7-cost-center-tracking · A true cost-center dimension distinct from the locked unit/driver Class field, plus varian
**FINDING:** ACCT-R-15 / pile `audit7-cost-center-tracking` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: A true cost-center dimension distinct from the locked unit/driver Class field, plus variance-analysis tooling on expense/GL rows.
...
Root-cause Fix for `audit7-cost-center-tracking` — not a patch:
...
`scripts/verify-audit7-cost-center-tracking.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: audit7-cost-center-tracking
...
ROOT CAUSE: audit7-cost-center-tracking still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
