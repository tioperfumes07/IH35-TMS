...
**FINDING:** BANK-R-02 / pile `CONN-3-relay-internal-bank` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** banking.
...
Root-cause Fix for `CONN-3-relay-internal-bank` — not a patch:
...
`scripts/verify-conn-3-relay-internal-bank.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: CONN-3-relay-internal-bank
MIGRATE: N/A until owner unlock
ROOT CAUSE: CONN-3-relay-internal-bank still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
