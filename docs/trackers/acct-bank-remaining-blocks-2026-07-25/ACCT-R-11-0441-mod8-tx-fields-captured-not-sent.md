...
**FINDING:** ACCT-R-11 / pile `0441-mod8-tx-fields-captured-not-sent` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: banking.bank_transactions has no check_number/class_name/location/is_billable/tags columns, and the categorize payload doesn't send the already-captured UI values even if it did [closure needs live proof]
...
1. Canonical target: `banking.bank_transactions` — verify `to_regclass` on prod before write/FK.
...
Root-cause Fix for `0441-mod8-tx-fields-captured-not-sent` — not a patch:
...
`scripts/verify-0441-mod8-tx-fields-captured-not-sent.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0441-mod8-tx-fields-captured-not-sent
...
ROOT CAUSE: 0441-mod8-tx-fields-captured-not-sent still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
