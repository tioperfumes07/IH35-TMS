# ACCT-R-21 — fact-par-1-submission-workflow · Only the manual_download channel is wired; no email-adapter or file-drop-adapter implement
**FINDING:** ACCT-R-21 / pile `fact-par-1-submission-workflow` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: Only the manual_download channel is wired; no email-adapter or file-drop-adapter implementation exists despite the doc requiring both as configurable channels. [closure needs live proof]
...
Root-cause Fix for `fact-par-1-submission-workflow` — not a patch:
...
`scripts/verify-fact-par-1-submission-workflow.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: fact-par-1-submission-workflow
...
ROOT CAUSE: fact-par-1-submission-workflow still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
