...
**FINDING:** ACCT-R-18 / pile `banking-b4-driver-vendor-account-mapping` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: A driver-keyed default-account mapping column/table equivalent to mdata.vendors.default_expense_account_id — confirmed absent.
...
1. Canonical target: `catalogs.accounts` — verify `to_regclass` on prod before write/FK.
...
Root-cause Fix for `banking-b4-driver-vendor-account-mapping` — not a patch:
...
`scripts/verify-banking-b4-driver-vendor-account-mapping.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: banking-b4-driver-vendor-account-mapping
...
ROOT CAUSE: banking-b4-driver-vendor-account-mapping still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
