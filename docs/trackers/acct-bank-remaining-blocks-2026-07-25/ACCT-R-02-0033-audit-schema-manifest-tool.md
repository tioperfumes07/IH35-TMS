...
**FINDING:** ACCT-R-02 / pile `0033-audit-schema-manifest-tool` (P2) · **Lane:** DOCS · **Module:** accounting.
...
OPEN: The literal live-Neon-pull manifest generator (scripts/audit-schema.mjs) and its output artifact (docs/schema/SCHEMA-MANIFEST.json) were never built; the substitute is migration-file-only and blind to prod drift.
...
Root-cause Fix for `0033-audit-schema-manifest-tool` — not a patch:
...
`scripts/verify-0033-audit-schema-manifest-tool.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0033-audit-schema-manifest-tool
...
ROOT CAUSE: 0033-audit-schema-manifest-tool still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
