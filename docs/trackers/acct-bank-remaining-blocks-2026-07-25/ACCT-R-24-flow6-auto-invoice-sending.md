# ACCT-R-24 — flow6-auto-invoice-sending · No automatic fire of the send/email path when an invoice leaves draft; no reminder cadence
**FINDING:** ACCT-R-24 / pile `flow6-auto-invoice-sending` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: No automatic fire of the send/email path when an invoice leaves draft; no reminder cadence for unpaid invoices found anywhere in apps/backend/src/accounting or cron.
...
1. Canonical target: `accounting.invoices` — verify `to_regclass` on prod before write/FK.
...
Root-cause Fix for `flow6-auto-invoice-sending` — not a patch:
...
`scripts/verify-flow6-auto-invoice-sending.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: flow6-auto-invoice-sending
...
ROOT CAUSE: flow6-auto-invoice-sending still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
