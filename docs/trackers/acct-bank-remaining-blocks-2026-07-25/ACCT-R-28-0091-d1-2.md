...
**FINDING:** ACCT-R-28 / pile `0091-d1-2` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
Root-cause Fix for `0091-d1-2` — not a patch:
...
`scripts/verify-0091-d1-2.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0091-d1-2
MIGRATE: N/A until owner unlock
ROOT CAUSE: 0091-d1-2 still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
