...
**FINDING:** ACCT-R-43 / pile `usmca-unhide-entity-switcher` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
Root-cause Fix for `usmca-unhide-entity-switcher` — not a patch:
...
`scripts/verify-usmca-unhide-entity-switcher.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: usmca-unhide-entity-switcher
MIGRATE: N/A until owner unlock
ROOT CAUSE: usmca-unhide-entity-switcher still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
