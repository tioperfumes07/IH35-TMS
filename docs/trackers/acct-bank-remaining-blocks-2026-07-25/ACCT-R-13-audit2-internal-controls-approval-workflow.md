...
**FINDING:** ACCT-R-13 / pile `audit2-internal-controls-approval-workflow` (P2) · **Lane:** NON-FINANCIAL · **Module:** accounting.
...
OPEN: settlements/approval.routes.ts (registerSettlementApprovalRoutes) is built but not registered/mounted anywhere — one of the three cited 'live' per-domain approval workflows is actually unwired; the broader unifying cross
...
Root-cause Fix for `audit2-internal-controls-approval-workflow` — not a patch:
...
`scripts/verify-audit2-internal-controls-approval-workfl.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: audit2-internal-controls-approval-workflow
...
ROOT CAUSE: audit2-internal-controls-approval-workflow still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
