# ACCT-R-09 — 0441-mod7-bill-subnav-filters-not-creators_UI · Maintenance/Repair/Fuel/Driver bill subnav links route to a filtered list, not a create fl
**FINDING:** ACCT-R-09 / pile `0441-mod7-bill-subnav-filters-not-creators_UI` (P2) · **Lane:** NON-FINANCIAL · **Module:** accounting.
...
OPEN: Maintenance/Repair/Fuel/Driver bill subnav links route to a filtered list, not a create flow, inconsistent with Vendor bill/Multiple bills in the same subnav group
...
Root-cause Fix for `0441-mod7-bill-subnav-filters-not-creators_UI` — not a patch:
...
`scripts/verify-0441-mod7-bill-subnav-filters-not-creato.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0441-mod7-bill-subnav-filters-not-creators_UI
...
ROOT CAUSE: 0441-mod7-bill-subnav-filters-not-creators_UI still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
