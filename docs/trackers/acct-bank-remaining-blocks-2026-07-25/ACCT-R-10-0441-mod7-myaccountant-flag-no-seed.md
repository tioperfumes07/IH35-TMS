# ACCT-R-10 — 0441-mod7-myaccountant-flag-no-seed · no seed migration for MY_ACCOUNTANT_ENABLED in db/migrations/ (consistent with other flags
**FINDING:** ACCT-R-10 / pile `0441-mod7-myaccountant-flag-no-seed` (P2) · **Lane:** NON-FINANCIAL · **Module:** accounting.
...
OPEN: no seed migration for MY_ACCOUNTANT_ENABLED in db/migrations/ (consistent with other flags' pattern); flag is discoverable only via an Owner typing the exact key [closure needs live proof]
...
Root-cause Fix for `0441-mod7-myaccountant-flag-no-seed` — not a patch:
...
`scripts/verify-0441-mod7-myaccountant-flag-no-seed.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0441-mod7-myaccountant-flag-no-seed
...
ROOT CAUSE: 0441-mod7-myaccountant-flag-no-seed still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
