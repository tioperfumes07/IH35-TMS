...
**FINDING:** ACCT-R-39 / pile `dip-mor-pre-post-petition-ap-split` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
Root-cause Fix for `dip-mor-pre-post-petition-ap-split` — not a patch:
...
`scripts/verify-dip-mor-pre-post-petition-ap-split.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: dip-mor-pre-post-petition-ap-split
MIGRATE: N/A until owner unlock
ROOT CAUSE: dip-mor-pre-post-petition-ap-split still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
