# BLOCK RECONCILIATION — 2026-07-02 (every block, built vs pending — verified)
...
## Universe — why 471 blocks (the "456 vs 294 .block-ready" gap, explained)
...
- **`.block-ready/*.json` files on disk:** 309
- **By source (after de-dup):** .block-ready: 298 · program: 61 · enterprise-29: 29 · accounting: 26 · gap-spec: 57
...
| RECON-01 | PENDING (GATED) |  | RECON-01 Schema + scheduled jobs + exception engine: additive CREATE TABLE accounting.recon_runs + accounting.recon_exce |
| RECON-02 | PENDING |  | RECON-02 UI tabs: extend the FIN-23 surface at /accounting/qbo-reconcile with Runs + Exceptions tabs (ParityTable gramma |
...
| RECON-02 | PENDING |  |  |  | .block-ready | 0/2 signature file(s) on main |
...
| RECON-01 | PENDING (GATED) | 💰 |  |  | .block-ready | 0/4 signature file(s) on main |
