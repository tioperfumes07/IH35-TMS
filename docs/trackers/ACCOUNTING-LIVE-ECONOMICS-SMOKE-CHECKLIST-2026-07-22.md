# Accounting live economics smoke checklist (19/22 → 2026-07-24 bill_lines)
...
- [ ] **QBO bill lines backfill (structural AP economics):** after merge of `fix/qbo-ap-bill-lines-projection`, with `QBO_AP_BILLS_PROJECTION_ENABLED` ON per entity, owner runs:
  ```bash
  ...
  ```
  Then Neon: `bill_lines` ≫ 1 (expect order ~10^5 AccountBased lines). Guard: `verify-qbo-ap-bill-lines-projection`.
...
| `~/Desktop/IH35-CURSOR-AUDIT/modules/banking.md` | Banking economics (transfers/recon) |
...
## Smoke D — Banking transfer + recon (closes banking economics zeros)

1. Banking → Transfer → one inter-account transfer → Neon `banking.transfers` ≥ 1  
2. Start one reconciliation session → Neon `banking.reconciliation_sessions` ≥ 1  

```sql
BEGIN;
SELECT set_config('app.bypass_rls', 'lucia', true);
SELECT count(*) AS transfers FROM banking.transfers;
SELECT count(*) AS recon_sessions FROM banking.reconciliation_sessions;
COMMIT;
```
...
GUARD: verify-acct-live-economics-evidence-pack.mjs / verify-qbo-ap-bill-lines-projection.mjs
...
- `scripts/verify-qbo-ap-bill-lines-projection.mjs` — QBO Line[] → bill_lines (step 1411)
