...
**FINDING:** ACCT-R-35 / pile `AF-1-entity-coa-fix` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
PENDING (GATED) — gated on owner decision/HELD gate — [verified 2026-07-03] held Tier-1 DO-NOT-RUN migration; prod still global — not applied
...
Root-cause Fix for `AF-1-entity-coa-fix` — not a patch:
...
`scripts/verify-af-1-entity-coa-fix.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: AF-1-entity-coa-fix
MIGRATE: N/A until owner unlock
ROOT CAUSE: AF-1-entity-coa-fix still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
