...
**FINDING:** ACCT-R-38 / pile `CHAIN-06-invoice-ar-chain-proof` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
PENDING (GATED) — gated on owner decision/HELD gate — [verified 2026-07-03] HOLD design doc only, no feature code
...
Root-cause Fix for `CHAIN-06-invoice-ar-chain-proof` — not a patch:
...
`scripts/verify-chain-06-invoice-ar-chain-proof.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: CHAIN-06-invoice-ar-chain-proof
MIGRATE: N/A until owner unlock
ROOT CAUSE: CHAIN-06-invoice-ar-chain-proof still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
