# ACCT-R-05 — 0251-gap11-commodity-gl · The product catalog (0251-gap10) that would key GL account selection off commodity does no
**FINDING:** ACCT-R-05 / pile `0251-gap11-commodity-gl` (P1, FIN-HOLD) · **Lane:** FINANCIAL-HOLD · **Module:** accounting.
...
OPEN: The product catalog (0251-gap10) that would key GL account selection off commodity does not exist, so revenue posting cannot vary by commodity/product -- this gap-11 item remains fully blocked on its stated prerequisite.
...
Root-cause Fix for `0251-gap11-commodity-gl` — not a patch:
...
`scripts/verify-0251-gap11-commodity-gl.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: 0251-gap11-commodity-gl
...
ROOT CAUSE: 0251-gap11-commodity-gl still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
