...
**FINDING:** BANK-R-04 / pile `sweepfix1727-8` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** banking.
**Pile source:** `docs/trackers/block-audit-piles-2026-07-21.json` post-purge 2026-07-25 · **pile:** `NEEDS-OWNER` · **status_reconcile:** `AUDIT-NOTE`.
...
Root-cause Fix for `sweepfix1727-8` — not a patch:
...
`scripts/verify-sweepfix1727-8.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: sweepfix1727-8
MIGRATE: N/A until owner unlock
ROOT CAUSE: sweepfix1727-8 still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
