...
**FINDING:** ACCT-R-40 / pile `factoring-asc860-cpa-control-test-open` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
backlog-verify NEEDS-OWNER (owner ruling required)
...
Root-cause Fix for `factoring-asc860-cpa-control-test-open` — not a patch:
...
`scripts/verify-factoring-asc860-cpa-control-test-open.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: factoring-asc860-cpa-control-test-open
MIGRATE: N/A until owner unlock
ROOT CAUSE: factoring-asc860-cpa-control-test-open still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
...
Bound by `.cursor/rules/00–24` + dual-lane-never-idle + COMPLIANCE-STANDARD-2026-07-25.md. Cursor builds; Claude merges; no Neon-apply by agent; no flag flips without owner sign-off; Rule 23 no money theater.
