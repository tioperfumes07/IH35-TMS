...
**FINDING:** ACCT-R-37 / pile `CHAIN-04-bill-payment-tieout` (P0, OWNER) · **Lane:** OWNER-GATE · **Module:** accounting.
...
Root-cause Fix for `CHAIN-04-bill-payment-tieout` — not a patch:
...
`scripts/verify-chain-04-bill-payment-tieout.mjs` + `scripts/verify-steps/NNN-verify-….mjs` (NEVER edit package.json / ci.yml / locked-guards.yml). FAIL on the pre-fix bug, PASS on the fix; `--selftest` mutates REAL source. One assertion per case.
...
ITEMS_TOUCHED: CHAIN-04-bill-payment-tieout
MIGRATE: N/A until owner unlock
ROOT CAUSE: CHAIN-04-bill-payment-tieout still open after 2026-07-25 Acct+Bank pile purge (not duplicate/wrong-module/BUILT).
