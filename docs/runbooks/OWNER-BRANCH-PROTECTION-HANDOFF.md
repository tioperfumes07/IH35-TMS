...
The verifier must print `LIVE PASS — N owner-approved contexts verified via GH_ADMIN_TOKEN` (N = length of `REQUIRED_GATE_CONTEXTS` in `scripts/verify-ci-policy-applied.mjs`, including `ui-design-system-ratchet / ui-design-system-ratchet` and `go26-consolidation-ratchet / go26-consolidation-ratchet`). A 403, 404,
