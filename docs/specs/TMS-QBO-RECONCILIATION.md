...
### HARD PRECONDITION — kill the residual JE auto-push before RECON is meaningful

RECON only tells the truth if the two books are **independent**. Today they are not: TMS still
auto-pushes journal entries into QBO on every JE create. `createJournalEntry`
(`apps/backend/src/accounting/journal-entries.service.ts:195` `enqueueSyncJob(... "journal_entry" ...)`
and `:209` `pushJournalEntryToQuickBooksImmediateBestEffort(...)`) fires **unconditionally** — there is
no feature flag, only an idempotency probe (`journal-entry-qbo-push.service.ts` has no `*_ENABLED` gate).
So a share of QBO's GL is a TMS **echo**, and reconciling against it would partly compare TMS to itself,
not to an independent book.

**Requirement:** before RECON-01's exception engine is trusted, this push must be behind a kill-switch
(`QBO_JE_PUSH_ENABLED`) that is **OFF**. Because it changes what TMS writes into the external book of
record, flipping it is a **§1.6 action** — the kill-switch ships behind a flag whose default *preserves
current behavior* (behavior-neutral merge), and Jorge flips it OFF when RECON goes live. Correspondingly,
