...
## 2026-06-03 · Block A17 · Drivers reference catalogs wire

Source: Block A17 spec (#89-Block-DRV-CAT)  
Status: LOCKED  
Relevant block: BLOCK-A17-DRV-CAT-DRIVERS-CATALOG-WIRE

Migration `0340` seeds five global `reference.*` driver lookup tables (license classes, CDL endorsements/restrictions, medical card status, employment status) with additive archive via `archived_at`. API namespace `GET|POST /api/v1/lists/drivers/<subcatalog>` + `PATCH /:id` archive toggle. UI at `/lists/drivers/*` (plural) with shared table page, search, archive filter, and **+ Create**. Pay catalogs unchanged on `/lists/driver/*`. CI guard `verify:drivers-catalogs-no-stub`.
