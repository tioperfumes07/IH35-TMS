...
**CREATE-TEST-THEN-VOID (owner 2026-08-22 — answered=closed; amended 2026-08-27 + 2026-08-28):** Canonical `docs/lockdown/CREATE-TEST-THEN-VOID-LAW-2026-08-22.md` + `docs/lockdown/TEST-LABEL-G1-AND-CUTOVER-FALSE-ALARM-LAW-2026-08-28.md`. Empty TMS tables are **expected**. **Create labeled TEST documents** through live wizards (save→canonical→reload) so Live Chrome / Fully-Wired item 2 can be proven. **$1,200 placeholders are sanctioned.** **All seats have permission** to create TEST / samples — do not wait for Jorge. **Seat-created prod financial fixtures forbidden** — `docs/lockdown/NO-SEAT-PROD-FINANCIAL-FIXTURES-LAW-2026-09-01.md` (owner-ordered walks only; create→prove→void same session). **Keep TEST on the books** (do not void-all-TEST; empty-table regression is forbidden). **Void only** to exercise void/reversal or when the document truly must be voided. **G1** = `is_sample_data` must actually be true on those rows (and on `factoring.batch`). **G2/9000** = detector, not fail-closed. Opening balances $0 / negative TEST bank = cutover design (`PARALLEL-BOOKS-CUTOVER-LOCKED-2026-07-16.md`). **Disabled** pay/+Create buttons: select the required picker row or create the TEST row — do not report “no transactions / cannot certify” without creating the hop that same session. No TMS→QBO write-back. **USMCA launch first**. **Urgent 14:** **14/14 CERTIFIED — CONCLUDED.** Never recertify.
...
## 11. URGENT 14 EXCLUSIVE MODULE CERTIFY (CONCLUDED — 14/14 CERTIFIED)

**Canonical:** `docs/lockdown/URGENT-14-EXCLUSIVE-MODULE-CERTIFY-LAW-2026-08-22.md`  
**Cursor rule:** `.cursor/rules/45-urgent-14-exclusive-module-certify.mdc` · `.cursor/rules/46-u14-closed-never-recertify.mdc`

U14 is **closed**. Do not recertify. One seat, one leftover NOW from POST-U14. HOLD forbidden. Unique FINDING only. Live SHA = `healthz/shallow`.
