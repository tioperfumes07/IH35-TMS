...
**CREATE-TEST-THEN-VOID (owner 2026-08-22 — answered=closed; amended 2026-08-27):** Canonical `docs/lockdown/CREATE-TEST-THEN-VOID-LAW-2026-08-22.md`. Empty TMS tables are **expected**. **Create labeled TEST documents** through live wizards (save→canonical→reload) so Live Chrome / Fully-Wired item 2 can be proven. **All seats have permission** to create TEST / samples (vendors, customers, loads, bills, expenses, catalog rows, whatever the wizard needs) — do not wait for Jorge. **Do not void until launch.** Voiding mid-sprint is double work; reuse those TEST rows on later hops. **One void-and-clean pass after launch.** **Disabled** pay/+Create buttons: select the required picker row or create the TEST row — do not report “no transactions / cannot certify” without creating the hop that same session. No TMS→QBO write-back. **USMCA launch first** (`docs/lockdown/USMCA-LAUNCH-FIRST-STANDING-LAW-2026-08-22.md`): Trucking / Transportation / QBO sync irrelevant; Banking TEST expense→match→reconcile is required. **Urgent 14 exclusive certify** (`docs/lockdown/URGENT-14-EXCLUSIVE-MODULE-CERTIFY-LAW-2026-08-22.md`): **14/14 CERTIFIED — CONCLUDED.** Never recertify. Leftover = POST-U14 sequence. HOLD forbidden.
