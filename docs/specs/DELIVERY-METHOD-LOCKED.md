...
14. **P0-BLOCKER OVERRIDE (owner 2026-08-05):** When **main is red for all lanes** (shared gate / shared guard floor — e.g. §7 palette baseline stale), the **first free lane** lands the **smallest fix immediately**. Cross-lane allowed. **No park, no ask.** That PR **does not count against WIP ≤ 3**. Resume normal WIP after it merges.
    - **P0-CLAIM (amendment 2026-08-05, root cause: 4 duplicate-fix collisions in one session — F117 ID clash, deduction-ack dup, cert-leak dup, and a second same-day orphaned-tracker-test P0):** the override stays no-ask and cross-lane, but the **first agent to start** the fix appends a `P0-CLAIM` row to `docs/audit/RESERVATION-LEDGER.md` **before writing any code**: `| <ISO-ts CST> | <agent> | P0-CLAIM | <files> | RESERVED | — |`. Any other agent who then sees an **active** `P0-CLAIM` covering the same files **STANDS DOWN** — do not build a competing fix; if you spot a gap in the claimant's approach, offer a diff/comment instead of a parallel PR. Release with a superseding `RELEASED` row (never edit in place) once the fix merges or is abandoned. This is the same optimistic-lock mechanism as Mechanism 1 in `RESERVATION-LEDGER-PROTOCOL-2026-08-05.md`, applied to the P0 path specifically — it keeps the override's speed (still no-ask, still cross-lane) while killing the duplicate-fix pattern.
    - **Mechanical lane ownership is unchanged:** CC-3 owns routine mechanical/FE work day-to-day; this override (and its P0-CLAIM gate) applies **only** to repo-wide blockers when the whole crew is frozen, not to normal mechanical assignment.  
