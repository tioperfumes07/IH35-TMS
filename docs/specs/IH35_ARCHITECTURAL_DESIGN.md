...
| **Plaid Connections** | Live bank feed config · `/banking/plaid-connections` (panel also on Accounts + Settings — additive) | Phase 6 ✅ |
...
| Compliance Docs & Monitoring | Geofence Alerts, Insurance, Permits, Integrity Reports, Position History |
...
Canonical inventory is **28 tabs / 9 groups**, exported from `SAFETY_TABS_CONFIG.ts` as `SAFETY_CANONICAL_TAB_COUNT`, `SAFETY_CANONICAL_GROUP_COUNT`, and `SAFETY_CANONICAL_TAB_KEYS`. Home quick-jump badge reads `SAFETY_CANONICAL_TAB_COUNT` (replaces stale hardcoded `6`). Safety module sidebar flyout links only under `/safety/*` (DOT Compliance → `/safety/dot-compliance`; global `/compliance` dashboard remains reachable from other modules). `foundation-kpis.routes.ts` KPI route allowlist matches all 28 tab keys.
...
**Deprecated (A17.2):** PR #403 `catalogs.driver_*` tables and `/api/v1/catalogs/driver/<subcatalog>` factory routes remain for ledger only. Tables receive `COMMENT ON TABLE` via `db/scripts/a17-2-deprecate-catalogs-driver-tables.sql` (idempotent, no migration). Factory responses emit `Deprecation`, `Link` (successor `/api/v1/lists/drivers/<subcatalog>`), and `Sunset: Wed, 03 Sep 2026`. Singular `/lists/driver/<subcatalog>` **reference** URLs Navigate-redirect to canonical `/lists/drivers/*` (pages under `lists/driver/deprecated-subcatalog-pages.tsx` are `@archived` — Rule 07). Pay/ops catalogs remain Live on singular `/lists/driver/{pay-rate-templates,teams,deduction-types,pay-types,escrow-types}`. `scripts/seed-driver-subcatalogs.mjs` is archived — do not run on new environments. **CI:** `verify:drivers-active-path`.
