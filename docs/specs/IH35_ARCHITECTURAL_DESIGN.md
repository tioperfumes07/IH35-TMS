...
**Accounting sub-nav — QBO mirror-scope "QBO ▾" menu (C7-ACCT-SUBNAV-CHROME fix, 2026-07-14):** the original C7 block (PR #865/282b9b41c, "Accounting sub-nav 12 QBO items + global topbar + Create/Tasks") shipped `QBO_ACCOUNTING_SUBNAV` (11 items — Bank transactions/Integration transactions/Receipts/Reconcile/Rules/Chart of accounts/Recurring transactions/Revenue recognition/Fixed assets/Prepaid expenses/My accountant, per `docs/specs/qbo-parity/QBO_PARITY_UI_SYSTEM.md` B5) rendered through a standalone left-rail component. Nav-unification (PR #1552) deleted that component for violating the top-bar-only rule (§7), and PR #1658 retired the now-dead array export — but the underlying routes (6 dedicated shell pages + `recurring-transactions` + 3 Banking-module routes) were never re-wired into the surviving `AccountingSubNavWrapper` chrome, leaving them reachable only by typing the URL. **Additive fix:** restored the 11 items as `ACCOUNTING_QBO_MIRROR_TABS` in `subnav-manifest.ts`, in the exact canonical QBO order, rendered as a new "QBO ▾" dropdown in `AccountingSubNavWrapper` positioned after the existing "More ▾" menu. `ACCOUNTING_CLEAN_TABS` and `ACCOUNTING_MORE_TABS` are untouched — no existing tab deleted or reordered (Chart of accounts intentionally appears in both menus, since its pre-existing `ACCOUNTING_MORE_TABS` position could not be moved). The "12 items" language in the historical tracker instruction does not resolve to a documented 12th item anywhere in `docs/specs/` or `docs/trackers/` — treated as a historical miscount, not evidence of a missing entry. Global toolbar (Topbar `+ Create` 6-item menu + `Tasks` button) was verified byte-identical to the original C7 delivery — no change needed there. **CI:** `verify:accounting-subnav-order`, `verify:accounting-nav`, `verify:accounting-route-map`, `verify:nav-integrity`.
