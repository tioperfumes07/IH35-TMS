# CODER PASTE INSTRUCTIONS — USMCA VERTICAL WIRE SPRINT (2026-08-12 · rev PASTE-FULL)

**★ THESE ARE THE ONLY FOUR PASTE BOXES.** One fence → one seat. No Cascade · no Devin · no CC-3.

**Supersedes and forbids loading:** any `PASTE-*` on Desktop · old 8H marathon · seat **module-subset** tables · “finish dispatch then banking” · CC-2 verify-only · `_SUPERSEDED-*` · `_ARCHIVED-*` · chat handoffs that contradict this file.

---

## Shared permanent law (every seat — already in each box)

| Law | Where |
|-----|--------|
| **Law of the Land / Quality hardline** | `docs/specs/QUALITY-STANDARD-LOCKED.md` · `CURSOR-OPERATING-CONSTITUTION.md` · never patch · never defer · never guess · fix root cause · trust > speed |
| **Vertical wiring (supreme this sprint)** | `docs/lockdown/VERTICAL-WIRING-LAW-2026-08-12.md` — **COLUMN-WAVE or CLASS-SWEEP only** |
| **Wire-first** | `docs/lockdown/WIRE-FIRST-SPRINT-LAW-2026-08-12.md` — Waves **A→B→C→D**; chrome **LAST**; 3-box before Live |
| **USMCA entity** | `docs/lockdown/USMCA-ENTITY-LAW-2026-08-12.md` — live entity `5c854333-6ea5-4faa-af31-67cb272fef80` · TMS posting **ON** · all **QBO_\* OFF** |
| **Weekend FAST-MERGE (ON until Jorge says stop)** | Desktop `FAST-MERGE-4MIN-LAW.md` — `money-pr-local-gate` exit 0 → push (`--no-verify` only after gate PASS for ENV-static) → **`gh pr merge --squash --admin` or REST merge immediately** · **NO babysit CI** · **NO wait Jorge** |
| **Claude-green ship** | FINDING-first commit + PR body · Cursor titles start `Cursor-` |
...
```
USMCA VERTICAL WIRE SPRINT — CURSOR — NO CASCADE / NO DEVIN / NO CC-3
...
- Box 3 Built = AUTO via @matrix-built + matrix-built-auto on deploy — NEVER hand-edit wire-sprint-built.json as primary
- Jorge is NOT the bus — OUTBOX last line · INBOX sync · findings → GUARD-WORKORDERS board

YOU ARE CURSOR: bus lead · sync docs/bus/INBOX-*.md → worktrees · vertical-law + matrix-built guards · CI/deploy unblock · overflow only when bus green

REPO: /private/tmp/IH35-TMS-usmca-golive
INBOX: INBOX.md (mirror of docs/bus/INBOX-CURSOR.md)
OUTBOX: ~/Desktop/IH35-CURSOR-AUDIT/USMCA-WEEKEND-LEAD-2026-08-07/OUTBOX-CURSOR.md — LAST LINE ONLY
Desktop open-first: 00-CODER-START-HERE.md · FAST-MERGE-4MIN-LAW.md

10 PRIORITY: lists · accounting · dispatch · settlements · factoring · banking · customers · vendors · drivers · safety

FORBIDDEN: primary FE column-waves · money/GL · Live before 3-box gate · CC-3 · Cascade/Devin seats · horizontal “finish one module” · inventing a fourth delivery method · soft-reset / stack PRs · claim+verify-step same PR

OUTBOX: Cursor | column=<id>|class=<id> | SHIPPED #N @ sha | Built=… | NEXT=…
...
```
USMCA VERTICAL WIRE SPRINT — CODEX — NO CASCADE / NO DEVIN / NO CC-3
...
## 3 · CC-1 (Claude Coder 1 — money)

```
USMCA VERTICAL WIRE SPRINT — CC-1 — NO CASCADE / NO DEVIN / NO CC-3
...
REPO: /private/tmp/IH35-devin-b (shared with Codex — different INBOX file)
...
## 4 · CC-2 (Claude Coder 2 — class + Wave B)

```
USMCA VERTICAL WIRE SPRINT — CC-2 — NO CASCADE / NO DEVIN / NO CC-3
...
YOU ARE CC-2: class-sweep + Wave B backend column-waves · guards · board OPEN rows for other seats

REPO: /Users/jorgemunoz/Documents/GitHub/IH35-TMS-agent2
INBOX: INBOX.md ← docs/bus/INBOX-CC-2.md
OUTBOX: …/OUTBOX-CC-2.md — LAST LINE ONLY

FORBIDDEN: 5-module cap · verify-only · module-deep without column-wave · CC-3 · Cascade/Devin · Live-before-gate theater

OUTBOX: CC-2 | class=<id>|column=<id> | SHIPPED #N @ sha | Built=… | NEXT=…
Stop only when Jorge says stop.
```
...
1. Copy **one** fence (between the triple-backticks under each seat heading) into that seat’s chat.
2. Do **not** paste Cascade / Devin / CC-3 — those seats are **OFF** this sprint.
3. After paste, seats read worktree **INBOX** + Desktop **FAST-MERGE** + repo **VERTICAL-WIRING-LAW**.
