...
- **CPA posture (loaded `ih35-cpa-accounting-decisions`):** reuse the existing poster, **write NO new GL math**, all money-posting flags stay **default OFF**, per-entity override only. An agent never posts/moves money; the flip is the owner's hand + CPA sign-off.
