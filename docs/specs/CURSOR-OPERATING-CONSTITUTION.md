...
- **`banking.bank_transactions.amount_cents` sign-vs-`is_credit` landmine (mandatory):** `amount_cents`'s sign runs OPPOSITE `is_credit` on this table — a credit/deposit (`is_credit=true`) is stored with a NEGATIVE `amount_cents` (Plaid convention), a debit/withdrawal (`is_credit=false`) with a POSITIVE one. Never infer money direction from `sign(amount_cents)`; always branch on `is_credit`. *Live-verified 2026-09-04 (BANK-F10005 amendment): 89 `is_credit=true` rows, all negative amount_cents, largest a WIRE IN; 254 `is_credit=false` rows, all positive amount_cents, largest a Zelle payment OUT — confirmed twice, owner independently reproduced the same split. `banking.routes.ts`'s deposits/withdrawals CASE happened to get this right by hardcoding the Plaid sign convention as a comment rather than reading `is_credit` — correct today by construction, but fragile: any future write path (manual entry, a different processor, a QBO import) that sets `is_credit` correctly without following this exact sign convention would silently swap deposits and withdrawals. Corrected to read `is_credit` directly. Guard: `verify-bank-transactions-direction-uses-is-credit.mjs` — no money-direction logic may branch on `sign(amount_cents)` for this table; it reads `is_credit`.*
