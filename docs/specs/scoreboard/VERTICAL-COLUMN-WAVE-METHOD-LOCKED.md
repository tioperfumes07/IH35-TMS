...
**TRIP COST / PER-TRUCK P&L (architecture — already law):** Blueprint + QBO parity: fuel matched to **load** → class=unit → **P&L per truck**; insurance pro-rated onto active loads; expenses carry unit#+load; one money story per load. **While on a trip**, the system Must be able to attach (and prefer auto-detect) fuel, expenses, repairs/WO, safety/accidents, fines, insurance burden — so monthly profit-and-loss **by truck** and **cost per trip** are computable (insurance, fines, fuel, maint, everything). Matrix Required on event leaves Must score that full hub; Built = wired live.
