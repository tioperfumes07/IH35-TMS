...
export async function registerItemsSyncRoutes(app: FastifyInstance) {
  app.post("/api/v1/qbo-sync/items/pull-now", { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (req, reply) => {
  ...
  app.post("/api/v1/qbo-sync/items/reconcile-now", { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (req, reply) => {
    const authUser = currentAuthUser(req, reply);
    if (!authUser) return reply;
    if (!isWriteRole(authUser.role)) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get("/api/v1/qbo-sync/items/status", { config: { rateLimit: { max: 60, timeWindow: "1 minute" } } }, async (req, reply) => {
    const authUser = currentAuthUser(req, reply);
    if (!authUser) return reply;
    ...
    try {
      ...
      app.log.error({ err: error }, "Items sync status fetch failed");
