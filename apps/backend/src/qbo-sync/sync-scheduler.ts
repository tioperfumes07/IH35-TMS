...
async function runScheduledSyncForCompany(
  ...
  // and idempotent (upsert / delete+insert lines), so re-running is safe. NO GL poster on this path.
  ...
  await runStep(
    ...
    async () => {
      ...
      log?.info(
        ...
        "[qbo-sync-scheduler] ap_bills_project complete (headers + lines from payload_json)"
      );
      return res;
    },
    failures,
