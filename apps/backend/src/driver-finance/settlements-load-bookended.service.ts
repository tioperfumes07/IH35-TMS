...
export async function aggregateSettlementTotals(
  ...
  const totalsRes = await client.query<{
    ...
    `
      SELECT
        COALESCE(SUM(CASE WHEN line_type IN ('earnings', 'extra_pay') THEN amount ELSE 0 END), 0) AS earnings,
...
export async function closeSettlementForFinalLoad(
  ...
  const driverId = load.assigned_primary_driver_id ?? load.assigned_secondary_driver_id ?? null;
  if (!driverId) return { closedSettlements: 0 };
  ...
  const busyRes = await client.query<{ cnt: number }>(
    ...
    `,
    [opts.operatingCompanyId, opts.loadId, driverId]
  );

  const busy = Number(busyRes.rows[0]?.cnt ?? 0);
  if (busy > 0) return { closedSettlements: 0 };
  ...
  const openRes = await client.query<{ id: string }>(
    `
      ...
      FOR UPDATE
    `,
    [opts.operatingCompanyId, driverId]
  );

  const settlementId = openRes.rows[0]?.id ? String(openRes.rows[0].id) : "";
  if (!settlementId) return { closedSettlements: 0 };
  ...
  await client.query(
    `
      ...
      WHERE id = $1
    `,
    [settlementId, closedAt, load.id, load.load_number]
  );
...
export async function pingSettlementOnLoadEvent(
  ...
  const loadRes = await client.query<{
    ...
    `
      SELECT assigned_primary_driver_id, assigned_secondary_driver_id
      FROM mdata.loads
      WHERE id = $1 AND operating_company_id = $2 AND soft_deleted_at IS NULL
      LIMIT 1
    `,
