...
export type DriversCatalogConfig = {
  urlSegment: string;
  tableName: string;
  displayName: string;
};

export const DRIVERS_CATALOG_CONFIGS: DriversCatalogConfig[] = [
...
export function assertCatalogConfig(config: DriversCatalogConfig) {
  if (!TABLE_GUARD.test(config.tableName)) {
    throw new Error(`invalid_table_name_for_drivers_catalog: ${config.tableName}`);
  }
  if (!SEGMENT_GUARD.test(config.urlSegment)) {
    throw new Error(`invalid_url_segment_for_drivers_catalog: ${config.urlSegment}`);
  }
}
