...
const patchBodySchema = z
  .object({
    ...
    event_overrides: z
      .record(z.string(), eventOverrideEntry.partial())
      .optional(),
...
function sanitizeOverrides(
  raw: Record<string, z.infer<typeof eventOverrideEntry>> | undefined,
...
export async function registerNotificationPreferenceRoutes(
  ...
  app.get(
    "/api/v1/identity/me/notification-preferences",
    ...
    async (req, reply) => {
      ...
      const row = await withCurrentUser(user.uuid, async (client) => {
        const reg = await client.query(
          `SELECT to_regclass('identity.user_notification_preferences') AS r`,
        );
    ...
    async (req, reply) => {
      ...
      if (body.reset_to_defaults) {
        await withCurrentUser(user.uuid, async (client) => {
          ...
          await client.query(
            `DELETE FROM identity.user_notification_preferences WHERE user_uuid = $1::uuid`,
            [user.uuid],
          );
        });
      ...
      const updated = await withCurrentUser(user.uuid, async (client) => {
        ...
        if (body.event_overrides !== undefined) {
          event_overrides = sanitizeOverrides(
            body.event_overrides as Record<
              string,
              z.infer<typeof eventOverrideEntry>
            >,
          );
        }
        ...
        if (body.quiet_hours_start !== undefined) {
          quiet_hours_start =
            body.quiet_hours_start === null
              ? null
              : coerceQuietTime(body.quiet_hours_start);
        }
        if (body.quiet_hours_end !== undefined) {
          quiet_hours_end =
            body.quiet_hours_end === null
              ? null
              : coerceQuietTime(body.quiet_hours_end);
        }
      ...
      if (!updated) {
        return reply
          .code(503)
          .send({ error: "notification_preferences_schema_missing" });
      }
