import { sendEmail } from "../../notifications/email.service.js";
import { createNotification } from "../../notifications/notification.service.js";
import type { OutboxEventHandler, OutboxHandlerContext, OutboxHandlerResult, OutboxPayload } from "./outbox-handler.types.js";

function requiredText(payload: OutboxPayload, key: string): string {
  const value = payload[key];
  if (typeof value !== "string" || !value.trim()) throw new Error(`safety_event_severe_notification_missing_${key}`);
  return value.trim();
}

function recipients(payload: OutboxPayload): string[] {
  const value = payload.recipient_user_ids;
  if (!Array.isArray(value) || value.some((id) => typeof id !== "string" || !id.trim())) {
    throw new Error("safety_event_severe_notification_invalid_recipients");
  }
  return value.map((id) => String(id).trim());
}

export class SafetyEventSevereNotificationHandler implements OutboxEventHandler {
  eventType = "safety.event.severe_notification" as const;
  requiresDelivery = true as const;
  canHandle() { return true; }

  async deliver(payload: OutboxPayload, ctx: OutboxHandlerContext): Promise<OutboxHandlerResult> {
    const companyId = requiredText(payload, "operating_company_id");
    const eventId = requiredText(payload, "aggregate_id");
    const title = requiredText(payload, "title");
    const body = requiredText(payload, "body");
    for (const userId of recipients(payload)) {
      const created = await createNotification({
        operating_company_id: companyId,
        user_id: userId,
        type: "driver_alert",
        severity: requiredText(payload, "severity") === "critical" ? "critical" : "high",
        title,
        body,
        action_link: `/safety/safety-events?event=${eventId}`,
        entity_type: "safety_event",
        entity_id: eventId,
        source_block: "0278-safety-gap3-auto-notifications",
      }, ctx.client);
      if (!created) throw new Error("safety_event_severe_notification_not_created");
    }
    await sendEmail({
      to: requiredText(payload, "to"),
      subject: title,
      html: requiredText(payload, "body_html"),
      sender: "noreply",
      eventClass: "safety.safety_events.severe_created",
    });
    return { message: "safety_event_severe_notification_sent" };
  }
}
