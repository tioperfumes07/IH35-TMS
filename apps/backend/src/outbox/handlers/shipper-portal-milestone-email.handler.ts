...
function requiredText(payload: OutboxPayload, key: string): string {
  ...
  if (typeof value !== "string" || !value.trim()) {
    throw new Error(`shipper_portal_milestone_email_missing_${key}`);
  }
  return value.trim();
}

export class ShipperPortalMilestoneEmailHandler implements OutboxEventHandler {
  eventType = "shipper_portal.milestone_email" as const;
  ...
  async deliver(payload: OutboxPayload, _ctx: OutboxHandlerContext): Promise<OutboxHandlerResult> {
    ...
    return { message: "shipper_portal_milestone_email_sent" };
  }
}
