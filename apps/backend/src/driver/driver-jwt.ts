...
export function verifyDriverAccessToken(token: string): DriverAccessClaims | null {
  try {
    const decoded = jwt.verify(token, jwtSecret(), { algorithms: ["HS256"] }) as DriverAccessClaims;
...
export function verifyDriverRefreshToken(token: string): DriverRefreshClaims | null {
  try {
    const decoded = jwt.verify(token, jwtSecret(), { algorithms: ["HS256"] }) as DriverRefreshClaims;
