...
const SETTLING_WINDOW_MINUTES = 10;
...
export async function processCargoSensorIncidents(client: DbClient, operatingCompanyId: string) {
...
export async function listCargoIncidents(client: DbClient, operatingCompanyId: string, loadId?: string) {
...
export async function resolveCargoIncident(client: DbClient, operatingCompanyId: string, incidentId: string, userId: string, note: string) {
  const result = await client.query<CargoIncidentRow>(
...
export async function fileCargoIncidentClaim(
  client: DbClient,
