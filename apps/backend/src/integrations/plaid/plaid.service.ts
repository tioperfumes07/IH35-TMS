...
import { scoreRuleMatch } from "../../banking/category-scoring.js";
...
// BANK-F02 specificity scorer moved to the dependency-free ../../banking/category-scoring.ts so the
// For-Review suggestion path (suggestion-engine.ts) can reuse the SAME canonical scorer without
// importing this side-effectful sync service. Imported above for autoCategorize; re-exported here so
// existing importers (and the bank-rule-specificity test) keep resolving it from plaid.service.
export { scoreRuleMatch };
