...
function getFrontendBaseUrl(): string {
  return (
    process.env.FRONTEND_BASE_URL || "https://app.ih35dispatch.com"
  ).replace(/\/$/, "");
}
...
export function templateKeyForMilestone(
  type: CustomerNotifyMilestone,
...
export function shouldDispatchNearArrival(
...
export function shouldDispatchDelayed(
  confidenceClass: string | null | undefined,
): boolean {
  const cls = (confidenceClass ?? "").toLowerCase();
...
export function buildSmsBody(
  ...
  if (milestone === "departed")
    return `IH35: Load ${loadNumber} has departed. ${etaNote}`;
  if (milestone === "arrived")
    return `IH35: Load ${loadNumber} has arrived at stop. ${etaNote}`;
  if (milestone === "near_arrival")
    return `IH35: Load ${loadNumber} is nearing arrival. ${etaNote}`;
...
async function fetchPreferences(
  ...
  operatingCompanyId: string,
  customerId: string,
): Promise<CustomerNotifyPreferences> {
...
async function customerBelongsToCompany(
  client: DbClient,
  operatingCompanyId: string,
  customerId: string,
) {
...
async function claimNotifyDelivery(
  ...
  // DSP-F7137: reserve the delivery identity BEFORE calling the external provider. The old
  // SELECT-then-send-then-INSERT sequence let two concurrent sync requests both send; only after
  // the duplicate escaped did the unique index reject the second log row and turn the request 500.
  // The partial unique index on pending/sent rows is the canonical cross-instance mutex.
  const res = await client.query<{ id: string }>(
    `
      INSERT INTO dispatch.notify_log (
        operating_company_id, load_id, customer_id, stop_id, milestone_type, channel,
        recipient, template_key, subject, status
      )
      VALUES ($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5, $6, $7, $8, $9, 'pending')
      ON CONFLICT (
        operating_company_id, load_id, milestone_type, channel,
...
async function finishNotifyDelivery(
  client: DbClient,
  input: {
    operatingCompanyId: string;
    logId: string;
  ...
  if (!res.rows[0]?.id) throw new Error("E_NOTIFY_DELIVERY_CLAIM_LOST");
}
...
async function fetchLoadNotifyContext(
  client: DbClient,
  loadId: string,
  operatingCompanyId: string,
): Promise<LoadNotifyContext | null> {
...
async function dispatchCustomerNotify(
  ...
  const routeLabel =
    [input.load.pickup_city, input.load.pickup_state]
      .filter(Boolean)
      .join(", ") +
    " → " +
    [input.load.delivery_city, input.load.delivery_state]
      .filter(Boolean)
      .join(", ");
  ...
  if (input.prefs.notify_email) {
    ...
    if (!email) {
      ...
      const logId = await claimNotifyDelivery(client, {
        operatingCompanyId,
        loadId: input.load.load_id,
        customerId: input.load.customer_id,
      ...
      if (!logId) {
        ...
        if (sms.success) {
          ...
          await finishNotifyDelivery(client, {
            ...
            logId,
            status: "failed",
            errorMessage: sms.error ?? "sms_failed",
          });
        }
      }
    }
...
export async function processStopArrivalNotifications(
  ...
  for (const row of arrivals.rows) {
    const load = await fetchLoadNotifyContext(
      client,
      row.load_id,
      operatingCompanyId,
    );
    ...
    const result = await dispatchCustomerNotify(
      client,
      userId,
      operatingCompanyId,
      {
...
export async function processEtaUpdateNotifications(
  ...
  for (const row of loads.rows) {
    ...
    const etaLabel = row.predicted_at
      ? new Date(row.predicted_at).toLocaleString()
      : "Check portal for live ETA.";
...
export async function syncCustomerNotifyFromEvents(
  userId: string,
  operatingCompanyId: string,
) {
  return withCompany(userId, operatingCompanyId, async (client) => {
    const arrivals = await processStopArrivalNotifications(
      client,
      userId,
      operatingCompanyId,
    );
    const eta = await processEtaUpdateNotifications(
      client,
      userId,
      operatingCompanyId,
    );
...
export async function getCustomerNotifyPreferences(
  userId: string,
  operatingCompanyId: string,
  customerId: string,
) {
  return withCompany(userId, operatingCompanyId, async (client) => {
    if (
      !(await customerBelongsToCompany(client, operatingCompanyId, customerId))
    ) {
