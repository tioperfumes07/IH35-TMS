...
export async function confirmPresettlementLink(client: DbClient, input: ConfirmInput) {
  ...
  if (input.action === "create_new") {
    ...
    // R-186.1 (owner 2026-09-25): open pre-settlements carry OUR P-series on display_id
    // (allocateNextSettlementDisplayId → allocateSettlementDisplayId). Do NOT call
    // allocateNextSettlementSourceDocumentRef here — that minted fake AlwaysTrack numbers
    // (5817/5818/5819). source_document_ref stays NULL until the owner types a real AT number
    // on the editable Creator / Pre-Settlement header.
  } else {
    // link_existing
