...
export type StopsGeocodeBackfillResult = {
  ...
  geofences_created: number; failures: StopGeocodeFailure[];
};
...
export async function geocodeStopsWithClient(client: DbClient, actorId: string, companyId: string, loadId?: string): Promise<StopsGeocodeBackfillResult> {
  ...
  for (const stop of stops) {
    ...
    const updated = await client.query<{ id: string }>(`
      UPDATE mdata.load_stops SET location_id=$2::uuid, latitude=$3, longitude=$4,
        geocode_source=$5, geocode_confidence=$6, geocode_attempted_at=now(), geocode_failure_reason=NULL, updated_at=now()
      WHERE id=$1::uuid RETURNING id::text`, [stop.stop_id, locationId, outcome.latitude, outcome.longitude, outcome.source, outcome.confidence]);
    if (!updated.rows[0]) continue;
    geocoded += 1; locations += 1;
    const inserted = await client.query<{ id: string }>(`
  ...
  return { stops_checked: stops.length, stops_geocoded: geocoded, locations_linked: locations, geofences_created: fences, failures };
}
