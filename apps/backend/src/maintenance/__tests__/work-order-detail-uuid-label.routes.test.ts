 ...
 * external_vendor_id, roadside_breakdown_load_id) had no accompanying display name, so
...
describe("GET /api/v1/maintenance/work-orders/:id — driver/vendor/roadside-load name joins", () => {
  it("selects the detail row with unit_number, driver_name, external_vendor_name, roadside_breakdown_load_number", () => {
    expect(routes).toMatch(/SELECT w\.\*, u\.unit_number,/);
    expect(routes).toMatch(
      /NULLIF\(TRIM\(COALESCE\(d\.first_name, ''\) \|\| ' ' \|\| COALESCE\(d\.last_name, ''\)\), ''\) AS driver_name/
    );
    expect(routes).toMatch(/v\.vendor_name AS external_vendor_name/);
  ...
  it("joins mdata.drivers, mdata.vendors, and mdata.loads scoped to the work order's operating company", () => {
    ...
    expect(routes).toMatch(/LEFT JOIN mdata\.vendors v ON v\.id = w\.external_vendor_id AND v\.operating_company_id = w\.operating_company_id/);
  ...
  it("still scopes the detail row itself to the requesting operating company", () => {
    expect(routes).toMatch(/WHERE w\.id = \$1 AND w\.operating_company_id = \$2 LIMIT 1/);
  });
});
