...
function client(context: { unit_id: string | null; vendor_id: string | null; cost_cents: number }, linked: Array<{ kind: "bill" | "expense"; id: string }> = []) {
...
describe("FLT-06 work-order close financial linkage", () => {
  it("permits a zero-cost work order without inventing a financial document", async () => {
    const db = client({ unit_id: null, vendor_id: null, cost_cents: 0 });
  ...
  it.each([
    [{ unit_id: null, vendor_id: "vendor", cost_cents: 100 }, "unit_required"],
    [{ unit_id: "unit", vendor_id: null, cost_cents: 100 }, "vendor_required"],
  ...
  it("rejects a positive-cost work order with no linked bill or expense", async () => {
    const db = client({ unit_id: "unit", vendor_id: "vendor", cost_cents: 25000 });
    await expect(assertWorkOrderCostFinancialLink(db, "company", "wo")).resolves.toEqual({
      ok: false,
      cost_cents: 25000,
  ...
  it("accepts a financial document linked to the exact work order, company, unit and vendor", async () => {
    const db = client(
      { unit_id: "unit", vendor_id: "vendor", cost_cents: 25000 },
      [{ kind: "bill", id: "bill" }]
    );
    await expect(assertWorkOrderCostFinancialLink(db, "company", "wo")).resolves.toEqual({
      ok: true,
