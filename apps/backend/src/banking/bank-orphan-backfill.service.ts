...
async function findOrphans(client: DbClient, operatingCompanyId: string): Promise<BankOrphanRow[]> {
  const r = await client.query<{
    ...
    `
      ...
      -- first time the live route actually ran ("column bp DOT voided_at does not exist"), exactly
      -- the class of bug the owner's void-column-convention finding warns about (4 parallel
      -- names: voided_at / revoked_at / unapplied_at / reversed_at).
