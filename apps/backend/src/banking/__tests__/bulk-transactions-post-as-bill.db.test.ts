...
describeIntegration("BANKING-GL-COMPLETION post-as-bill paid-in-full end-to-end (real Postgres)", () => {
  ...
  beforeAll(async () => {
    ...
    await bypass(async () => {
      ...
      // The bill/bill-payment posters resolve ap_control from accounting.chart_of_accounts_roles
      // (bank-transaction-splits.service.ts:809). This suite runs on the SHARED company from
      // ensureIntegrationPrerequisites(), and sibling suites DELETE that company's role rows in
      // teardown (test-helpers/isolated-company.ts:184) -- so depending on ambient state made this
      // suite order-dependent and flaky ("expected false to be true" on bill_posted). Guarantee the
      // mapping ourselves. ADDITIVE ONLY: if a mapping already exists we leave it untouched, so we
      // never stomp a sibling suite's designation.
      await db.query(
        `INSERT INTO catalogs.accounts (id, operating_company_id, account_number, account_name, account_type, is_postable)
