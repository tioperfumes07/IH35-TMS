...
describe("transaction-aware canonical bill-payment void", () => {
  ...
  it("reverses GL and restores payment, bill paid/status, and bank balance on one client", async () => {
    ...
    const result = await voidBillPaymentInClientTx(client, {
      ...
      paymentId: PAYMENT,
      reason: "settlement cancellation",
      userId: USER,
    ...
    expect(
      calls.some(
        (call) =>
          call.sql.includes("UPDATE accounting.bill_payments") &&
          call.sql.includes("revoked_at = now()"),
      ),
    ).toBe(true);
    expect(
      calls.some(
        (call) =>
          call.sql.includes("UPDATE accounting.bills") &&
          call.values?.[1] === 2_500 &&
          call.values?.[3] === "partially_paid",
      ),
    ).toBe(true);
    expect(
      calls.some(
        (call) =>
          call.sql.includes("UPDATE banking.bank_accounts") &&
          call.values?.[2] === 7_500,
      ),
    ).toBe(true);
  });

  it("rolls no subledger mutation after a GL reversal failure", async () => {
    const { client, calls } = paymentClient();
    reverseSource.mockRejectedValue(new Error("PERIOD_LOCKED"));
    const { voidBillPaymentInClientTx } = await import("../bills.service.js");

    await expect(
      voidBillPaymentInClientTx(client, {
        operatingCompanyId: COMPANY,
        paymentId: PAYMENT,
        reason: "settlement cancellation",
        userId: USER,
    ...
    expect(
      calls.some((call) => call.sql.includes("UPDATE accounting.bills")),
    ).toBe(false);
    expect(
      calls.some((call) => call.sql.includes("UPDATE banking.bank_accounts")),
    ).toBe(false);
  });
});
