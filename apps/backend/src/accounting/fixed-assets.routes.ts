...
/**
 * Compute a monthly depreciation schedule for display (no persistence, no posting).
 ...
 * - units_of_production: requires per-period usage that this read-only module does not track,
 *   so the schedule is returned empty (the caller surfaces an honest note).
 */
function computeDepreciationSchedule(a: AssetForCompute): { rows: ScheduleRow[]; note: string | null } {
...
function asOfToday(rows: ScheduleRow[]): { depr_to_date_cents: number; book_value_now_cents: number } {
