...
import {
  ...
  type SettlementBillPaymentResult,
...
function encryptTestToken(plain: string): string {
  ...
  const key = crypto
    .createHash("sha256")
    .update(process.env.ENCRYPTION_KEY!, "utf8")
    .digest();
...
describeIntegration(
  "SETTLEMENT-BILL-PAYMENT GL posting (real Postgres)",
  () => {
    let db: pg.Client;
    ...
    async function resolveReversalJournalEntryIds(
      originalJeIds: string[],
    ): Promise<string[]> {
    ...
    beforeAll(async () => {
      ...
      await bypass(async () => {
        ...
        await mkAcct(
          acct.advanceSub,
          "Driver Cash Advance- Mecor Perez",
          "Asset",
          null,
          acct.advanceParent,
        );
        await mkAcct(
          acct.escrowSub,
          "Mecor Perez — Driver Escrow (hired unknown)",
          "Liability",
          null,
          acct.escrowParent,
        );
        ...
        await upsertDriverEscrowAccountLink(db, {
          operatingCompanyId: companyId,
          driverId,
          coaAccountId: acct.escrowSub,
        });
        await upsertDriverAdvanceAccountLink(db, {
          operatingCompanyId: companyId,
          driverId,
          coaAccountId: acct.advanceSub,
          actorUserId: userId,
        });
        ...
        await db.query(
         ...
         ON CONFLICT (operating_company_id, realm_id) WHERE revoked_at IS NULL DO NOTHING`,
          [
            companyId,
            `TEST-REALM-${suffix}`,
            encryptTestToken("test-access-token"),
            encryptTestToken("test-refresh-token"),
            userId,
          ],
        );
      });
    ...
    afterAll(async () => {
      ...
      try {
        ...
        const originalJeIds = settlementId
          ? await resolveOwnJournalEntryIds()
          : [];
        const reversalJeIds = settlementId
          ? await resolveReversalJournalEntryIds(originalJeIds)
          : [];
        await db.query("SELECT pg_advisory_lock(4200000001)");
        await bypass(async () => {
          await db.query(
            `DELETE FROM driver_finance.driver_settlement_gl_bills WHERE settlement_id = $1::uuid`,
            [settlementId],
          );
          await db.query(
            `DELETE FROM driver_finance.driver_settlement_gl_runs WHERE settlement_id = $1::uuid`,
            [settlementId],
          );
          ...
          await db.query(
            `DELETE FROM accounting.transaction_source_links WHERE operating_company_id=$1::uuid AND linked_object_type IN ('bill','bill_payment','journal_entry')`,
            [companyId],
          );
          await db.query(
            `DELETE FROM accounting.journal_entry_postings WHERE operating_company_id=$1::uuid AND source_transaction_type IN ('bill','bill_payment')`,
            [companyId],
          );
          await db.query(
            `DELETE FROM accounting.posting_batches WHERE operating_company_id=$1::uuid`,
            [companyId],
          );
          await db.query(
            `DELETE FROM accounting.bill_payments WHERE operating_company_id=$1::uuid AND bill_id = ANY($2::uuid[])`,
            [companyId, glBillIds],
          );
          await db.query(
            `DELETE FROM accounting.bill_lines WHERE bill_id = ANY($1::uuid[])`,
            [glBillIds],
          );
          // the deduction JE has no source_transaction_type='bill*'; delete via memo match
          await db.query(
            `DELETE FROM accounting.journal_entry_postings jep USING accounting.journal_entries je WHERE je.id=jep.journal_entry_uuid AND je.operating_company_id=$1::uuid AND je.memo LIKE $2`,
            [companyId, `Settlement S-${suffix}%`],
          );
          await db.query(
          ...
          await db.query(
            `DELETE FROM accounting.journal_entries WHERE id = ANY($1::uuid[])`,
            [reversalJeIds],
          );
          await db.query(
            `DELETE FROM accounting.journal_entries WHERE operating_company_id=$1::uuid AND (memo LIKE $2 OR memo LIKE $3)`,
            [companyId, `%S-${suffix}%`, `%Bill %`],
          );
          await db.query(
            `DELETE FROM accounting.bills WHERE operating_company_id=$1::uuid AND id = ANY($2::uuid[])`,
            [companyId, glBillIds],
          );
          await db.query(
            `DELETE FROM driver_finance.driver_settlement_deductions WHERE applied_to_settlement_id = $1::uuid`,
            [settlementId],
          );
          await db.query(
            `DELETE FROM driver_finance.driver_bills WHERE operating_company_id=$1::uuid AND driver_id=$2::uuid`,
            [companyId, driverId],
          );
          await db.query(
            `DELETE FROM driver_finance.driver_settlements WHERE id = $1::uuid`,
            [settlementId],
          );
          ...
          await db.query(`DELETE FROM mdata.loads WHERE id = ANY($1::uuid[])`, [
            loadIds,
          ]);
          await db.query(
            `DELETE FROM banking.bank_accounts WHERE id = $1::uuid`,
            [dipBankId],
          );
          await db.query(
            `DELETE FROM accounting.chart_of_accounts_roles WHERE operating_company_id=$1::uuid AND role='ap_control'`,
            [companyId],
          );
          ...
          await db.query(
            `DELETE FROM catalogs.account_role_bindings WHERE role_key = ANY($1)`,
            [["driver_pay_expense", "cash_dip"]],
          );
          await db.query(
          ...
          await db.query(`DELETE FROM mdata.drivers WHERE id = $1::uuid`, [
            driverId,
          ]);
          await db.query(`DELETE FROM mdata.customers WHERE id = $1::uuid`, [
            customerId,
          ]);
          await db.query(
            `DELETE FROM lib.feature_flag_overrides WHERE flag_key='SETTLEMENT_GL_POSTING_ENABLED' AND operating_company_id=$1::uuid`,
            [companyId],
          );
          await db.query(
            `DELETE FROM org.user_company_access WHERE user_id=$1::uuid AND company_id=$2::uuid`,
            [userId, companyId],
          );
          await db.query(
            `DELETE FROM integrations.qbo_sync_queue WHERE operating_company_id=$1::uuid AND entity_id = ANY($2::uuid[])`,
            [companyId, glBillIds],
          );
          await db.query(
            `DELETE FROM integrations.qbo_connections WHERE operating_company_id=$1::uuid AND realm_id=$2`,
            [companyId, `TEST-REALM-${suffix}`],
          );
        });
      } catch {
        /* best-effort */
    ...
    it("(a) flag OFF -> no-op, no run / bills / journal entries", async () => {
      ...
      const result = (await postSettlementBillPayment(
        ...
        { userId },
      )) as SettlementBillPaymentResult;
    ...
    it("(b) blueprint worked example: 3 per-load bills, driver-own sub-accounts, net to DIP, A/P nets to 0", async () => {
      ...
      const result = (await (async () => {
        ...
        try {
          ...
          return await postSettlementBillPayment(
            { operatingCompanyId: companyId, settlementId },
            { userId },
          );
        } finally {
      ...
      expect(new Set(bills.map((b) => b.bill_number))).toEqual(
        new Set(loadNumbers),
      );
      ...
      const apDebit = dedLines.find(
        (l) => l.account_id === acct.ap && l.debit_or_credit === "debit",
      );
      const advCredit = dedLines.find(
        (l) =>
          l.account_id === acct.advanceSub && l.debit_or_credit === "credit",
      );
      const escCredit = dedLines.find(
        (l) =>
          l.account_id === acct.escrowSub && l.debit_or_credit === "credit",
      );
    ...
    it("(c) idempotent -> second post is already_posted, no duplicate bills", async () => {
      ...
      const glBillIds = await resolveGlBillIds();
      const billCount = await read<{ c: string }>(
        `SELECT count(*)::text AS c FROM accounting.bills WHERE id = ANY($1::uuid[])`,
        [glBillIds],
      );
    ...
    it("(d) CPA veto: concurrent/retried reversal is atomic, idempotent, linked, and whole-settlement net-zero", async () => {
        ...
        reverseSettlementBillPayment(
          {
            operatingCompanyId: companyId,
            settlementId,
            reason: "test concurrent reversal A",
          },
          { userId },
        ),
        reverseSettlementBillPayment(
          {
            operatingCompanyId: companyId,
            settlementId,
            reason: "test concurrent reversal B",
          },
          { userId },
        ),
      ]);
      expect([revA.result, revB.result].sort()).toEqual([
        "nothing_to_reverse",
        "reversed",
      ]);
      ...
      const glBillIds = await resolveGlBillIds();
      const restoredPayments = await read<{
        total: string;
        active: string;
      }>(
      ...
      expect(Number(restoredPayments[0]!.total)).toBe(6); // 3 cash + 3 deduction/non-cash
      expect(Number(restoredPayments[0]!.active)).toBe(0);

      const restoredBills = await read<{
        total: string;
        nonvoid: string;
        paid_cents: string;
      }>(
        `SELECT COUNT(*)::text AS total,
              COUNT(*) FILTER (WHERE revoked_at IS NULL)::text AS nonvoid,
      ...
      const restoredDriverBills = await read<{
        status: string;
      ...
      expect(
        restoredDriverBills.every(
          (row) =>
            row.status === "open" && row.settled_in_settlement_id === null,
        ),
      ).toBe(true);

      const bank = await read<{ current_balance_cents: string }>(
        `SELECT current_balance_cents::text
      ...
      expect(
        dates.every((row) => row.reversal_date === row.original_date),
      ).toBe(true);
      ...
      const residual = await read<{
        nonzero_dimensions: string;
        absolute_residual_cents: string;
      }>(
        `WITH dimensional AS (
      ...
      const retry = await reverseSettlementBillPayment(
        {
          operatingCompanyId: companyId,
          settlementId,
          reason: "retry after commit",
        },
        { userId },
      );
      expect(retry.result).toBe("nothing_to_reverse");
      expect(await resolveReversalJournalEntryIds(originalJeIds)).toHaveLength(
        7,
      );
    });
  },
