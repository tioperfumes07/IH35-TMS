...
vi.mock("../../../auth/db.js", () => ({
  withCurrentUser: mocks.withCurrentUser,
}));
...
vi.mock("../../../audit/crud-audit.js", () => ({
  appendCrudAudit: mocks.appendAudit,
}));
  ...
  const client = {
    query: vi.fn(async (sql: string, values?: unknown[]) => {
          ...
          rows: [
            {
              id: RUN,
              status: "posted",
              deduction_journal_entry_id: DEDUCTION_JE,
            },
          ],
        };
      }
      if (
        sql.includes("SELECT driver_bill_id::text, accounting_bill_id::text")
      ) {
        return {
          rows: [
            {
      ...
      if (sql.includes("UPDATE driver_finance.driver_bills")) {
        return { rows: [{ id: DRIVER_BILL }], rowCount: 1 };
      }
...
describe("settlement Bill+BillPayment reversal orchestration", () => {
  beforeEach(() => {
    ...
    mocks.reverseSource.mockImplementation(async (_client, input) => ({
      result: "reversed",
      journal_entry_id:
        input.source_transaction_type === "bill" ? BILL_REV : PAYMENT_REV,
    }));
    ...
    mocks.voidBill.mockResolvedValue({
      ok: true,
      reversal_journal_entry_id: BILL_REV,
    });
  ...
  it("fails loud on any source-leg reversal error and never marks the run reversed", async () => {
    ...
    const periodLocked = Object.assign(new Error("IH35_CLOSED_PERIOD"), {
      code: "PERIOD_LOCKED",
    });
    mocks.voidPayment.mockRejectedValueOnce(periodLocked);

    const { reverseSettlementBillPayment } =
      await import("../settlement-bill-payment-posting.service.js");
    await expect(
    ...
    expect(calls.some((c) => c.sql.includes("SET status = 'reversed'"))).toBe(
      false,
    );
  ...
  it("uses one transaction client for every canonical reversal, proves full net-zero, then transitions state", async () => {
    ...
    expect(
      mocks.voidPayment.mock.calls.every((call) => call[0] === client),
    ).toBe(true);
    ...
    const proofIndex = calls.findIndex((c) =>
      c.sql.includes("WITH selected AS"),
    );
    const statusIndex = calls.findIndex((c) =>
      c.sql.includes("SET status = 'reversed'"),
    );
