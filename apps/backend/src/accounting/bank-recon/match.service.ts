...
export async function acceptMatchWithResolveDifference(input: ResolveDifferenceInput): Promise<ResolveDifferenceResult> {
  return withLuciaBypass(async (client) => {
    ...
    if (input.ledger_entry_kind === "payment") {
      ...
      // ACCT-F5620 (re-applied — a concurrent squash-merge from another seat silently reverted this
      // exact diff after it first merged as PR #10787/11fe5d73a; see docs/bus/OUTBOX-CC-1.md for the
      // landmine record) — the payment→invoice back-link (backlinkBankTransactionToInvoice,
