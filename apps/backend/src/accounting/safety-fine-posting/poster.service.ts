...
//   * Idempotency: safety.civil_fines.expense_je_id is the latch, backed by a deterministic-memo check;
//     a re-run finds the existing JE and no-ops.
...
export async function postCompanyPaidCivilFine(input: PostCompanyPaidFineInput): Promise<SafetyFinePostResult> {
  const prepared = await withLuciaBypass(async (client: DbClient) => {
    ...
    // Idempotency latch — already posted.
    if (fine.expense_je_id) return { gate: "already_posted" as const };
  ...
  // Total-Connectivity: fine -> JE (and JE -> fine via ix_civil_fines_expense_je). The WHERE pins
  // expense_je_id IS NULL so a concurrent double-post can never overwrite the first JE id.
  await withLuciaBypass(async (client: DbClient) => {
    await client.query(`SELECT set_config('app.operating_company_id', $1::text, true)`, [input.operating_company_id]);
    await client.query(
      `
        UPDATE safety.civil_fines
        SET expense_je_id = $3::uuid,
            expense_posted_at = now(),
            expense_posted_by_user_id = $4::uuid,
            updated_at = now()
        WHERE operating_company_id = $1::uuid
          AND id = $2::uuid
          AND expense_je_id IS NULL
      `,
      [input.operating_company_id, input.fine_id, created.id, input.actor_user_id]
    );
