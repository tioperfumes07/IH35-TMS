...
describe("F1 break-even — static read-only invariant", () => {
  ...
  // ── FLAG-SPLIT-BRAIN guard: the backend must gate on the SAME DB feature flag the frontend reads
  //    (via isEnabled), NOT process.env — so the two sides can never split-brain. ──
  it("gates on the DB feature flag via isEnabled (no process.env read for the flag)", () => {
    const src = fs.readFileSync(path.resolve(__dirname, "break-even.routes.ts"), "utf8");
    expect(src).toMatch(/isEnabled\s*\(/);
    expect(src).toMatch(/FINANCE_BREAK_EVEN_UI_ENABLED/);
    expect(
      /process\.env\.FINANCE_BREAK_EVEN_UI_ENABLED/.test(src),
      "break-even route must not read the flag from process.env — resolve it via isEnabled(DB flag)",
    ).toBe(false);
  });
});
...
// The gate now resolves the DB feature flag via isEnabled (same source the frontend reads), so we drive
// it with a mocked isEnabled instead of process.env — proving backend+frontend read one flag.
const { inputsMock, membershipMock, isEnabledMock } = vi.hoisted(() => ({
  inputsMock: vi.fn(),
