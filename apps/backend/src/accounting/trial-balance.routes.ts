...
export async function registerTrialBalanceRoutes(app: FastifyInstance) {
  app.get("/api/v1/accounting/trial-balance", { config: { rateLimit: { max: 60, timeWindow: "1 minute" } } }, async (req, reply) => {
    ...
    // withCompanyScope, but the accrual (default) branch calls getTrialBalanceReport directly,
    // bypassing withCompanyScope entirely — a real cross-tenant trial-balance leak.
