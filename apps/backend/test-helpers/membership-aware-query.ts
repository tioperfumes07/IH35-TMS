 ...
 * gap, not a product defect, and it appeared in eleven suites at once when the CLS-GUC sweep landed.
 *
 * THE FIX MUST NOT BE `vi.mock("../_helpers/company-membership-guard.js")`. Stubbing the guard away
 * deletes the control from the test, so every one of those suites would keep passing if someone removed
 * the real gate — the vacuous-control shape this codebase keeps getting bitten by. Here the REAL assert
 * still executes; it just receives a truthful answer.
 *
