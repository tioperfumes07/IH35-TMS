...
// GAP-49 — Pre-flight DVIR severity tagging (major vs minor vs observation).
export type DvirSeverityLevel = "major" | "minor" | "observation";

export type MajorDefectCatalogEntry = {
  code: string;
  label: string;
  cfr: string;
  keywords: string[];
...
export type PreFlightDvirQueueRow = {
  ...
  item_key: string;
  notes: string;
  submitted_severity: string;
  severity: DvirSeverityLevel | string;
...
export function listPreFlightDvirQueue(
  operatingCompanyId: string,
  params: { severity?: DvirSeverityLevel; status?: "open" | "resolved" | "all" } = {}
) {
  const q = new URLSearchParams({ operating_company_id: operatingCompanyId });
...
export function getPreFlightMajorDefectCatalog() {
  return apiRequest<{ catalog: MajorDefectCatalogEntry[] }>(
    "/api/v1/maintenance/pre-flight/major-defect-catalog"
  );
}
...
export function setPreFlightDvirSeverity(
  ...
  body: {
    ...
    major_defect_code?: string;
    reason?: string;
  }
) {
  return apiRequest<{ ok: true; tag_id: string; severity: DvirSeverityLevel; major_defect_code: string | null }>(
    `/api/v1/maintenance/pre-flight/defects/${encodeURIComponent(defectId)}/severity`,
    { method: "PATCH", body }
  );
}
...
export function routePreFlightDvirDefect(defectId: string, operatingCompanyId: string) {
  return apiRequest<{
    ...
    work_order_id: string | null;
    display_id: string | null;
  }>(`/api/v1/maintenance/pre-flight/defects/${encodeURIComponent(defectId)}/route`, {
    method: "POST",
    body: { operating_company_id: operatingCompanyId },
  });
}
