...
export function listAssignableUsers(operatingCompanyId?: string, signal?: AbortSignal) {
  const query = operatingCompanyId
    ? `?operating_company_id=${encodeURIComponent(operatingCompanyId)}`
    : "";
  return apiRequest<{ users: IdentityUser[] }>(`/api/v1/identity/users/assignable${query}`, { signal });
}
