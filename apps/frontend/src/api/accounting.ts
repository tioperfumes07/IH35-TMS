...
export type RecurringBillGenerationLog = {
  uuid: string;
  template_uuid: string;
  template_name: string;
  vendor_uuid: string;
  generated_bill_uuid: string | null;
  generated_at: string;
  status: "success" | "failed";
  error_message: string | null;
};

export type CreateRecurringBillTemplateInput = {
  vendor_uuid: string;
  template_name: string;
  amount: number;
  memo?: string | null;
...
function rbUrl(path: string, operatingCompanyId: string) {
  const url = new URL(`/api/accounting/recurring-bills${path}`, window.location.origin);
  url.searchParams.set("operating_company_id", operatingCompanyId);
  return url.toString();
}
...
export function listRecurringBillTemplates(
  ...
  const url = new URL(`/api/accounting/recurring-bills/templates`, window.location.origin);
  url.searchParams.set("operating_company_id", operatingCompanyId);
  if (opts.activeOnly) url.searchParams.set("active_only", "true");
  if (opts.dueSoon) url.searchParams.set("due_soon", "true");
  return apiRequest<{ rows: RecurringBillTemplate[] }>(url.toString());
}

export function getRecurringBillTemplate(uuid: string, operatingCompanyId: string) {
  return apiRequest<RecurringBillTemplate>(rbUrl(`/templates/${uuid}`, operatingCompanyId));
}

export function createRecurringBillTemplate(
  operatingCompanyId: string,
  body: CreateRecurringBillTemplateInput,
  idempotencyKey: string
) {
  const url = new URL(`/api/accounting/recurring-bills/templates`, window.location.origin);
  url.searchParams.set("operating_company_id", operatingCompanyId);
  return apiRequest<{ uuid: string }>(url.toString(), {
    method: "POST",
...
export function updateRecurringBillTemplate(
  uuid: string,
  operatingCompanyId: string,
  body: Partial<CreateRecurringBillTemplateInput>
) {
  return apiRequest<{ uuid: string }>(rbUrl(`/templates/${uuid}`, operatingCompanyId), {
    method: "PATCH",
    body: JSON.stringify(body),
  });
}
...
export function deactivateRecurringBillTemplate(uuid: string, operatingCompanyId: string) {
  return apiRequest<{ uuid: string; is_active: false }>(
    rbUrl(`/templates/${uuid}/deactivate`, operatingCompanyId),
    { method: "PATCH" }
  );
}
...
export function generateRecurringBillNow(
  ...
  return apiRequest<{ billUuid: string; nextGenerationDate: string }>(
    rbUrl(`/templates/${uuid}/generate-now`, operatingCompanyId),
    {
      method: "POST",
...
export function listRecurringBillGenerationLog(
  operatingCompanyId: string,
  opts: { templateUuid?: string; limit?: number; offset?: number } = {}
) {
  const url = new URL(`/api/accounting/recurring-bills/generation-log`, window.location.origin);
  url.searchParams.set("operating_company_id", operatingCompanyId);
  if (opts.templateUuid) url.searchParams.set("template_uuid", opts.templateUuid);
  if (opts.limit !== undefined) url.searchParams.set("limit", String(opts.limit));
  if (opts.offset !== undefined) url.searchParams.set("offset", String(opts.offset));
  return apiRequest<{ rows: RecurringBillGenerationLog[] }>(url.toString());
}
