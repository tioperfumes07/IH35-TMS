...
export function DriverLateArrivalCard({ operatingCompanyId, driverId }: Props) {
  ...
  if (query.isLoading) {
    return (
      <div data-testid="driver-late-arrival-card" className="rounded-sm border border-slate-200 bg-white p-3 text-sm text-slate-500">
  ...
  return (
    <div
      ...
      className={`rounded-sm border p-3 ${data.chronic_offender ? "border-slate-300 bg-slate-100" : "border-slate-200 bg-white"}`}
    >
