...
export function UnitPmSchedulesReverseSection({
  ...
  return (
    <section className="space-y-2 rounded-sm border border-gray-200 bg-white p-3" data-testid="vehicle-profile-pm-schedules">
      <h2 className="text-xs font-semibold text-slate-900">PM schedules{rows.length ? ` (${rows.length})` : ""}</h2>
      ...
      {!query.isLoading && !query.isError && rows.length === 0 ? <p className="text-xs text-gray-500">No PM schedules linked to this unit.</p> : null}
      {rows.map((row) => (
