...
export function DvirMaintenanceInspectionsReverseSection({
  ...
  return (
    <section className="space-y-2 rounded-sm border border-gray-200 bg-white p-3" data-testid="dvir-maintenance-inspections-reverse">
      ...
      {!query.isLoading && !query.isError && rows.length === 0 ? <p className="text-xs text-gray-500">No maintenance inspection links this DVIR.</p> : null}
      {rows.map((inspection) => (
        <div key={inspection.id} className="px-2 py-1.5 text-xs">
          <EntityLink
