  ...
  return (
    <section className="space-y-2 rounded-sm border border-gray-200 bg-white p-3" data-testid="load-driver-reports-reverse">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-xs font-semibold text-slate-900">Driver Reports{totalCount ? ` (${totalCount})` : ""}</h3>
      ...
      {query.isLoading ? <p className="text-xs text-gray-500">Loading driver reports…</p> : null}
      {query.isError ? (
      ...
      {!query.isLoading && !query.isError && rows.length === 0 ? <p className="text-xs text-gray-500">No driver reports linked to this load.</p> : null}
      ...
      {rows.length > 0 ? (
        <ul className="space-y-2">
          {rows.map((row) => (
            <li key={row.id} className="rounded-sm border border-gray-200 px-2 py-1.5 text-xs">
