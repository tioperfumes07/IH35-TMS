...
export function WizardStep5({ driverId, fastCardVerified, fastCardWarning, checking }: Props) {
  return (
    <section data-testid="border-wizard-step-5" className="space-y-3">
      <h3 className="text-xs font-semibold">Step 5 — Driver credentials (FAST card)</h3>
      {!driverId ? (
        <p className="text-xs text-amber-700">No driver selected — FAST card verification skipped.</p>
      ) : checking ? (
        <p className="text-xs text-gray-600">Checking driver FAST card expiration…</p>
      ) : (
        <div
          data-testid="fast-card-status"
          className={`rounded-sm border p-3 text-xs ${fastCardVerified ? "border-green-300 bg-green-50" : "border-amber-300 bg-amber-50"}`}
        >
          {fastCardVerified ? (
