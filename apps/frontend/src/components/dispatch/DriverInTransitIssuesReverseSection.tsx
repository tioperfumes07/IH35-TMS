  ...
  return (
    <section className="space-y-2 rounded-sm border border-gray-200 bg-white p-3" data-testid="driver-intransit-issues-reverse">
      <div className="flex items-center justify-between gap-2">
        ...
        <Link className="text-xs font-semibold text-slate-700 underline" to="/dispatch/in-transit-issues">Open issue queue</Link>
      </div>
      {query.isLoading ? <p className="text-sm text-gray-500">Loading in-transit issues…</p> : null}
      {query.isError ? <p className="text-sm text-red-600">Could not load in-transit issues for this driver.</p> : null}
