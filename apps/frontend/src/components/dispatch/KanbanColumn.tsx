...
export function KanbanColumn({ columnKey, title, loads, activeGeofenceBreachVehicleIds, collapsed = false, onLoadClick }: Props) {
  ...
  if (collapsed) {
    return (
      <section className="min-w-[270px] rounded-sm border border-gray-200 bg-white p-2">
  ...
  return (
    <section className="min-w-[290px] flex-1 rounded-sm border border-gray-200 bg-white p-2">
      ...
      <div
        ref={setNodeRef}
        className={`max-h-[68vh] space-y-2 overflow-y-auto rounded-sm p-1 ${isOver ? "bg-slate-100" : "bg-transparent"}`}
      >
