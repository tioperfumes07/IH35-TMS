  ...
  return (
    <section className="space-y-2 rounded-sm border border-gray-200 bg-white p-3" data-testid="vendor-legal-contracts-reverse">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-xs font-semibold text-slate-900">Legal Contracts{rows.length ? ` (${rows.length})` : ""}</h3>
      ...
      {query.isLoading ? <p className="text-xs text-gray-500">Loading legal contracts…</p> : null}
      ...
      {!query.isLoading && !query.isError && rows.length === 0 ? <p className="text-xs text-gray-500">No legal contracts are linked to this vendor.</p> : null}
