...
type Filter =
  | { unit_id: string; insurance_claim_id?: never; related_driver_id?: never }
  | { insurance_claim_id: string; unit_id?: never; related_driver_id?: never }
  | { related_driver_id: string; unit_id?: never; insurance_claim_id?: never };
      ...
      {matters.length > 0 ? (
        <ul className="space-y-2">
          {matters.map((m: Record<string, unknown>) => {
            ...
            const number = String(m.matter_number ?? id.slice(0, 8));
            return (
