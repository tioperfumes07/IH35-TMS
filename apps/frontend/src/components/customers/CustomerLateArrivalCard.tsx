...
function fetchCustomerLateArrival(companyId: string, customerId: string) {
  const from = monthStart();
  const to = today();
  const q = new URLSearchParams({ operating_company_id: companyId, from, to });
  return apiRequest<LateArrivalEntityDetail>(
    `/api/v1/dispatch/analytics/late-arrivals/customer/${customerId}?${q.toString()}`
  );
}
...
export function CustomerLateArrivalCard({ operatingCompanyId, customerId }: Props) {
  const query = useQuery({
    queryKey: ["late-arrival", "customer", operatingCompanyId, customerId],
    queryFn: () => fetchCustomerLateArrival(operatingCompanyId, customerId),
  ...
  return (
    ...
    >
      ...
      {data.chronic_offender ? (
        <div className="mt-2 text-xs font-medium text-amber-800">High late rate (&gt;20%) — review planning</div>
      ) : null}
    </div>
  );
}
