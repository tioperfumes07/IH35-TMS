...
export function MedicalCardsHistorySection({ operatingCompanyId, driverId }: { operatingCompanyId: string; driverId?: string }) {
  ...
  const createMutation = useMutation({
    mutationFn: () => createSafetyMedicalCard(operatingCompanyId, {
    ...
    onSuccess: async () => {
      ...
      setExpiryDate("");
      setNotes("");
      await queryClient.invalidateQueries({ queryKey: ["safety", "medical-cards", operatingCompanyId] });
    },
  });
    ...
    <section className="rounded-sm border border-gray-200 bg-white p-4" data-testid="medical-cards-history-section">
      ...
      <Modal variant="drawer" open={open} onClose={() => setOpen(false)} title="Add DOT medical card">
        <form className="space-y-3" onSubmit={(event) => { event.preventDefault(); createMutation.mutate(); }}>
          ...
          <div className="block text-xs text-slate-600"><span>Issued date</span><DatePicker className="mt-1 w-full" value={issuedDate} onChange={setIssuedDate} /></div>
          <div className="block text-xs text-slate-600"><span>Expiry date</span><DatePicker className="mt-1 w-full" value={expiryDate} onChange={setExpiryDate} /></div>
          ...
          {createMutation.isError ? <p className="text-xs text-red-700">The card could not be saved. Confirm the driver belongs to this company and try again.</p> : null}
