    ...
    <section className="rounded-sm border border-gray-200 bg-white p-4" data-testid="background-checks-section">
      ...
      <Modal variant="drawer" open={open} onClose={() => setOpen(false)} title="Add background check">
        <form className="space-y-3" onSubmit={(event) => { event.preventDefault(); createMutation.mutate(); }}>
          ...
          <label className="block text-xs text-slate-600">Checked date<DatePicker className="mt-1 w-full" value={checkedAt} onChange={setCheckedAt} /></label>
          <label className="block text-xs text-slate-600">Expiry date (optional)<DatePicker className="mt-1 w-full" value={expiryDate} onChange={setExpiryDate} /></label>
