...
export function CustomerCargoClaimsReverseSection({
  operatingCompanyId,
  customerId,
}: {
  ...
  const query = useQuery({
    queryKey: ["cargo-claims", operatingCompanyId, "customer", customerId],
        ...
        {(query.data?.incidents ?? []).map((row) => (
          <div key={String(row.id)} className="text-sm">
            <EntityLinkOrTombstone
              ...
              name={row.claim_reason_code ?? row.description}
              noun="Cargo claim"
            />
            {row.load_id ? (
              <>
