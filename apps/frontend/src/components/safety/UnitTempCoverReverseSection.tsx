...
export function UnitTempCoverReverseSection({ operatingCompanyId, unitId }: { operatingCompanyId: string; unitId: string }) {
  ...
  return <section className="space-y-2 rounded-sm border border-gray-200 bg-white p-3" data-testid="unit-temp-cover-reverse">
    <div className="flex items-center justify-between gap-2">
      <h3 className="text-xs font-semibold text-slate-900">Temporary Driver Coverage{rows.length ? ` (${rows.length})` : ""}</h3>
    ...
    {query.isLoading ? <p className="text-xs text-gray-500">Loading temporary coverage…</p> : null}
    ...
    {!query.isLoading && !query.isError && rows.length === 0 ? <p className="text-xs text-gray-500">No active temporary driver coverage is linked to this unit.</p> : null}
