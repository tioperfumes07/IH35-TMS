  ...
  return <section className="space-y-4 rounded-sm border border-gray-200 bg-white p-3" data-testid="driver-temp-cover-reverse">
    <div className="space-y-2" data-testid="driver-leave-requests-reverse">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-xs font-semibold text-slate-900">Leave Requests{leaveRows.length ? ` (${leaveRows.length})` : ""}</h3>
      ...
      {leaveQuery.isLoading ? <p className="text-xs text-gray-500">Loading leave requests…</p> : null}
      ...
      {!leaveQuery.isLoading && !leaveQuery.isError && leaveRows.length === 0 ? <p className="text-xs text-gray-500">No leave requests are linked to this driver.</p> : null}
    ...
    <div className="flex items-center justify-between gap-2">
      <h3 className="text-xs font-semibold text-slate-900">Temporary Cover Assignments{rows.length ? ` (${rows.length})` : ""}</h3>
    </div>
    {query.isLoading ? <p className="text-xs text-gray-500">Loading temporary assignments…</p> : null}
    ...
    {!query.isLoading && !query.isError && rows.length === 0 ? <p className="text-xs text-gray-500">No active temporary assignments are linked to this driver.</p> : null}
