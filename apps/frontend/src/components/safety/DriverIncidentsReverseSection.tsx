...
function DriverIncidentKind({ companyId, driverId, kind, label, openKind }: { companyId: string; driverId: string; kind: SafetyIncidentType; label: string; openKind: "damage_reports_driver" | "trailer_interchanges_driver" | "cargo_claims_driver" }) {
  ...
  return <div className="rounded-sm border border-gray-200 bg-white p-3"><div className="flex items-center justify-between gap-2"><h3 className="text-sm font-semibold text-gray-800">{label}</h3><EntityLink kind={openKind} id={driverId} label={`Open ${label}`} className="text-xs font-semibold text-slate-700 underline" /></div>{query.isError ? <ListErrorState status={0} message={`${label} could not be loaded.`} onRetry={() => void query.refetch()} /> : null}{query.isLoading ? <p className="mt-2 text-sm text-gray-500">Loading {label.toLowerCase()}…</p> : null}{!query.isLoading && !query.isError && incidents.length === 0 ? <p className="mt-2 text-sm text-gray-500">No {label.toLowerCase()} are linked to this driver.</p> : null}<div className="mt-2 space-y-2">{incidents.map((row, index) => <div key={row.id == null ? `${kind}-${index}` : String(row.id)} className="text-sm"><EntityLinkOrTombstone kind={kind} id={row.id == null ? null : String(row.id)} name={row.description} noun={label.slice(0, -1)} />{row.load_id ? <> · <EntityLinkOrTombstone kind="load" id={String(row.load_id)} name={row.load_number} noun="Load" /></> : null}</div>)}</div></div>;
}
