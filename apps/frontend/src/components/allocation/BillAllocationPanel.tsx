  ...
  return (
    <section className="space-y-3 rounded-sm border border-gray-200 bg-white p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-xs font-semibold text-gray-900">Bill unit allocation</h3>
        ...
        <button
          type="button"
          className="rounded-sm bg-[#1F2A44] px-3 py-1 text-xs font-medium text-white hover:bg-[#1F2A44] disabled:opacity-60"
      ...
      {sourceMode === "fallback" ? (
        <p className="rounded-sm border border-slate-300 bg-slate-100 px-3 py-2 text-xs text-slate-700">
      ...
      <div className="space-y-2">
        ...
        {isLoadingAssets ? <p className="text-xs text-gray-500">Loading assets…</p> : null}
        <div className="grid gap-2 md:grid-cols-2">
          {assets.map((asset) => (
            <label key={asset.id} className="flex items-center gap-2 rounded-sm border border-gray-200 px-2 py-1 text-xs">
              <input
                type="checkbox"
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {selectedAssets.map((asset) => (
            <label key={asset.id} className="text-xs font-semibold text-gray-600">
              {asset.unit_code} %
              <input
                className="mt-1 w-full rounded-sm border border-gray-300 px-2 py-1 text-xs font-normal text-gray-900"
