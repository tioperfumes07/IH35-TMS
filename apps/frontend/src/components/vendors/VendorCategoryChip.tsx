...
export function VendorCategoryChip({ code }: { code: string | null | undefined }) {
  ...
  return (
    <span className={`inline-flex max-w-40 truncate rounded-sm border px-1.5 py-0.5 text-xs font-semibold capitalize ${vendorCategoryChipClasses(code)}`} title={label}>
      {label}
    </span>
  );
}
