...
const columns: Array<ParityColumn<UnitWorkOrderHistoryRow>> = [
...
export function UnitMaintenanceHistorySection({ operatingCompanyId, unitId }: { operatingCompanyId: string; unitId: string }) {
  ...
  return (
    <section className="mt-3 overflow-hidden rounded-sm border border-gray-200 bg-white" data-testid="unit-maintenance-history">
      ...
      <ParityTable storageKey="unit-maintenance-history" tableTestId="unit-maintenance-history-table" columns={columns} rows={query.data?.rows ?? []} rowKey={(row) => row.id} loading={query.isLoading} emptyText="No work orders are linked to this unit." initialPageSize={25} pageSizeOptions={[25, 50, 100]} />
    </section>
  );
}
