...
// K2 (GO-23 Wave 4, 2026-09-02): SelectCombobox was components/shared/SelectCombobox.tsx, a
// native-<select>-drop-in adapter (children-based <option>/<optgroup> API) that already delegated
// through SimpleCombobox above to this file's own Combobox engine -- relocated per the owner's K2
// ruling (KEEP components/Combobox.tsx; retire the three adapters) so every importer converges on
// ONE module. Logic is verbatim unchanged; JSX below calls SimpleCombobox explicitly (not the bare
// Combobox engine -- different, incompatible prop shape) exactly as the original did via its own
// `import { SimpleCombobox as Combobox } from "../Combobox"` aliasing.
type SelectComboboxProps = Omit<SelectHTMLAttributes<HTMLSelectElement>, "onChange" | "value" | "defaultValue"> & {
...
type SelectComboboxOptionRow = {
  value: string;
  label: string;
  disabled?: boolean;
};

function normalizeSelectComboboxValue(
  value: SelectComboboxProps["value"] | SelectComboboxProps["defaultValue"],
): string | null {
...
function flattenSelectComboboxOptions(children: ReactNode): SelectComboboxOptionRow[] {
  const out: SelectComboboxOptionRow[] = [];
  Children.forEach(children, (child) => {
    ...
    if (child.type === "optgroup") {
      out.push(...flattenSelectComboboxOptions(props.children));
    }
  });
  return out;
}
...
export function SelectCombobox({
  ...
  const [internalValue, setInternalValue] = useState<string | null>(() => normalizeSelectComboboxValue(defaultValue));
  const options = useMemo(() => flattenSelectComboboxOptions(children), [children]);
  const selected = controlled ? normalizeSelectComboboxValue(value) : internalValue;
