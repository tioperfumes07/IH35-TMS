...
export function VendorInsurancePoliciesReverseSection({ operatingCompanyId, vendorId }: { operatingCompanyId: string; vendorId: string }) {
  ...
  return <section className="space-y-2 rounded-sm border border-gray-200 bg-white p-3" data-testid="vendor-insurance-policies-reverse">
    <div className="flex items-center justify-between gap-2">
      <h3 className="text-xs font-semibold text-slate-900">Insurance Policies{rows.length ? ` (${rows.length})` : ""}</h3>
    ...
    {query.isLoading ? <p className="text-xs text-gray-500">Loading insurance policies…</p> : null}
    {query.isError ? (
      <ListErrorState
    ...
    {!query.isLoading && !query.isError && rows.length === 0 ? <p className="text-xs text-gray-500">No active insurance policies are linked to this vendor.</p> : null}
