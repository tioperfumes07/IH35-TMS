...
export function buildEditPrefill(load: LoadDetail): AnyValues {
  ...
  const stops = (load.stops ?? []).map((s, i) => ({
    ...
    pickup_time_type_id: str(s.pickup_time_type_id),
  ...
  return {
    ...
    detention_reason_id: str(load.detention_reason_id),
    catalog_load_type_id: str(load.catalog_load_type_id),
    load_trailer_equipment_id: str(load.load_trailer_equipment_id),
...
export function buildEditPatchBody(values: AnyValues, dirty: Dirty, operatingCompanyId: string): AnyValues {
  ...
  if (isDirty("stops")) {
    ...
    body.stops = stops.map((s, i) => ({
      ...
      pickup_time_type_id: str(s.pickup_time_type_id) || undefined,
