...
/**
 * CargoSensorTimeline — GAP-64 / CAP-14
 * Per-load temp/humidity chart over time.
 */
import { useMemo } from "react";
import { useQuery } from "@tanstack/react-query";
...
function fetchTimeline(companyId: string, loadUuid: string) {
  const q = new URLSearchParams({ operating_company_id: companyId });
  return apiRequest<{ rows: Reading[] }>(
    `/api/v1/dispatch/cargo-sensors/load/${loadUuid}/timeline?${q.toString()}`
  );
}

export function CargoSensorTimeline({ loadUuid, operatingCompanyId }: Props) {
  const timelineQ = useQuery({
    queryKey: ["dispatch", "cargo-sensors", "timeline", operatingCompanyId, loadUuid],
    queryFn: () => fetchTimeline(operatingCompanyId, loadUuid),
    enabled: Boolean(operatingCompanyId && loadUuid),
  });

  const chartData = useMemo(
    () =>
      (timelineQ.data?.rows ?? []).map((row) => ({
        at: new Date(row.reading_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
  ...
  return (
    <section className="rounded border border-gray-200 bg-white p-4" data-testid="cargo-sensor-timeline">
      <h2 className="text-sm font-semibold text-gray-900">Cargo sensor timeline</h2>
      <p className="text-xs text-gray-500">CAP-14 reefer temp/humidity · FSMA compliance evidence</p>
      {timelineQ.isLoading ? <p className="mt-2 text-xs text-gray-500">Loading readings…</p> : null}
      {!timelineQ.isLoading && chartData.length === 0 ? (
        <p className="mt-2 text-xs text-gray-500">No sensor readings yet for this load.</p>
      ) : null}
      {chartData.length > 0 ? (
        <div className="mt-3 h-48">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Tooltip />
              <Line yAxisId="temp" type="monotone" dataKey="temp" stroke="#0284c7" dot={false} name="Temp °C" />
              <Line yAxisId="humidity" type="monotone" dataKey="humidity" stroke="#16a34a" dot={false} name="Humidity %" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      ) : null}
