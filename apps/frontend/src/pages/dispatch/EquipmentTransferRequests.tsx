...
export function EquipmentTransferRequests() {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      apiRequest<{ data: TransferRow[] }>(
        `/api/v1/dispatch/equipment-transfers/in-progress?operating_company_id=${encodeURIComponent(companyId)}`
      ),
  });

  return (
    <div className="space-y-4 p-4">
      <PageHeader title="Equipment transfer requests" subtitle="WF-047 dual-confirm queue" />
