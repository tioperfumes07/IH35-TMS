...
function responseRows(reportId: string, payload: any): Record<string, unknown>[] {
  ...
  if (reportId === "dot-audit-pack") return (payload.dot_inspections ?? []) as Record<string, unknown>[];
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <button type="button" className="rounded-sm border border-slate-300 bg-white px-3 py-1.5 text-sm font-semibold text-slate-700 disabled:opacity-50" disabled aria-label="Save (available soon)">
              Save
            </button>
          </div>
        }
