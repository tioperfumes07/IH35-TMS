...
import { listAccountingCustomers, type AccountingCustomerListRow } from "../../api/accounting-qbo-entities";
...
async function postAccountingCustomer(
  ...
  return apiRequest<{ id: string }>("/api/v1/accounting/customers", {
    method: "POST",
...
export function CustomersListPage() {
  ...
  const categories = useMemo(() => ["All", "Broker", "Direct", "Other"], []);

  const listQ = useQuery({
    queryKey: ["accounting", "customers", companyId, search, category, cursor],
    queryFn: async () => {
      try {
        return await listAccountingCustomers(companyId, {
        ...
        if (e instanceof ApiError && e.status === 404) {
          return { items: [] as AccountingCustomerListRow[], next_cursor: null as string | null, wave404: true as const };
        }
        throw e;
      }
    },
  ...
  const createMut = useMutation({
    mutationFn: (body: CreateForm) =>
      postAccountingCustomer(companyId, {
    ...
    onSuccess: (res) => {
      ...
      setCreateOpen(false);
      form.reset();
      void qc.invalidateQueries({ queryKey: ["accounting", "customers"] });
      if (res.id) navigate(`/customers/${res.id}`);
    },
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-4 bg-gray-50 p-4">
      ...
      {editCategories && selectedCount > 0 ? (
        <div className="sticky bottom-0 z-10 flex flex-wrap items-center gap-3 rounded-t-lg border border-gray-200 bg-white p-3 shadow-lg">
          ...
          <Button className="bg-emerald-600 text-white" aria-label={`Apply category to ${selectedCount} customers`}>
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        {!companyId ? (
          ...
          <p className="p-6 text-sm text-red-600">Could not load customers.</p>
          ...
          <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
            ...
            <p className="text-base font-medium text-gray-800">No customers yet</p>
            <Button className="bg-emerald-600 text-white" onClick={() => setCreateOpen(true)} aria-label="Create first customer">
              + Customer
            </Button>
          </div>
        ) : (
              ...
              <thead className="border-b border-gray-200 bg-gray-50 text-[11px] font-semibold uppercase tracking-wide text-gray-600">
                <tr>
                  ...
                  <th className="px-3 py-2">
                    <button type="button" className="flex items-center gap-1 font-semibold" aria-label="Sort by name">
                      Name
                  ...
                  <th className="px-3 py-2 text-right">Total billed YTD</th>
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 font-medium text-gray-900">
                      <Link to={`/customers/${r.id}`} className="text-blue-700 hover:underline">
                        {r.display_name || "—"}
                      </Link>
                    </td>
                    <td className="px-3 py-2 text-gray-700">{r.email ?? "—"}</td>
                    <td className="px-3 py-2 text-gray-700">{r.phone ?? "—"}</td>
                    <td className="px-3 py-2 text-right tabular-nums">{r.open_invoice_count ?? "—"}</td>
                    <td className="px-3 py-2 text-right tabular-nums">{formatCurrencyCents(r.open_balance_cents ?? 0)}</td>
                    <td className="px-3 py-2 text-right tabular-nums">{formatCurrencyCents(r.total_billed_ytd_cents ?? 0)}</td>
                    ...
                    <td className="px-3 py-2 text-gray-700">{r.last_invoice_date ? formatDate(r.last_invoice_date) : "—"}</td>
                    <td className="px-3 py-2">
        ...
        {listQ.data?.next_cursor ? (
          <div className="flex justify-end border-t border-gray-100 p-2">
            <Button variant="secondary" onClick={() => setCursor(listQ.data.next_cursor ?? null)} aria-label="Load more customers">
              Next page
            </Button>
          </div>
        ) : null}
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Create customer">
        ...
        >
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="secondary" type="button" onClick={() => setCreateOpen(false)} aria-label="Cancel create customer">
              Cancel
            </Button>
            <Button type="submit" className="bg-emerald-600 text-white" disabled={createMut.isPending} aria-label="Submit create customer">
              + Create
            </Button>
          </div>
        </form>
