...
export function LoadCostsBoardPage() {
  ...
  const rows=useMemo(()=>query.data?.rows??[],[query.data?.rows]); const visible=useMemo(()=>rows.filter(r=>matches(r,filter)),[rows,filter]); const revenue=visible.reduce((n,r)=>n+Number(r.revenue_cents),0); const costs=visible.reduce((n,r)=>n+rowCosts(r),0); const driver=visible.reduce((n,r)=>n+rowPay(r),0); const margin=revenue-costs-driver;
  const columns:Array<ParityColumn<BoardRow>>=[
    ...
    {key:"late_fee",label:"Late fee",testId:"col-late-fee",sortable:true,className:"text-right",render:r=>fmt(Number(r.late_fee_cents))},
    {key:"lumper",label:"Lumper",testId:"col-lumper",sortable:true,className:"text-right",render:r=>fmt(Number(r.lumper_cents))},
    {key:"fuel",label:"Fuel",testId:"col-fuel",sortable:true,className:"text-right",render:r=>fmt(Number(r.fuel_cents))},
    {key:"rm_exp",label:"R&M exp",testId:"col-rm-exp",sortable:true,className:"text-right",render:r=>fmt(Number(r.rm_exp_cents))},
    {key:"repairs_maintenance",label:"R&M WO",testId:"col-repairs-maintenance",sortable:true,className:"text-right",render:r=>fmt(Number(r.repairs_maintenance_cents))},
