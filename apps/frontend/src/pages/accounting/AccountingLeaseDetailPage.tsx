...
export function AccountingLeaseDetailPage() {
  ...
  return (
    <AccountingSubNavWrapper title={detail?.contract.display_id || "Lease contract"} subtitle="ASC 842 contract, linked assets, and stored schedule">
      {!companyId ? <p className="rounded-sm border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">Select an operating company to view this lease.</p> : null}
      ...
      {detailQuery.isPending && companyId ? <p className="text-xs text-slate-500">Loading lease contract…</p> : null}
      {detail ? (
        <div className="space-y-4">
          ...
          <section><h2 className="mb-2 text-xs font-semibold text-slate-900">Linked assets</h2><ParityTable columns={assetColumns} rows={detail.assets} rowKey={(row) => row.id} storageKey="accounting-lease-assets" tableTestId="accounting-lease-assets-table" emptyText="No active assets linked to this lease." /></section>
