...
export function AccountingScheduleRowDetailPage() {
  ...
  return <AccountingSubNavWrapper title={row ? `${names[row.kind]} ${row.sequence}` : "Schedule period"} subtitle="Canonical period, parent record, and posted journal entry">
    {!companyId ? <p className="rounded-sm border border-slate-200 bg-slate-50 p-3 text-xs">Select an operating company to view this period.</p> : null}
    ...
    {query.isPending && companyId && validKind ? <p className="text-xs text-slate-500">Loading schedule period…</p> : null}
