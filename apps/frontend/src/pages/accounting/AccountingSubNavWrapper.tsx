...
import { ACCOUNTING_CLEAN_TABS, ACCOUNTING_MORE_TABS, ACCOUNTING_QBO_MIRROR_TABS } from "./subnav-manifest";
...
export function AccountingSubNavWrapper({ title = "Accounting", subtitle, actions, children, kpiStrip }: Props) {
  ...
  const [qboMenuOpen, setQboMenuOpen] = useState(false);
  const createMenuRef = useRef<HTMLDivElement | null>(null);
  const moreMenuRef = useRef<HTMLDivElement | null>(null);
  const qboMenuRef = useRef<HTMLDivElement | null>(null);

  const moreActive = ACCOUNTING_MORE_TABS.some((t) => pathname === t.to || pathname.startsWith(`${t.to}/`));
  const qboActive = ACCOUNTING_QBO_MIRROR_TABS.some((t) => pathname === t.to || pathname.startsWith(`${t.to}/`));
  ...
  useEffect(() => {
    const onDown = (event: MouseEvent) => {
      ...
      if (!qboMenuRef.current?.contains(event.target as Node)) setQboMenuOpen(false);
    };
    ...
    <div className="space-y-4" data-accounting-subnav-wrapper="true">
      ...
      <nav className="overflow-x-auto rounded-sm border border-gray-200 bg-white px-2 py-1" aria-label="Accounting sub-navigation">
        <div className="flex min-w-max gap-1">
          ...
          <div ref={qboMenuRef} className="relative">
            <button
              type="button"
              data-testid="accounting-qbo-menu-trigger"
              onClick={() => setQboMenuOpen((o) => !o)}
              className={`rounded px-3 py-1 text-sm whitespace-nowrap ${
                qboActive ? "border-b-2 border-slate-300 bg-gray-100 font-semibold text-gray-900" : "text-gray-700 hover:bg-gray-50"
              }`}
            >
              QBO ▾
            </button>
            {qboMenuOpen ? (
              <div
                data-testid="accounting-qbo-menu"
                className="absolute left-0 top-full z-20 mt-1 min-w-[220px] rounded-sm border border-gray-200 bg-white shadow-md"
              >
                {ACCOUNTING_QBO_MIRROR_TABS.map((item) => (
                  <NavLink
                    key={item.label}
                    to={item.to}
                    onClick={() => setQboMenuOpen(false)}
                    className={({ isActive }: { isActive: boolean }) =>
                      `block border-b border-gray-100 px-3 py-2 text-sm last:border-b-0 ${
                        isActive ? "bg-gray-100 font-semibold text-gray-900" : "text-gray-800 hover:bg-gray-50"
                      }`
