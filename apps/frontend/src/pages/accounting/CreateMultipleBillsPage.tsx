  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      for (const row of rows) {
        ...
        if (row.driver_id) {
          // Label chrome is owned by EntityPicker; memo keeps the FK id for audit traceability.
          memoParts.push(`driver:${row.driver_id}`);
        }
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-gray-100">
                ...
                <td className="px-2 py-1.5">
                  <div className="min-w-[140px]" data-testid="multi-bills-driver">
                    {/* SAF-B29: EntityPicker kind=driver — never Combobox over listDrivers(limit:200). */}
                    <EntityPicker
                      kind="driver"
                      operatingCompanyId={companyId}
                      ...
                      allowClear
                      nestedInDrawer
                      dataTestId={`multi-bills-driver-${row.id}`}
                    />
                  </div>
                </td>
                <td className="px-2 py-1.5">
