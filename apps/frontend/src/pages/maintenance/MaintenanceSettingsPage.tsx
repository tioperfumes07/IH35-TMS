    ...
    <div className="space-y-2 rounded-sm border border-gray-200 bg-white p-3">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <section className="rounded-sm border border-gray-200 p-2">
          ...
          <input className="mt-1 h-8 w-full rounded-sm border border-gray-300 px-2 text-sm" value={String(settings?.pm_interval_days_default ?? 30)} readOnly />
        ...
        <section className="rounded-sm border border-gray-200 p-2">
          ...
          <input className="mt-1 h-8 w-full rounded-sm border border-gray-300 px-2 text-sm" value={String(settings?.maintenance_vendors ?? 0)} readOnly />
          <label className="mt-2 block text-xs text-gray-600">Default shop location</label>
          <input className="mt-1 h-8 w-full rounded-sm border border-gray-300 px-2 text-sm" value={String(settings?.default_shop_location ?? "Main yard")} readOnly />
        </section>
        ...
        <section className="rounded-sm border border-gray-200 p-2">
          ...
          <input className="mt-1 h-8 w-full rounded-sm border border-gray-300 px-2 text-sm" value="Auto-assign by first available bay" readOnly />
        </section>
        ...
        <section className="rounded-sm border border-gray-200 p-2">
          ...
          <input className="mt-1 h-8 w-full rounded-sm border border-gray-300 px-2 text-sm" value={settings?.notification_email_enabled ? "Yes" : "No"} readOnly />
          <label className="mt-2 block text-xs text-gray-600">PM schedules tracked</label>
          <input className="mt-1 h-8 w-full rounded-sm border border-gray-300 px-2 text-sm" value={String(settings?.pm_schedules ?? 0)} readOnly />
        </section>
      </div>

      <div className="flex justify-end">
        <button type="button" className="rounded-sm border border-gray-300 bg-white px-3 py-1.5 text-xs font-semibold text-gray-700">
          Save
        </button>
      </div>
    </div>
