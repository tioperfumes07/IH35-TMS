    ...
    <div className="space-y-3">
      ...
      {canCreate ? (
        <div className="grid gap-2 rounded-sm border border-gray-200 bg-white p-3 md:grid-cols-6">
          ...
          <input className="rounded-sm border border-gray-300 px-2 py-1 text-xs" placeholder="Respondent driver_id" value={form.respondent_driver_id} onChange={(e) => setForm((v) => ({ ...v, respondent_driver_id: e.target.value }))} />
          <input className="rounded-sm border border-gray-300 px-2 py-1 text-xs" placeholder="Type" value={form.complaint_type} onChange={(e) => setForm((v) => ({ ...v, complaint_type: e.target.value }))} />
          ...
          <button type="button" className="rounded-sm bg-[#1f2a44] px-2 py-1 text-xs font-semibold text-white disabled:opacity-60" disabled={!form.complainant_external_name || !form.respondent_driver_id || !form.complaint_type || !form.summary || createMutation.isPending} onClick={() => createMutation.mutate()}>
            + Create
          </button>
        </div>
      ) : null}
