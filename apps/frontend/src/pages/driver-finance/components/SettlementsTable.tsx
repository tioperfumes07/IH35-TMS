...
// Display-only ParityTable migration: column order, amount formatting ($ + toFixed(2)),
// status pill, debt-flag red highlight, and the Open → handler are preserved 1:1.
function buildColumns(onOpen: (id: string) => void): Array<ParityColumn<SettlementListRow>> {
  return [
    {
      key: "driver",
      label: "Driver",
    ...
    },
    {
      ...
      render: (row) => <span className={`rounded-full px-2 py-0.5 ${statusClass(row.status)}`}>{row.status}</span>,
    },
...
export function SettlementsTable({ rows, onOpen }: Props) {
  ...
  // shared/bookmarked settlements link preserves the chosen column sort. ParityTable's
  // controlled-sort mode performs the row sort using each column's sortValue.
