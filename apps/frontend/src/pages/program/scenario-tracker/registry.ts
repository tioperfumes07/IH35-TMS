...
> = [
  { key: "scenario.customer", title: "New Customer", lane: "ops", kind: "ops", trigger: "create customer (universal picker/creator)", spec_ref: "QBO Lists parity" , links: "Canonical entity-scoped write; A/R-ready; links ⇄ loads, invoices, factoring. Relationship health live (LV-001 fixed, proven on prod).", href: "/customers?create=1" },
  { key: "scenario.driver_onboarding", title: "New Driver onboarding", lane: "ops", kind: "ops", trigger: "onboard driver", spec_ref: "MUST 3.13.1" , links: "One txn: driver + driver-vendor + W-8BEN + escrow ledger + pay_basis + Driver Receivable.", href: "/drivers?create=1" },
  ...
  },
  {
    ...
    lane: "ops",
    kind: "ops",
    trigger: "Drop original trailer, hook replacement, keep same load #",
    spec_ref: "COMPLICATED-BATTERY-02",
    links: "load_assignment_history previous_trailer_id ≠ new_trailer_id; load still one customer/invoice; IFTA miles stay on the load.",
    href: "/dispatch",
  },
  {
    ...
    lane: "money",
    kind: "money",
    trigger: "Tow or shop bill from the in-transit WO",
    je: "DR Repair / CR A/P — TMS-native bill only",
    spec_ref: "COMPLICATED-BATTERY-03",
    links: "bill.linked_work_order_uuid ⇄ WO ⇄ intransit_issues.load_id; JE required (posting LIVE). QBO clones do not count.",
  ...
  },
  {
    ...
    lane: "ops",
    kind: "ops",
    trigger: "Purchase receipt qty + optional WO consume",
    je: "DR Inventory / CR A/P when parts GL flag ON",
    spec_ref: "COMPLICATED-BATTERY-04",
    links: "maintenance.parts_purchases ⇄ parts_inventory on-hand ⇄ work_order_id ⇄ bill. Print receive + WO.",
