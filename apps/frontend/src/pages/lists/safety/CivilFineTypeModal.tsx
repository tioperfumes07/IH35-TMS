...
export function CivilFineTypeModal({ open, companyId, row, onClose, onSaved }: Props) {
  ...
  return (
    <Modal variant="drawer" open={open} onClose={onClose} title={isEdit ? "Edit Civil Fine Type" : "Create Civil Fine Type"}>
      <div className="space-y-3">
        <label className="block text-xs font-semibold text-gray-600">
          Code
          <input value={form.code} onChange={(event) => setForm((v) => ({ ...v, code: event.target.value.toUpperCase() }))} className="mt-1 h-9 w-full rounded-sm border border-gray-300 px-2 text-xs" placeholder="DOT-FINE" />
        ...
        <label className="block text-xs font-semibold text-gray-600">
          Description
          <textarea value={form.description} onChange={(event) => setForm((v) => ({ ...v, description: event.target.value }))} rows={3} className="mt-1 w-full rounded-sm border border-gray-300 px-2 py-1 text-xs" />
        </label>

        <label className="block text-xs font-semibold text-gray-600">
          Sort Order
          <input type="number" min={0} step={1} value={form.sort_order} onChange={(event) => setForm((v) => ({ ...v, sort_order: Number(event.target.value || 0) }))} className="mt-1 h-9 w-full rounded-sm border border-gray-300 px-2 text-xs" />
