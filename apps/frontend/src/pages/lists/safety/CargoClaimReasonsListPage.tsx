...
export function CargoClaimReasonsListPage() {
  ...
  const emptyText = useMemo(() => {
    if (query.isLoading) return "Loading cargo claim reasons...";
    if (rows.length > 0) return "";
    return "No cargo claim reasons found.";
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="cursor-pointer border-t border-gray-100 hover:bg-gray-50" onClick={() => { setSelectedRow(row); setModalOpen(true); }}>
                ...
                <td className="px-3 py-2 text-slate-700">{row.claim_category ? CATEGORY_LABELS[row.claim_category] : "—"}</td>
