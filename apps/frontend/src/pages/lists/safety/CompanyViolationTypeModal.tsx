...
type Props = {
  open: boolean;
  companyId: string;
  row: CompanyViolationTypeRow | null;
  onClose: () => void;
  onSaved: () => void;
};
...
export function CompanyViolationTypeModal({ open, companyId, row, onClose, onSaved }: Props) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (row) {
        await updateCompanyViolationType(companyId, row.id, payload);
      } else {
        await createCompanyViolationType(companyId, payload);
      }
      onSaved();
      onClose();
    } catch (error) {
  ...
  async function deactivate() {
    ...
    setSubmitError("");
    try {
      await deactivateCompanyViolationType(companyId, row.id);
      onSaved();
      onClose();
    } catch (error) {
      if (error instanceof ApiError) {
  ...
  return (
    <Modal variant="drawer" open={open} onClose={onClose} title={isEdit ? "Edit Company Violation Type" : "Create Company Violation Type"}>
