...
import type { MaintenanceCatalogCreateBody, MaintenanceCatalogRow, MaintenanceCatalogUpdateBody } from "../../../api/catalogs-maintenance";
import { Button } from "../../../components/Button";
import { Modal } from "../../../components/Modal";

const CODE_REGEX = /^[A-Z][A-Z0-9-]+$/;

export type MaintenanceCatalogClient = {
  create: (operating_company_id: string, body: MaintenanceCatalogCreateBody) => Promise<MaintenanceCatalogRow>;
  update: (id: string, operating_company_id: string, body: MaintenanceCatalogUpdateBody) => Promise<MaintenanceCatalogRow>;
...
type Props = {
  ...
  mode: "create" | "edit";
  row: MaintenanceCatalogRow | null;
  onClose: () => void;
  onSaved: () => void;
};
...
export function MaintenanceCatalogModal({ open, operatingCompanyId, displayName, client, mode, row, onClose, onSaved }: Props) {
  ...
  return (
    <Modal variant="drawer" open={open} onClose={onClose} title={mode === "create" ? `New ${displayName}` : `Edit ${displayName}`}>
      <div className="space-y-3">
        ...
        <label className="block text-xs font-semibold text-gray-600">
          Display Name
          <input value={form.display_name} onChange={(event) => setForm((value) => ({ ...value, display_name: event.target.value }))} className="mt-1 h-9 w-full rounded-sm border border-gray-300 px-2 text-xs" />
          {errors.display_name ? <div className="mt-1 text-[11px] text-red-700">{errors.display_name}</div> : null}
        </label>

        <label className="block text-xs font-semibold text-gray-600">
          Description
          <textarea value={form.description} onChange={(event) => setForm((value) => ({ ...value, description: event.target.value }))} rows={3} className="mt-1 w-full rounded-sm border border-gray-300 px-2 py-1 text-xs" />
        </label>

        <label className="block text-xs font-semibold text-gray-600">
          Sort Order
          <input type="number" min={0} step={1} value={form.sort_order} onChange={(event) => setForm((value) => ({ ...value, sort_order: Number(event.target.value || 0) }))} className="mt-1 h-9 w-full rounded-sm border border-gray-300 px-2 text-xs" />
