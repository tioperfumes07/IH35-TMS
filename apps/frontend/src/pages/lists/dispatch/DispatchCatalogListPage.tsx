...
import type { DispatchCatalogRow } from "../../../api/catalogs-dispatch";
import { Button } from "../../../components/Button";
import { PageHeader } from "../../../components/layout/PageHeader";
import { useCompanyContext } from "../../../contexts/CompanyContext";
import { DispatchCatalogModal, type DispatchCatalogClient } from "./DispatchCatalogModal";
...
type Props = {
  client: DispatchCatalogClient & {
    list: (filters: {
      ...
      limit?: number;
      offset?: number;
    }) => Promise<{ rows: DispatchCatalogRow[]; total: number }>;
  };
  displayName: string;
...
export function DispatchCatalogListPage({ client, displayName, breadcrumbPath }: Props) {
  const { selectedCompanyId } = useCompanyContext();
  const companyId = selectedCompanyId ?? "";
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState<"true" | "false" | "all">("true");
  const [modalMode, setModalMode] = useState<"create" | "edit">("create");
  const [selectedRow, setSelectedRow] = useState<DispatchCatalogRow | null>(null);
  const [modalOpen, setModalOpen] = useState(false);

  const query = useQuery({
    queryKey: ["catalogs", "dispatch", displayName, companyId, search, status],
