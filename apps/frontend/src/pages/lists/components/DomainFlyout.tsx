...
export function DomainFlyout({ rows, onCatalogClick, onViewAllInDomain, onCreateNewCatalog }: Props) {
  return (
    <div className="absolute left-0 top-full z-40 mt-1 w-max min-w-[280px] whitespace-nowrap rounded-sm border border-slate-200 bg-white p-2 shadow-xl">
      ...
      <div className="mt-2 border-t border-slate-200 pt-2">
        <button type="button" onClick={onCreateNewCatalog} className="block w-full rounded-sm px-2 py-1 text-left text-xs font-semibold text-slate-700 hover:bg-slate-100">
          + Create new catalog
        </button>
        <button type="button" onClick={onViewAllInDomain} className="mt-1 block w-full rounded-sm px-2 py-1 text-left text-xs font-semibold text-slate-700 hover:bg-slate-100">
          View all in domain
        </button>
      </div>
    </div>
