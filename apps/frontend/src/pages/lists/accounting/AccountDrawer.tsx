...
import {
  ...
  QBO_TYPE_GROUPS,
  QBO_TYPE_META,
  findQboTypeForDetail,
} from "./coa-account-type-map";
...
type FormState = {
  ...
  // qbo_type = the fine QBO Account type shown/selected in the UI (e.g. "Bank", "Accounts
  // Receivable (A/R)") — see coa-account-type-map.ts. account_type is always kept in lockstep as
  // the derived coarse 8-value enum that actually goes on the wire (catalogs.accounts CHECK).
  qbo_type: string;
  account_type: string;
...
function formFromAccount(account: CatalogAccount): FormState {
  return {
    ...
    qbo_type: findQboTypeForDetail(account.account_type, account.account_subtype) ?? "",
...
export function AccountDrawer({ open, mode, account, operatingCompanyId, onClose, onSaved }: Props) {
  ...
  // Detail type options cascade off the selected QBO Account type — static client-side map
  // (coa-account-type-map.ts), no endpoint. Legacy/edit-mode safety: if the persisted account_subtype
  // isn't one of the static list's entries (older data), keep it selectable at the top so save-without-
  // touching-this-field never silently rewrites the value.
  const detailOptionsForType = useMemo<string[]>(() => {
    const base = form.qbo_type ? DETAIL_TYPES[form.qbo_type] ?? [] : [];
    if (form.account_subtype && !base.includes(form.account_subtype)) {
      return [form.account_subtype, ...base];
    }
    return base;
  }, [form.qbo_type, form.account_subtype]);
  ...
  // Live preview metadata — static QBO_TYPE_META lookup keyed by the selected fine Account type.
  const previewMeta = form.qbo_type ? QBO_TYPE_META[form.qbo_type] ?? null : null;
  ...
  function validate(): boolean {
    ...
    if (!form.qbo_type || !form.account_type) next.account_type = "Account Type is required.";
    if (!form.account_subtype.trim()) next.account_subtype = "Detail Type is required.";
  ...
  return (
    <>
      ...
      <aside
        ...
        className="fixed right-0 top-0 z-50 flex h-full w-[576px] max-w-[96vw] flex-col border-l border-gray-200 bg-white shadow-xl"
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {/* A2 576px two-column: LEFT = fields, RIGHT = live "Edit account preview".
              docs/specs/qbo-parity/QBO_PARITY_UI_SYSTEM_v2_v3.md A2 CoA panel spec. */}
          <div className="grid grid-cols-1 gap-x-6 gap-y-4 md:grid-cols-[1fr_180px]">
            {/* LEFT — fields */}
            <div className="space-y-4 min-w-0">
              ...
              {/* Account Type — QBO fine-grained type list (static, coa-account-type-map.ts).
                  CASCADES the Detail Type list below. Always resolves to one of the 8 valid
                  catalogs.accounts.account_type enum values on submit (form.account_type). */}
              <FieldLabel label="Account Type" required>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const meta = QBO_TYPE_META[nextType];
                    setForm((prev) => ({
                      ...prev,
                      qbo_type: nextType,
                      account_type: meta ? meta.enumValue : "",
                      account_subtype: "",
              ...
              {/* Detail Type — CASCADES from Account Type (static DETAIL_TYPES[qbo_type] map, no
                  endpoint). Written to catalogs.accounts.account_subtype (free text, unchanged). */}
              <FieldLabel label="Detail Type" required>
                <select
                  value={form.account_subtype}
                  disabled={readOnly || detailOptionsForType.length === 0}
                  onChange={(e) => setField("account_subtype", e.target.value)}
                  className="mt-1 h-9 w-full rounded-sm border border-gray-300 px-2.5 text-sm focus:border-slate-300 focus:outline-hidden focus:ring-1 focus:ring-slate-400 disabled:bg-slate-50 disabled:text-slate-500"
                >
                  <option value="">
                    {detailOptionsForType.length === 0 ? "Select an account type first" : "Select a detail type…"}
                  </option>
                  {detailOptionsForType.map((dt) => (
                    <option key={dt} value={dt}>
                      {dt}
                    </option>
                  ))}
                </select>
                <FieldError msg={errors.account_subtype} />
              </FieldLabel>
                    ...
                    <Combobox
                      ...
                      placeholder={
                        parentOptions.length === 0 && !parentAccountsQuery.isLoading
                          ? `No other ${form.qbo_type || form.account_type} accounts to nest under`
            ...
            {/* RIGHT — live "Edit account preview" (static QBO_TYPE_META, updates as type changes). */}
            <div className="min-w-0">
              <div
                className="sticky top-0 rounded-sm border border-slate-200 bg-slate-50 px-3 py-3"
                data-testid="account-preview-pane"
              >
                <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                  {mode === "create" ? "New account preview" : "Edit account preview"}
                </div>
                {previewMeta ? (
                  <div className="space-y-1.5 text-xs text-slate-700">
                    <div className="font-semibold text-slate-900 break-words" data-testid="preview-name">
                      {(form.account_number ? `${form.account_number} ` : "") +
                        (form.account_name.trim() || "Untitled account")}
                    </div>
                    <div className="flex flex-col gap-0.5">
                      <span className="text-slate-500">Classification</span>
                      <span data-testid="preview-classification">
                        {`${GROUP_DISPLAY_LABELS[previewMeta.group]} › ${form.qbo_type} › ${
                    ...
                    <div className="flex justify-between gap-3">
                      ...
                      <span data-testid="preview-statement">{STATEMENT_LABELS[previewMeta.statement]}</span>
                    </div>
                    <div className="flex justify-between gap-3">
                      <span className="text-slate-500">Normal balance</span>
                      <span data-testid="preview-normal-balance">{previewMeta.normalBalance}</span>
                    </div>
                    <div className="flex justify-between gap-3">
                      <span className="text-slate-500">Opens as</span>
                      <span data-testid="preview-opens-as">{ACTION_LABELS[previewMeta.defaultAction]}</span>
                    </div>
                    <div className="pt-1 text-[11px] text-slate-500" data-testid="preview-description">
                      {`Lands under ${GROUP_DISPLAY_LABELS[previewMeta.group]} on your ${
                        STATEMENT_LABELS[previewMeta.statement]
                      }${form.is_subaccount ? ", nested as a subaccount." : "."}`}
                    </div>
                  </div>
                ) : (
                  <p className="text-xs text-slate-400">Select an account type to preview placement.</p>
                )}
              </div>
            </div>
          </div>
