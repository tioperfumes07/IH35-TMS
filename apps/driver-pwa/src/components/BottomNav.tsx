import { CalendarDays, Clock, DollarSign, FileText, Home, Truck, User } from "lucide-react";
        ...
        {ITEMS.map((item) => {
          const active =
            item.to === "/today"
              ...
              : item.to === "/loads"
                ...
                : item.to === "/hos"
                  ? location.pathname.startsWith("/hos")
                  : item.to === "/documents"
          ...
          return (
            <Link
              key={item.key}
              to={item.to}
              className={`flex min-h-11 flex-col items-center justify-center gap-1 border-t-2 px-0.5 text-[10px] font-semibold ${
