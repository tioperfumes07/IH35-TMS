...
export function DeliveryCard({ stop, onArrived, onDeparted, onUploadDoc, busy }: DeliveryCardProps) {
  const { t } = useTranslation();
  const status = t(`dispatch.${stopStatusKey(stop)}`);

  return (
    <PwaCard
      title={t("dispatch.delivery_title", { sequence: stop.sequence })}
      subtitle={stop.location_name}
      data-testid="dispatch-delivery-card"
    >
