...
function audit(s) {
  ...
  if (!/listPoliciesQuerySchema[\s\S]{0,220}vendor_id: z\.string\(\)\.uuid\(\)\.optional\(\)/.test(s.route) || !/filters\.push\(`p\.vendor_id = \$\$\{values\.length\}::uuid`\)/.test(s.route)) failures.push("exact vendor list filter missing");
...
if (process.argv.includes("--selftest")) {
  const mutations = [
    ...
    ["filter", "route", /filters\.push\(`p\.vendor_id = \$\$\{values\.length\}::uuid`\)/, "void 0"],
