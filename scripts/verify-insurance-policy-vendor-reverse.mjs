...
function audit(s) {
  ...
  if (!/listPoliciesQuerySchema[\s\S]{0,220}vendor_id: z\.string\(\)\.uuid\(\)\.optional\(\)/.test(s.route) || !/filters\.push\(`p\.vendor_id = \$\$\{values\.length\}::text`\)/.test(s.route)) failures.push("exact text-safe vendor list filter missing");
...
if (process.argv.includes("--selftest")) {
  const mutations = [
    ...
    ["filter", "route", /filters\.push\(`p\.vendor_id = \$\$\{values\.length\}::text`\)/, "void 0"],
    ["unsafe_cast", "route", /filters\.push\(`p\.vendor_id = \$\$\{values\.length\}::text`\)/, 'filters.push(`p.vendor_id = $${values.length}::uuid`)'],
