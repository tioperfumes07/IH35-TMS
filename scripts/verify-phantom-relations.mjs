...
const KNOWN_PHANTOM_DEBT = [
  ...
  { rel: "accounting.related_party_loan_entries", why: "LOAN-01 forward-ref — the related-party lending register (owner/spouse/employee/driver, both directions). Ships in migration 202611230000_related_party_loans_data_model.sql in the SAME PR; rehearsed twice on a Neon fork of prod (br-square-star-akcya8z8: FORCE RLS + bypass-branch assertions passed, entity isolation proven as role ih35_app TRANSP 1 / USMCA 0, apply-twice safe) and applies to prod through the deploy pipeline on merge, NOT by hand — doc 02 §1 forbids hand-running DDL on a raw console/MCP connection, and the ih35_app role cannot run DDL anyway. DELETE THIS ENTRY once 202611230000 is applied and canonical-relations.json is regenerated." },
  { rel: "accounting.related_party_loan_schedule", why: "LOAN-01 forward-ref — the amortization rows for the register above (shape mirrors finance.loan_amortization_rows so the system keeps one amortization vocabulary, FK'd to the new entries table because finance.loans cannot hold a loan-out). Same migration, same PR, same rehearsal. DELETE THIS ENTRY once 202611230000 is applied and canonical-relations.json is regenerated." },
