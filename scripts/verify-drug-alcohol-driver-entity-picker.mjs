...
const LABEL="verify-drug-alcohol-driver-entity-picker"; const FILE="apps/frontend/src/pages/safety/tabs/DrugAlcoholTab.tsx";
...
export function collectProblems(root=ROOT){
  ...
  if(!/EntityPicker[\s\S]*?kind=["']driver["']/.test(code)) problems.push("need EntityPicker kind=driver");
  if(/DriverPickerWithCreate/.test(code)) problems.push("no DriverPickerWithCreate");
  return problems;
}
if(process.argv.includes("--selftest")){
  if(collectProblems().length) process.exit(1);
  const stubRoot=fs.mkdtempSync(path.join(ROOT,".tmp-drug-alcohol-driver-entity-picker-"));
