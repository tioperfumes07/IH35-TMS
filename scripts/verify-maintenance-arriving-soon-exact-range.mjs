...
function problems(candidateBackend = backend, candidateApi = api, candidatePage = page) {
  const checks = [
    ...
    [candidateApi.includes("limit?: number") && candidateApi.includes("offset?: number"), "typed API range"],
...
if (process.argv.includes("--selftest")) {
  const mutations = [
    ...
    [backend, api.replaceAll("offset?: number", "offsetValue?: number").replace('qs.set("offset"', 'qs.set("offsetValue"'), page],
