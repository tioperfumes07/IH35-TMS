...
function assertRouteFilters() {
  ...
  const customersPath = path.join(ROOT, "apps/backend/src/accounting/qbo-master-read.routes.ts");
  const usersPath = path.join(ROOT, "apps/backend/src/identity/users.routes.ts");

  for (const target of [driversPath, customersPath, usersPath]) {
    if (!fs.existsSync(target)) fail(`missing ${path.relative(ROOT, target)}`);
  }

  const driversSrc = fs.readFileSync(driversPath, "utf8");
  const customersSrc = fs.readFileSync(customersPath, "utf8");
  ...
  if (!customersSrc.includes("EXCLUDE_ARCHIVED_QBO_CUSTOMERS_SQL")) {
