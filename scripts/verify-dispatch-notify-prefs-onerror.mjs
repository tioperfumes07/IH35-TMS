...
export function audit(src) {
  ...
  if (!/mutationFn:\s*\(submittedCompanyId:\s*string\)\s*=>\s*syncCustomerNotify\(submittedCompanyId\)/.test(src)) failures.push("notification sync must submit an immutable company variable.");
  if (!/onSuccess:\s*\(_data, submittedCompanyId\)[\s\S]{0,180}queryKey:\s*\["customer-notify-log", submittedCompanyId\]/.test(src)) failures.push("notification sync must invalidate only the submitted company scope.");
  if (!/onClick=\{\(\)\s*=>\s*syncM\.mutate\(companyId\)\}/.test(src)) failures.push("notification sync action must pass the selected company explicitly.");
...
if (process.argv.includes("--selftest")) {
  ...
  for (const [name, before, after] of [["scope", "input.customerId", "customerId"], ["sync submitted scope", 'queryKey: ["customer-notify-log", submittedCompanyId]', 'queryKey: ["customer-notify-log", companyId]'], ["sync action scope", "syncM.mutate(companyId)", "syncM.mutate()"], ["picker lock", "enabled={Boolean(companyId) && !saveM.isPending}", "enabled={Boolean(companyId)}"], ["toggle lock", "disabled={saveM.isPending}", "disabled={false}"]]) {
