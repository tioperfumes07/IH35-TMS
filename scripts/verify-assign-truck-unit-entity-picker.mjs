...
if (process.argv.includes("--selftest")) {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "assign-truck-guard-"));
  try {
    const tmpFixturePath = path.join(tmpRoot, FILE);
    fs.mkdirSync(path.dirname(tmpFixturePath), { recursive: true });
    fs.writeFileSync(tmpFixturePath, `listUnits({ limit: 500 })\n<SelectCombobox><option/></SelectCombobox>\n`);
    if (!collectProblems(tmpRoot).length) { console.error("planted miss"); process.exit(1); }
    fs.writeFileSync(
      tmpFixturePath,
      `<EntityPicker kind="unit" allowCreate={false} />\n`
    );
    if (!collectProblems(tmpRoot).some((p) => /allowCreate/.test(p))) {
    ...
    for (const [name, mutation] of [
      ...
      fs.writeFileSync(tmpFixturePath, mutation);
      if (!collectProblems(tmpRoot).length) {
