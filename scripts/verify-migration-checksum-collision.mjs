...
const BASELINE = "scripts/known-migration-ledger-exceptions.json";
...
function check() {
  ...
  // LV-087 second clause — the runner must also reconcile the TWO ledgers.
  //
  // Asserted here as source structure rather than by querying, on purpose: this guard runs in CI where
  // the only database is a fresh ephemeral one whose two ledgers agree trivially, so a live query would
  // pass unconditionally and prove nothing about prod. What CAN be asserted statically is that the
  // refusal EXISTS in the runner, which is where it actually meets the production ledger.
  if (runner && !runnerReconcilesLedgers(runner)) {
    errors.push(
      `${RUNNER}: no reconciliation between ${"_system._schema_migrations"} and ` +
        `${"ih35_migrations.applied_migrations"}. Backend boot accepts a migration present in EITHER ledger, ` +
        `so a one-sided row makes a migration look applied while its DDL may never have run — and the held ` +
        `union (held + applied_held + superseded) is the only thing that legitimately explains one.`
    );
  }
...
/**
 * The runner must compare BOTH ledgers and exempt only (a) held-union members and (b) frozen orphans.
 * Requiring the held-set consultation matters as much as the comparison itself: a reconciliation that
 * did not know about held migrations would flag all six legitimate mirror-only rows and would be turned
 * off within a day.
 */
function runnerReconcilesLedgers(src) {
  return /ledgerDivergence/.test(src) && /knownLedgerOrphans/.test(src) && /HELD_SET\.has/.test(src);
}
...
if (process.argv.includes("--selftest")) {
  ...
  // Mutation 6: a runner with no ledger reconciliation must be rejected.
  if (runnerReconcilesLedgers("async function main() { const a = ledgerByFile; }")) {
    console.error(`${LABEL} --selftest FAIL — a runner with no ledger reconciliation was accepted.`);
    process.exit(1);
  }
  // Mutation 7: a reconciliation that does NOT consult the held union must be rejected. Without it the
  // check fires on the six legitimate mirror-only held rows, and a guard that is red on expected state
  // gets disabled — so this is the difference between a control and a nuisance.
  if (runnerReconcilesLedgers("const ledgerDivergence = []; const knownLedgerOrphans = new Set();")) {
    console.error(
      `${LABEL} --selftest FAIL — a reconciliation ignoring the held union was accepted; it would flag ` +
        `the 6 legitimately mirror-only held migrations as defects.`
    );
    process.exit(1);
  }
  // Mutation 8: the orphan baseline must name a file that is genuinely absent from db/migrations/. If
  // the file came back, the entry is stale and is silently exempting a migration that now exists.
  for (const orphan of real.ledger_orphans ?? []) {
    let present = true;
    try {
      readFileSync(path.join(MIGRATIONS_DIR, orphan.file), "utf8");
    } catch {
      present = false;
    }
    if (present) {
      console.error(
        `${LABEL} --selftest FAIL — ledger_orphans names ${orphan.file}, but that file IS on disk now. ` +
          `The entry is stale and would exempt a real migration from the ledger check.`
      );
      process.exit(1);
    }
  }
  console.log(`${LABEL} --selftest PASS — 8 mutations all detected; baseline matches the files it names.`);
  process.exit(0);
}
