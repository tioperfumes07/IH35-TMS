...
function audit(s) {
  const failures = [];
  if (!/vendor_id:\s*z\.string\(\)\.uuid\(\)\.optional\(\)/.test(s.route) || !/filters\.push\(`pi\.vendor_id = \$\$\{values\.length\}::uuid`\)/.test(s.route)) failures.push("list must expose exact alias-safe vendor_id predicate");
  ...
  if (!/<EntityLink kind="inventory_part" id=\{part\.id\} label=\{entityLabel\(part\.name, part\.id, "Part"\)\}/.test(s.reverse)) failures.push("reverse rows must use canonical exact-part EntityLink");
...
if (process.argv.includes("--selftest")) {
  const mutations = [
    ["filter", "route", /filters\.push\(`pi\.vendor_id = \$\$\{values\.length\}::uuid`\)/, "filters.push(`TRUE`)"] ,
    ...
    ["drill", "reverse", /kind="inventory_part"/, 'kind="task"'],
