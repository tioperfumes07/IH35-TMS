#!/usr/bin/env node
/**
 * verify-customer-compliance-entitylink-unresolved-tombstone.mjs
 * LV-CUSTOMER-COMPLIANCE-ENTITYLINK-UNRESOLVED-TOMBSTONE
 */
import fs from "node:fs";
import path from "node:path";
import process from "node:process";

const LABEL = "verify-customer-compliance-entitylink-unresolved-tombstone";
...
function selftest() {
  ...
  const badConsumer = `<EntityLink label={entityLabel(l.load_number, l.id, "Load")} />`;
...
if (!fs.existsSync(path.join(process.cwd(), HELPER))) fail(`missing helper ${HELPER}`);
