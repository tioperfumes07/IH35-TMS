...
function failures(source) {
  ...
  if (!source.includes("typesQuery.isError ?")) found.push("type-catalog failure is not branched");
  if (!source.includes('title="Couldn\'t load coverage types"')) found.push("type-catalog failure is not visible");
  if (!source.includes("onRetry={() => void typesQuery.refetch()}")) found.push("type-catalog failure lacks exact Retry");
  return found;
}
