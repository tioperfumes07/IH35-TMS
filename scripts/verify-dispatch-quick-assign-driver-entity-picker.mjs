...
const LABEL = "verify-dispatch-quick-assign-driver-entity-picker";
...
export function collectProblems(root = ROOT) {
  ...
  if (!/kind=["']driver["']/.test(code)) problems.push(`${FILE}: must use EntityPicker kind=driver`);
  if (/DriverPickerWithCreate/.test(code)) problems.push(`${FILE}: must not use DriverPickerWithCreate`);
  return problems;
}
if (process.argv.includes("--selftest")) {
  if (collectProblems().length) { console.error("baseline red"); process.exit(1); }
  const stubRoot = fs.mkdtempSync(path.join(ROOT, ".tmp-dispatch-quick-assign-driver-entity-picker-"));
  try {
