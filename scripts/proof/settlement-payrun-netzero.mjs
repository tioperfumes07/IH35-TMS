...
//   escrow  = min(standard, max(0, ESCROW_CAP_CENTS − escrow_balance_before))   [ESCROW_CAP_CENTS = 200000]
...
const ESCROW_CAP_CENTS = 200_000; // I3 LOCKED — matches escrow-resolver.service.ts ESCROW_CAP_CENTS
...
function payRunLegs({ gross, deductions, chargebacks, advance, escrowBalanceBefore, standardEscrow }) {
  ...
  const legs = [
    ...
    { dc: "credit", cents: escrow, memo: "escrow contribution (capped @ $2,000)" },
...
console.log("\n── CASE 2: escrow AT / OVER the $2,000 cap — contribution == 0 ──");
check("CASE2 @cap(200000) contribution == 0", cappedEscrow(200000, 25000) === 0);
check("CASE2 @over(250000) contribution == 0", cappedEscrow(250000, 25000) === 0);
const c2 = payRunLegs({ gross: 300000, deductions: 40000, chargebacks: 10000, advance: 20000, escrowBalanceBefore: 200000, standardEscrow: 25000 });
check("CASE2 at-cap JE still balances", c2.debit === c2.credit);

console.log("\n── CASE 3: partial-headroom cap — contribution clamps to remaining room ──");
check("CASE3 bal=190000 std=25000 -> contribution == 10000", cappedEscrow(190000, 25000) === 10000);
const c3 = payRunLegs({ gross: 300000, deductions: 40000, chargebacks: 10000, advance: 20000, escrowBalanceBefore: 190000, standardEscrow: 25000 });
