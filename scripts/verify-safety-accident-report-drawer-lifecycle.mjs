...
function failures(input = source) {
  ...
  return [
    ...
    ["one safely registered guarded dirty-close boundary", input.includes("confirmDiscardOnClose") && input.includes("isDirty={isDirty}") && input.includes("onRegisterAttemptClose={(next) => setAttemptClose(() => next)}") && input.includes("onClick={attemptClose}")],
