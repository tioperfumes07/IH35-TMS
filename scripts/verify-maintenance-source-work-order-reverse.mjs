...
if(process.argv.includes("--selftest")){const d=defect.replace('kind="work_order" id={defect.follow_up_wo_id}','kind="unit" id={defect.follow_up_wo_id}');const p=preflight.replace('kind="work_order" id={row.work_order_id ?? row.auto_wo_id}','kind="unit" id={row.work_order_id ?? row.auto_wo_id}');const a=alerts.replace('kind="work_order" id={alert.scheduled_work_order_id}','kind="unit" id={alert.scheduled_work_order_id}');const checks=[failures(d,preflight,alerts).includes("defect WO drill"),failures(defect,p,alerts).includes("preflight WO drill"),failures(defect,preflight,a).includes("scheduled WO drill"),failures(defect,preflight,alerts,defectsRoute.replace("wo.display_id AS follow_up_wo_display_id","NULL AS follow_up_wo_display_id"),inbox).includes("defect inbox display join"),failures(defect,preflight,alerts,defectsRoute.replace("AND wo.operating_company_id = dd.operating_company_id",""),inbox).includes("defect inbox display join"),failures(defect,preflight,alerts,defectsRoute,inbox.replace('id={row.follow_up_wo_id}','id={undefined}')).includes("defect inbox WO drill"),failures(defect,preflight,alerts,defectsRoute,inbox.replace("!row.follow_up_wo_id ? (","true ? (")).includes("defect inbox duplicate conversion suppressed")];if(checks.some(x=>!x)){console.error(`verify-maintenance-source-work-order-reverse selftest FAIL — ${checks.map((ok,index)=>ok?null:index+1).filter(Boolean).join(",")}`);process.exit(1);}console.log(`verify-maintenance-source-work-order-reverse selftest PASS — ${checks.length}/${checks.length} mutations red`);process.exit(0);}
