#!/usr/bin/env node
/**
 * verify-finance-loan-wizard-no-box-in-box.mjs
 * Finance Loan Wizard workspace must use a single outer section frame with flat
 * divide-x columns — no nested `rounded-sm border` tiles (Cascade BOX-IN-BOX class).
 *
...
const TARGET = "apps/frontend/src/pages/finance/LoanWizardPage.tsx";
const LABEL = "verify-finance-loan-wizard-no-box-in-box";
...
/** Slice covering the main loan wizard workspace (Loan & asset + preview columns). */
export function workspaceSection(src) {
  const body = stripComments(src);
  const start = body.indexOf("Loan &amp; asset");
  if (start < 0) {
    const alt = body.indexOf("Loan & asset");
    if (alt < 0) return "";
    const sectionStart = body.lastIndexOf("<section", alt);
    const endMarker = body.indexOf("</section>", alt);
    const end = endMarker >= 0 ? endMarker + "</section>".length : body.length;
    return body.slice(sectionStart >= 0 ? sectionStart : alt, end);
  }
...
export function collectProblems(section) {
  const problems = [];
  if (!section.includes("Loan &") && !section.includes("Loan &amp;")) {
    problems.push(`${TARGET}: missing Loan & asset column header`);
    return problems;
  }
  if (!section.includes("Will auto-create")) {
    problems.push(`${TARGET}: missing preview column header`);
  }
  ...
  if (/grid grid-cols-1 gap-6 lg:grid-cols-2/.test(section)) {
  ...
  if (!/bg-\[#1f2a44\]/.test(section)) {
    problems.push(`${TARGET}: primary Preview action must use navy bg-[#1f2a44]`);
  }
  return problems;
}
  ...
  const good = `
    <section className="overflow-hidden rounded-sm border border-slate-200 bg-white">
      <div className="grid grid-cols-1 lg:grid-cols-2 lg:divide-x lg:divide-slate-100">
        <div>
          <div className="border-b border-slate-200 bg-slate-50 px-4 py-3">Loan &amp; asset</div>
          <div className="px-4 py-3">
            <button className="bg-[#1f2a44]">Preview</button>
          </div>
        </div>
        <div>
          <div className="border-b border-slate-200 bg-slate-50 px-4 py-3">Will auto-create (preview)</div>
        </div>
      </div>
    </section>`;
...
if (isMain) {
  ...
  console.log(`[${LABEL}] OK — Loan Wizard workspace is a flat section frame (no box-in-box).`);
}
