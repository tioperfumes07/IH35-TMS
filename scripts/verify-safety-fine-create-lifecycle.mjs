...
function failures(input = source) {
  ...
  return [
    ...
    ["dirty drawer uses one confirmation boundary", input.includes("confirmDiscardOnClose") && input.includes("isDirty={isDirty}") && input.includes("onRegisterAttemptClose={setAttemptClose}") && /onClick=\{attemptClose\} disabled=\{createMutation\.isPending\}/.test(input)],
