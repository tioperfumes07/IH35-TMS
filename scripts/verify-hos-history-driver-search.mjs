#!/usr/bin/env node
/**
 * HosHistorySection — driver picker must server-search (not silent listDrivers limit:500).
 * Accepts either legacy Combobox+onSearch OR EntityPicker kind="driver" (picker law).
 * Cursor even claim: 2132.
 */
...
const LABEL = "verify-hos-history-driver-search";
const FILE = "apps/frontend/src/pages/compliance/HosHistorySection.tsx";
...
if (process.argv.includes("--selftest")) {
  ...
  const stubRoot = fs.mkdtempSync(path.join(ROOT, ".tmp-hos-history-"));
  try {
    const dir = path.join(stubRoot, "apps/frontend/src/pages/compliance");
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(
      path.join(dir, "HosHistorySection.tsx"),
...
`
  ...
  console.log(`${LABEL} OK — HosHistory driver search (EntityPicker or Combobox)`);
}
