#!/usr/bin/env node
/** @matrix-built {"modules":["lists"],"cols":["connectivity"],"leaves":["catalog.dispatch.load_trailer_equipment.list","catalog.dispatch.load_trailer_equipment.create","catalog.dispatch.dispatcher_error_reasons.list","catalog.dispatch.dispatcher_error_reasons.create","catalog.dispatch.lumper_providers.list","catalog.dispatch.lumper_providers.create"],"task":"LISTS-F5959-DISPATCH-GENERIC-CATALOG-CONNECTIVITY-EXACT","vertical":"class-sweep"} */
import fs from "node:fs";

const SELF = "scripts/verify-lists-dispatch-generic-catalog-connectivity-exact.mjs";
...
const CATALOGS = [
  ["load_trailer_equipment", "load-trailer-equipment", "loadTrailerEquipmentCatalogConfig"],
  ["dispatcher_error_reasons", "dispatcher-error-reasons", "dispatchErrorReasonsCatalogConfig"],
  ["lumper_providers", "lumper-providers", "dispatchLumperProvidersCatalogConfig"],
];
...
export function audit(s = {}) {
  ...
  for (const [leafKey, slug, configName] of CATALOGS) {
    ...
    if (!src.hub.includes(`catalogKey: "${slug}"`)) failures.push(`${slug} must remain reachable from the Dispatch hub`);
    const registryStart = src.registry.indexOf(`"dispatch.${leafKey}":`);
    const registryBlock = src.registry.slice(registryStart, registryStart + 1000);
    if (registryStart < 0 || !registryBlock.includes('domain: "dispatch"') || !registryBlock.includes(`catalogKey: "${slug}"`)) failures.push(`${slug} frontend registry entry missing`);
    const configStart = src.backend.indexOf(`export const ${configName}`);
    const config = src.backend.slice(configStart, configStart + 1800);
    if (configStart < 0 || !config.includes('routePrefix: "/api/v1/catalogs/dispatch"') || !config.includes(`urlSegment: "${slug}"`) || !config.includes(`tableName: "${leafKey}"`)) failures.push(`${slug} canonical backend config missing`);
...
if (process.argv.includes("--selftest")) {
  ...
  const mutants = [
    ["matrix", original.matrix.replace('"id": "catalog.dispatch.load_trailer_equipment.list"', '"id": "catalog.dispatch.load_trailer_equipment.list.broken"')],
    ["manifest", original.manifest.replace('path="/lists/:domain/:catalogKey"', 'path="/lists/:domain/:broken"')],
    ["hub", original.hub.replace('catalogKey: "lumper-providers"', 'catalogKey: "lumper-providers-broken"')],
    ["registry", original.registry.replace('catalogKey: "dispatcher-error-reasons"', 'catalogKey: "dispatcher-error-reasons-broken"')],
    ["page", original.page.replace("useCatalogMutations(catalogName ?? \"\", companyId)", "useCatalogMutations(catalogName ?? \"\", \"\")")],
    ["backend", original.backend.replace('urlSegment: "load-trailer-equipment"', 'urlSegment: "load-trailer-equipment-broken"')],
  ];
  ...
  console.log(`verify-lists-dispatch-generic-catalog-connectivity-exact SELFTEST PASS — ${mutants.length + 1} planted defects rejected`);
  process.exit(0);
}

const failures = audit();
if (failures.length) { console.error(`verify-lists-dispatch-generic-catalog-connectivity-exact FAIL\n- ${failures.join("\n- ")}`); process.exit(1); }
console.log("verify-lists-dispatch-generic-catalog-connectivity-exact PASS — 3 Dispatch catalogs × list/create retain hub→stable route→scoped generic read/create/reload→canonical CRUD connectivity");
