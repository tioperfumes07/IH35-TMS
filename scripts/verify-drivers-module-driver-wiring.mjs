...
const CHECKS = [
  ...
  // DRIVER-VISIBILITY (owner order 2026-09-04, Rule 4): status is no longer a hardcoded "All"
  // literal -- it's gated behind an off-by-default "Show inactive" toggle (driverStatusFilter).
  ["apps/frontend/src/pages/drivers/DriversListPage.tsx", /listDrivers\(\{ operating_company_id: companyId, status: driverStatusFilter, search, limit: pageSize, offset: page \* pageSize \}\)/],
  ...
  ["apps/frontend/src/components/drivers/AddTrainingModal.tsx", /await createDriverTrainingRecord\(input\.driverId, input\.companyId, input\.body\)/],
  ["apps/frontend/src/components/drivers/SendMessageModal.tsx", /await sendDriverProfileMessage\(input\.driverId, input\.companyId, input\.body\)/],
