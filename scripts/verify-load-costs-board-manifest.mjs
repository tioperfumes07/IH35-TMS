...
const COLUMN_ORDER = ["load", "status", "pickup-date", "projected-delivery", "delivered", "route-crew", "revenue", "costs", "driver", "margin"];
...
if (process.argv.includes("--selftest")) {
  ...
  const structural = [
    board.replace('sortMode="external"', 'sortMode="internal"'),
    board.replaceAll("actual_delivery_at", "delivered_guess"),
    board.replace('backgroundColor:"#14314F"', 'backgroundColor:"#F7F8FA"'),
  ];
  for (const [index, source] of structural.entries()) {
    try { check(source, backend); }
    catch { caught += 1; continue; }
