...
/**
 * E22 — Faro 8/24/26.
 * inv 21 OSTT PO 119922 load 13537 $3300/adv 3191
 * inv 25 Jericho PO 21148 load 13538 $800/adv 776
...
const LOADS: LoadSpec[] = [
  {
    ...
    wo: "119922",
    at_wo: "119922",
    customer_id: "348907b7-8323-42fd-8138-889238bebdb5", // OSTT LOGISTICS LLC
    ...
    empty_miles: 335.7,
    rate: 0.5,
    pickup: { date: "2026-08-21", city: "Edison", state: "NJ", zip: "08817" },
    delivery: { date: "2026-08-24", city: "Houston", state: "TX", zip: "77001" },
    fuel: [
      { date: "2026-08-22", vendor: "LOVES", location: "10465LONESOME PINE TRAIL M,TN", invoice: "99456225", gallons: 198, actual: 1164.04 },
      { date: "2026-08-24", vendor: "LOVES", location: "21548FM471S NATALIA,TX", invoice: "99529093", gallons: 88.604, actual: 500.52 },
    ],
    expenses: [
      { date: "2026-08-22", vendor: "LOVES", description: "Fuel-DEF-Diesel Exhaust Fluid", invoice: "99456225-DEF", amount: 40.25 },
    ],
    reserve_pct: 1.5,
    factor_fee_pct: (59.5 / 3300) * 100,
    expected_advance_cents: 319100,
  },
  {
    load_number: "13538",
    settlement_no: "5785",
    ...
    wo: "21148",
    at_wo: "21148",
    customer_id: "6249ca59-50e8-42d8-8fc2-d7534098a558", // Jericho freight LLC
    driver_id: "6edcb351-e81b-4bf2-adf7-5eca9eff9137", // GENARO GUERRERO CHAVEZ
    unit_id: "19d29860-9753-4376-93c4-dc963cc86483", // T152
    ...
    empty_miles: 126.9,
    rate: 0.45,
    pickup: { date: "2026-08-21", city: "Fort Pierce", state: "FL", zip: "34945" },
    delivery: { date: "2026-08-24", city: "Forest Park", state: "GA", zip: "30297" },
    fuel: [
      { date: "2026-08-18", vendor: "LOVES", location: "2510HWY 231 COTTONDALE,FL", invoice: "2466361", gallons: 41.071, actual: 217.22 },
      { date: "2026-08-21", vendor: "LOVES", location: "I-75& SOUTHWELL BULEVARD TIFTO,GA", invoice: "99389982", gallons: 155.015, actual: 830.73 },
    ],
    expenses: [
      { date: "2026-08-18", vendor: "LOVES", description: "Fuel-DEF-Diesel Exhaust Fluid", invoice: "2466361-DEF", amount: 26.51 },
      { date: "2026-08-21", vendor: "LOVES", description: "Fuel-DEF-Diesel Exhaust Fluid", invoice: "99389982-DEF", amount: 29.67 },
      { date: "2026-08-21", vendor: "LOVES", description: "Warehouse-Lumper Fee Expense", invoice: "13538-LUMPER", amount: 10 },
    ],
    reserve_pct: 1.5,
    factor_fee_pct: 1.5,
    expected_advance_cents: 77600,
  },
]
...
async function feedOne(
  ...
  if (!loadId) {
    const bookInput: BookLoadInput = {
      ...
      override_reason: `Historical Faro 8/24 feed: load ${LOAD.load_number} (settlement ${LOAD.settlement_no})`,
      override_rules: [
  ...
  if (alreadyAdv) {
    ...
    const createRes = await app.inject({
      ...
      payload: {
        factoring_company_vendor_id: FARO_VENDOR,
        submission_batch_ref: `FARO-824-INV-${LOAD.faro_inv}`,
        invoice_ids: [invoiceId.id],
        reserve_pct: LOAD.reserve_pct,
        factor_fee_pct: LOAD.factor_fee_pct,
        notes: `Faro inv ${LOAD.faro_inv} WO ${LOAD.at_wo} — 08/24/2026`,
      },
    });
    ...
    const advRes = await app.inject({
      ...
      headers: auth,
      payload: { advanced_at: "2026-08-24T18:00:00.000Z", notes: `Wire / Faro 08/24/26 inv ${LOAD.faro_inv}` },
    });
    report.push(
...
async function main() {
  ...
  if (!APPLY) {
    ...
    console.log("Control want: cum purchases $53775 · 20 inv fed (DARDINI15/18+MPH16 STOPPED) · banking 1133");
    return;
  }
  ...
  const report: string[] = [];
  report.push("EXCEPTION carry: DARDINI15/18 + MPH16 STOPPED. Feeding 8/24 OSTT+Jericho.");
  try {
    for (const L of LOADS) {
