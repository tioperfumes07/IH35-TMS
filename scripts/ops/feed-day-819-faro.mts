#!/usr/bin/env tsx
/**
 * E22 — Faro 8/19/26. inv 17 SAJACKS PO 9020844 load 13528 $3100/adv 3007.
 * inv 19 J RAYL PO 251839 load 13526 $3500/adv 3385.
 * 8/18 MPH PO MPHC261334 STOPPED (W.O. unmatched). 8/17 DARDINI STOPPED.
 */
...
const LOADS: LoadSpec[] = [
  {
    ...
    wo: "9020844",
    at_wo: "9020844",
    customer_id: "5a1cc76b-b397-4e17-9964-42ec5d45aeec",
    driver_id: "45fac397-860e-4fe8-ae18-67e12e1959c1",
    unit_id: "033dcdff-98c7-4b2e-8db3-2c94519dbc89",
    trailer_id: "15a0fc9a-8882-43ff-99f1-a68f446f7149",
    ...
    empty_miles: 121.6,
    rate: 0.43,
    pickup: { date: "2026-08-18", city: "Harrison", state: "OH", zip: "45030" },
    delivery: { date: "2026-08-19", city: "Cockeysville", state: "MD", zip: "21030" },
    fuel: [
      { date: "2026-08-17", vendor: "LOVES", location: "313PORTER RD", invoice: "99558645", gallons: 143.381, actual: 758.34 },
      { date: "2026-08-18", vendor: "LOVES", location: "66320GALMONT", invoice: "99415804", gallons: 67.37, actual: 390.0 },
      { date: "2026-08-19", vendor: "LOVES", location: "22 OLD FORGE ROAD", invoice: "99193164", gallons: 127.621, actual: 797.5 },
    ],
    expenses: [],
    reserve_pct: 1.5,
    factor_fee_pct: 1.5,
  ...
  },
  {
    ...
    wo: "251839",
    at_wo: "251839",
    customer_id: "2326f780-fa78-491d-8a4c-4e76ab49b86a",
    driver_id: "26870c49-c37b-4ae0-8729-eccbe58643ee",
    ...
    empty_miles: 487.9,
    rate: 0.45,
    pickup: { date: "2026-08-18", city: "Uhrichsville", state: "OH", zip: "44683" },
    delivery: { date: "2026-08-19", city: "Mesquite", state: "TX", zip: "75149" },
    fuel: [
      { date: "2026-08-18", vendor: "LOVES", location: "203NHAUGHTON HWY", invoice: "99264345", gallons: 134.954, actual: 762.36 },
      { date: "2026-08-19", vendor: "LOVES", location: "215EAST I-30NSVC RD", invoice: "99291854", gallons: 117.384, actual: 656.06 },
    ],
    expenses: [],
    reserve_pct: 1.5,
    factor_fee_pct: ((52.5+10)/3500)*100,
    expected_advance_cents: 338500,
  },
];
...
async function feedOne(
  ...
  if (!loadId) {
    const bookInput: BookLoadInput = {
      ...
      override_reason: `Historical Faro 8/19 feed: load ${LOAD.load_number} (settlement ${LOAD.settlement_no})`,
      override_rules: [
  ...
  if (alreadyAdv) {
    ...
    const createRes = await app.inject({
      ...
      payload: {
        factoring_company_vendor_id: FARO_VENDOR,
        submission_batch_ref: `FARO-819-INV-${LOAD.faro_inv}`,
        invoice_ids: [invoiceId.id],
        reserve_pct: LOAD.reserve_pct,
        factor_fee_pct: LOAD.factor_fee_pct,
        notes: `Faro inv ${LOAD.faro_inv} WO ${LOAD.at_wo} — 08/19/2026`,
      },
    });
    ...
    const advRes = await app.inject({
      ...
      headers: auth,
      payload: { advanced_at: "2026-08-19T18:00:00.000Z", notes: `Wire / Faro 08/19/26 inv ${LOAD.faro_inv}` },
    });
    report.push(
...
async function main() {
  ...
  if (!APPLY) {
    ...
    console.log("Control want: cum purchases $36675 · 14 inv · banking 1133 (DARDINI15+MPH16 STOPPED)");
    return;
  }
  ...
  const report: string[] = [];
  report.push("EXCEPTION carry: inv15 DARDINI 154100 STOPPED; inv16 MPH MPHC261334 STOPPED (W.O. unmatched). Feeding 8/19.");
  try {
    for (const L of LOADS) {
