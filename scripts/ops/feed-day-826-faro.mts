...
/**
 * E22 — Faro 8/26/26.
 * inv 28 Hawkeye PO 66006 load 13544 $600/adv 582
 * inv 27 PFL PO LD88719 load 13543 $2500/adv 2415
...
const LOADS: LoadSpec[] = [
  {
    ...
    wo: "66006",
    at_wo: "66006",
    customer_id: "ba40f2bf-6033-41fc-8078-841c34c15029", // Hawkeye Transportation Services
    driver_id: "c864a4bb-a7ff-4373-a5e1-c1590eefe3b7", // Rafael Rogelio Rivero Reynoso
    unit_id: "ea1b0fe4-1731-49ca-a50a-3363dfc76ae4", // T148
    trailer_id: "8813cde2-399c-4c4f-aa6d-127651a9bbbf", // 10876
    ...
    empty_miles: 0,
    rate: 0,
    pickup: { date: "2026-08-24", city: "Laredo", state: "TX", zip: "78045" },
    delivery: { date: "2026-08-26", city: "Laredo", state: "TX", zip: "78045" },
    fuel: [],
    expenses: [],
    reserve_pct: 1.5,
    factor_fee_pct: 1.5,
    expected_advance_cents: 58200,
  },
  {
    load_number: "13543",
    settlement_no: "5785",
    ...
    wo: "LD88719",
    at_wo: "LD88719",
    customer_id: "1d1f8b21-3ea6-423e-8ced-af0c2b6a21fb", // PFL Logistics LLC (Faro)
    ...
    empty_miles: 439.8,
    rate: 0.45,
    pickup: { date: "2026-08-24", city: "Albertville", state: "AL", zip: "35950" },
    delivery: { date: "2026-08-26", city: "Temple", state: "TX", zip: "76504" },
    fuel: [
      { date: "2026-08-25", vendor: "LOVES", location: "6138LAKE NORRIS ROAD LAKE MS", invoice: "99554605", gallons: 157.962, actual: 892.33 },
      { date: "2026-08-26", vendor: "LOVES", location: "21548FM471S NATALIA,TX", invoice: "1848853", gallons: 105.489, actual: 585.36 },
    ],
    expenses: [
      { date: "2026-08-25", vendor: "LOVES", description: "Fuel-DEF-Diesel Exhaust Fluid", invoice: "99554605-DEF", amount: 29.39 },
      { date: "2026-08-26", vendor: "LOVES", description: "Fuel-DEF-Diesel Exhaust Fluid", invoice: "1848853-DEF", amount: 29.43 },
    ],
    reserve_pct: 1.5,
    factor_fee_pct: (47.5 / 2500) * 100,
    expected_advance_cents: 241500,
  },
]
...
async function feedOne(
  ...
  if (!loadId) {
    const bookInput: BookLoadInput = {
      ...
      override_reason: `Historical Faro 8/26 feed: load ${LOAD.load_number} (settlement ${LOAD.settlement_no})`,
      override_rules: [
  ...
  if (alreadyAdv) {
    ...
    const createRes = await app.inject({
      ...
      payload: {
        factoring_company_vendor_id: FARO_VENDOR,
        submission_batch_ref: `FARO-826-INV-${LOAD.faro_inv}`,
        invoice_ids: [invoiceId.id],
        reserve_pct: LOAD.reserve_pct,
        factor_fee_pct: LOAD.factor_fee_pct,
        notes: `Faro inv ${LOAD.faro_inv} WO ${LOAD.at_wo} — 08/26/2026`,
      },
    });
    ...
    const advRes = await app.inject({
      ...
      headers: auth,
      payload: { advanced_at: "2026-08-26T18:00:00.000Z", notes: `Wire / Faro 08/26/26 inv ${LOAD.faro_inv}` },
    });
    report.push(
  ...
  // LAW 2 — faro_purchase_date + faro_invoice_number mandatory (COALESCE, set once).
  await withCurrentUser(OWNER, async (c) => {
    await setScopedCompanyContext(c, OWNER, USMCA);
    const u = await c.query<{ display_id: string; faro_purchase_date: string; faro_invoice_number: string }>(
      ...
      [invoiceId.id, LOAD.faro_inv, "2026-08-26", USMCA]
    );
...
async function main() {
  ...
  if (!APPLY) {
    ...
    console.log("Control want: cum purchases $56875 · 22 inv fed (DARDINI15/18+MPH16 STOPPED) · banking 1133");
    return;
  }
  ...
  const report: string[] = [];
  report.push("EXCEPTION carry: DARDINI15/18 + MPH16 STOPPED. Feeding 8/26 Hawkeye+PFL.");
  try {
    for (const L of LOADS) {
