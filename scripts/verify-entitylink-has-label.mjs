 ...
 *
 * There are no exceptions: every frontend module, route, tab, leaf, drawer, modal, and creator is
 * scanned. A new unlabelled instance fails immediately instead of entering a debt ratchet.
 *
...
export function collectProblems(files) {
  ...
  for (const { rel, src } of files) {
    for (const tag of entityLinkTags(strip(src))) {
      ...
      problems.push(
        `${rel}: <EntityLink> has an id and no label. EntityLink prints the raw id when it has no ` +
          `label, so this shows a uuid to a human (FAIL-CP1). Pass a human label resolved by an ` +
          `entity-scoped backend join.`
      );
    }
  }
  return problems;
