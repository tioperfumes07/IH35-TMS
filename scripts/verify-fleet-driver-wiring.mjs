...
const CHECKS = [
  ...
  ["apps/frontend/src/pages/fleet/VehicleProfilePage.tsx", /target=\{\{ equipmentKind: "truck", equipmentId: id, equipmentLabel: unitNumber \}\}[\s\S]{0,420}const input = \{ unitId: id, companyId, driverId, generation: actionGenerationRef\.current \}[\s\S]{0,180}quicksaveEquipmentAssignment\(\{[\s\S]{0,180}equipment_id: input\.unitId,[\s\S]{0,80}driver_id: input\.driverId/],
  ["apps/frontend/src/pages/fleet/VehicleProfilePage.tsx", /invalidateQueries\(\{ queryKey: \["unit-profile", input\.unitId, input\.companyId\] \}\)/],
