#!/usr/bin/env node
/**
 * verify-dispatch-load-detail-entitylink-unresolved-tombstone.mjs
 * LV-DISPATCH-LOAD-DETAIL-ENTITYLINK-UNRESOLVED-TOMBSTONE
 *
 * Load detail drawer / settlement / WO reverse / fines / factoring / pre-settlement
 * must not pass entityLabel(...) into EntityLink label=.
 */
import fs from "node:fs";
import path from "node:path";
import process from "node:process";

const LABEL = "verify-dispatch-load-detail-entitylink-unresolved-tombstone";
...
const CONSUMERS = [
  ...
  "apps/frontend/src/components/dispatch/LoadDetailSettlementTab.tsx",
