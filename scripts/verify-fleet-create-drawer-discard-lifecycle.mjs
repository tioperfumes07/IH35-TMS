...
function failures(input = files) {
  ...
  for (const [kind, source] of Object.entries(input)) {
    ...
    if (!/<Button type="button" variant="secondary" onClick=\{\(\) => attemptCloseRef\.current\(\)\}>/.test(source)) missing.push(`${kind} Cancel bypasses guarded close`);
...
if (process.argv.includes("--selftest")) {
  ...
  if (escaped.length) {
    console.error(`verify-fleet-create-drawer-discard-lifecycle SELFTEST FAIL — ${escaped.length}/4 mutations escaped`);
    process.exit(1);
  }
  console.log("verify-fleet-create-drawer-discard-lifecycle selftest PASS — 4/4 planted silent-discard defects rejected");
  process.exit(0);
}
