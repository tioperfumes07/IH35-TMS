#!/usr/bin/env tsx
/**
 * ROUND 152.1 / Rule 52 — Faro purchase day 8/18/26 LIVE feed for missing inv 16.
 * Faro inv 16 · MPH · PO MPHC261334 · load 13524 · settl 5778 · $3,800 / net adv $3,676
 * (cash_rsv $57 + discount $57 + wire $10). Faro face $3800 (settlement linehaul $4200 —
 * invoice Faro bought is $3800 per purchase report / Round 151.5).
 *
...
const LOAD = {
  ...
  wo: "MPHC261334",
  at_wo: "MPHC261334",
  customer_id: "a760ee40-ad82-4bb5-9120-4d0766b46297", // MPH CARRIER SERVICES, INC
  driver_id: "3445cf68-4a7f-4d73-89f7-04bf1fd207b4", // HUGO GAYTAN Active
  unit_id: "82db522d-9efe-4dca-958f-bb931e4a55ca", // T173
  linehaul: 3800, // Faro purchase face (not settlement 4200)
  driver_gross: 853.61,
  loaded_miles: 1185.4,
  empty_miles: 711.5,
  rate: 0.45,
  pickup: {
    ...
    state: "IA",
    zip: "50588",
    facility: "TYSON FRESH STORM LAKE 054",
  },
  delivery: {
    date: "2026-08-17",
    city: "Laredo",
    state: "TX",
    zip: "78041",
    facility: "Grupo Comercio Mundial Inc / Servicios Aduaneros del Norte",
  },
  rest: {
    date: "2026-08-10",
    city: "ERLANGER",
  ...
  fuel: [
    { date: "2026-08-11", vendor: "LOVES", location: "21548FM471S NATALIA,TX", invoice: "99517553", gallons: 60.207, actual: 314.82, fuel_type: "diesel" },
    { date: "2026-08-12", vendor: "LOVES", location: "2005HWY 19NORTH", invoice: "99491524", gallons: 198.0, actual: 1059.1, fuel_type: "diesel" },
    { date: "2026-08-13", vendor: "LOVES", location: "400L&N TURNPIKE HORSE", invoice: "99418954", gallons: 104.829, actual: 518.8, fuel_type: "diesel" },
    { date: "2026-08-14", vendor: "LOVES", location: "3301 GREYHOUND DR", invoice: "99694065", gallons: 129.699, actual: 698.95, fuel_type: "diesel" },
    { date: "2026-08-15", vendor: "LOVES", location: "I-35/SR22EXIT 368", invoice: "99366495", gallons: 175.929, actual: 965.67, fuel_type: "diesel" },
    { date: "2026-08-12", vendor: "LOVES", location: "DEF", invoice: "DEF-13524-1", gallons: 0, actual: 20.2, fuel_type: "def" },
    { date: "2026-08-13", vendor: "LOVES", location: "DEF", invoice: "DEF-13524-2", gallons: 0, actual: 32.98, fuel_type: "def" },
    { date: "2026-08-14", vendor: "LOVES", location: "DEF", invoice: "DEF-13524-3", gallons: 0, actual: 17.21, fuel_type: "def" },
    { date: "2026-08-15", vendor: "LOVES", location: "DEF", invoice: "DEF-13524-4", gallons: 0, actual: 30.17, fuel_type: "def" },
  ],
  expenses: [
    { date: "2026-08-14", vendor: "LOVES", description: "Warehouse-Lumper Fee Expense", invoice: "LUMP-13524", amount: 18.0 },
    { date: "2026-08-15", vendor: "LOVES", description: "Driver Reimbursement-TPE-Scale Expense", invoice: "SCALE-13524", amount: 15.25 },
  ],
  reserve_pct: 1.5, // cash_rsv $57 / $3800
  factor_fee_pct: 1.7632,
  expected_purchase_cents: 380000,
  expected_reserve_cents: 5700, // Cash Rsv $57
  expected_fee_cents: 6700, // Discount $57 + Wire $10
  expected_ach_cents: 0,
  expected_advance_cents: 367600, // Net Adv $3,676
...
async function main() {
  ...
  if (!APPLY) {
    console.log(
      `DRY 8/18 Faro inv ${LOAD.faro_inv} WO ${LOAD.at_wo} load ${LOAD.load_number} settl ${LOAD.settlement_no} $${LOAD.linehaul} → adv $${(LOAD.expected_advance_cents / 100).toFixed(2)} fuel=${LOAD.fuel.length} exp=${LOAD.expenses.length}`
    );
    return;
  }
  ...
  try {
    ...
    if (existing) {
      ...
      const bookInput: BookLoadInput = {
        ...
        override_reason: `Historical Faro 8/18 feed: load ${LOAD.load_number} already completed (settlement ${LOAD.settlement_no})`,
        override_rules: [
    ...
    // Rest/empty stop — bump existing seq to +100 first to avoid unique collision, then renumber 1/2/3.
    await withCurrentUser(OWNER, async (c) => {
      await setScopedCompanyContext(c, OWNER, USMCA);
    ...
    const createRes = await app.inject({
      ...
      payload: {
        ...
        submission_batch_ref: `FARO-818-INV-${LOAD.faro_inv}`,
        invoice_ids: [invoiceId.id],
        reserve_pct: LOAD.reserve_pct,
        factor_fee_pct: LOAD.factor_fee_pct,
        notes: `Wire / Faro 08/18/26 inv ${LOAD.faro_inv}`,
      },
    });
    ...
    await withCurrentUser(OWNER, async (c) => {
      ...
      await c.query(
        ...
        [created.id, `${LOAD.purchase_date}T18:00:00.000Z`, `Wire / Faro 08/18/26 inv ${LOAD.faro_inv}`, USMCA]
      );
      await c.query(
    ...
    report.push(`DONE 8/18 inv ${LOAD.faro_inv} load ${LOAD.load_number} — full composition via app writers`);
  } finally {
    await app.close();
    await pool.end();
  }
