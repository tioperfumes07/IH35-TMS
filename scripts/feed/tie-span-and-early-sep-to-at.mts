#!/usr/bin/env tsx
/**
 * Tie SPAN 5789–5794 + early-Sep 5797–5803 + paperwork 5812 to AlwaysTrack total_due.
 *
 * Pure-Aug 5769–5788/5795/5796 and Sep 5804–5815 already tied. Owner: August 100% + Sep closing.
 *
 ...
 *
 * Usage:
 *   E11_LEAD_AUTH=1 npx tsx scripts/feed/tie-span-early-sep-to-at.mts
 *   E11_LEAD_AUTH=1 npx tsx scripts/feed/tie-span-early-sep-to-at.mts --apply
 *   ... --only 5769,5775
 */
...
const REASON = "ACCT-F20260925 tie SPAN+early-Sep 5789-5803/5812 to AlwaysTrack total_due / settlement_control";

/** SPAN + early Sep + 5812 paperwork (TOTAL DUE -50). */
const DOCS = [
  ...Array.from({ length: 6 }, (_, i) => String(5789 + i)), // 5789-5794 SPAN
  ...Array.from({ length: 7 }, (_, i) => String(5797 + i)), // 5797-5803
  "5812",
];
...
async function main() {
  ...
  const report: string[] = [];
  report.push(`tie-span-early-sep apply=${APPLY} targets=${targets.join(",")}`);
  ...
  for (const doc of targets) {
    ...
    // 1-cent tolerance: AT PDFs print xx.x5 with banker's rounding vs our cent math (5792: 1386.04 vs 1386.05).
    const ok = Math.abs(gotNet - expect) < 0.015;
    report.push(
    ...
    // Stamp signed AT total when within 1¢ so DRIVER_NET dimension matches the PDF exactly.
    if (Math.abs(gotNet - expect) >= 0.005 && Math.abs(gotNet - expect) < 0.015) {
      await withCurrentUser(REPOST_ACTOR, async (c) => {
        await setScopedCompanyContext(c, REPOST_ACTOR, USMCA);
        await c.query(
          `UPDATE driver_finance.driver_settlements
              SET net_pay = $2::numeric, updated_at = now()
            WHERE id = $1::uuid AND operating_company_id = $3::uuid`,
          [row.id, money(expect), USMCA]
        );
      });
      report.push(`  stamped net_pay to AT ${money(expect)} (1¢ PDF round)`);
    }
  }
