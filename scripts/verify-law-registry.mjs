 ...
 * that would have made the answer citable was never built.
 *
 ...
 * are other checks' jobs (verify-guard-wired, the guards themselves). This one is designed to cost
 * ~30ms so it can be a REQUIRED check on every PR without adding measurable PR time — per the law's
 ...
 *
 * REGISTERING A NEW LAW: append an entry to docs/law/LAW.json. If the rule is mechanically checkable,
 * ship its guard in the SAME PR and set type='enforced'. If it is a judgment rule ("professional",
 * "McLeod-quality"), set type='judgment' and guard=null — judgment rules are not force-guarded, but
 * they ARE registered, so the set of live law is enumerable in one file.
 */
 ...
 * Kept pure and exported so the selftest can exercise it without touching disk, and so any future
 * caller can reuse the rule instead of re-implementing (a second copy is a copy that will disagree).
 */
export function auditRegistry(entries, fileExists = (rel) => existsSync(path.join(ROOT, rel))) {
  ...
  entries.forEach((entry, i) => {
    ...
    if (typeof type !== "string" || !VALID_TYPES.has(type)) {
      problems.push(`${who}: "type" must be one of ${[...VALID_TYPES].join("|")} (got ${JSON.stringify(type)}).`);
    }
...
function run() {
  ...
  console.log(
    `${LABEL} OK — ${entries.length} law(s) registered: ${enforced} enforced (every guard file resolves on disk), ` +
      `${judgment} judgment.`,
  );
  return 0;
}
 ...
 * against the REAL registry file. An in-memory-only selftest would not prove the on-disk existence
 * check is the thing that reddens, which is the entire contract here.
 *
...
function selftest() {
  ...
  try {
    ...
    if (planted.code === 0) {
      failures.push("case2 FAIL — a registered enforced law with a NON-EXISTENT guard file did not go RED.");
  ...
  const after = readFileSync(REGISTRY_ABS, "utf8");
  if (after !== original) failures.push("restore FAIL — registry not byte-identical after selftest.");
  ...
  console.log(
    `${LABEL}: selftest PASS — GREEN(0) on the real registry, RED(1) on a planted missing guard naming ` +
      `(id, guard), GREEN(0) again after restore; judgment-with-guard, duplicate id and enforced-with-null-guard ` +
      `all RED. Registry restored byte-identical.`,
  );
  process.exit(0);
}
