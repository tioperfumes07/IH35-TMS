...
function failures(input = source) {
  ...
  if (!/confirmDiscardOnClose[\s\S]*?isDirty=\{outcome !== "warning" \|\| Boolean\(resolutionNotes\.trim\(\) \|\| fineOverrideCents\.trim\(\)\) \|\| correctiveActionDirty\}[\s\S]*?onRegisterAttemptClose=\{setAttemptClose\}/.test(input.company)) out.push("company resolution and corrective drafts lack one discard boundary");
  ...
  if (!/confirmDiscardOnClose[\s\S]*?isDirty=\{Boolean\(note\.trim\(\)\)\}[\s\S]*?onRegisterAttemptClose=\{setAttemptClose\}/.test(input.anomaly)) out.push("anomaly note lacks one discard boundary");
