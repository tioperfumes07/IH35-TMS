...
function failures(candidate) {
  ...
  if (!/listDrivers\(\{ operating_company_id: operatingCompanyId, status: "Active", limit: ROSTER_LIMIT \}\)/.test(candidate)) found.push("pre-create roster is not explicitly company scoped");
  ...
  if (!/hos_safe:\s*false/.test(candidate)) found.push("pre-create roster invents HOS clearance");
...
if (process.argv.includes("--selftest") || process.argv.includes("--self-test")) {
  const mutations = [
    ...
    ['listDrivers({ operating_company_id: operatingCompanyId, status: "Active", limit: ROSTER_LIMIT })', 'listDrivers({ status: "Active", limit: ROSTER_LIMIT })', "company scope"],
