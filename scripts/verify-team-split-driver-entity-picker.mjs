#!/usr/bin/env node
/** TeamSplitConfig — EntityPicker kind=driver. Cursor even claim: 2470. */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const LABEL = "verify-team-split-driver-entity-picker";
const FILE = "apps/frontend/src/pages/drivers/TeamSplitConfig.tsx";
...
if (process.argv.includes("--selftest")) {
  ...
  const stubRoot = fs.mkdtempSync(path.join(ROOT, ".tmp-team-split-driver-entity-picker-"));
  try {
