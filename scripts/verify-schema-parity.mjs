...
function serialise(schemaMap) {
  ...
  return {
    // NO volatile timestamp here on purpose. A `generated_at: new Date()` made every branch's
    // post-merge `--update` rewrite this one line, so every open PR conflicted with main on that
    // single line the instant any PR merged → GitHub-DIRTY → rebase → full CI re-run → re-dirty on the
    // next merge (the "hours per PR" conflict treadmill). The baseline is now byte-deterministic:
    // `tables` is sorted, so a no-op regenerate produces an identical file, non-migration PRs never
    // touch it, and they never re-conflict. verify() only ever reads `.tables`, never a timestamp.
    note: "Auto-generated by scripts/verify-schema-parity.mjs --update. Do not hand-edit. Deterministic (no timestamp) so it never causes spurious merge conflicts.",
    tables,
  };
}
