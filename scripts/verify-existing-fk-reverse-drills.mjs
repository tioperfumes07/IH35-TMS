...
if(process.argv.includes("--selftest")){const inv=inventory.replace('kind="work_order"','kind="work_order_broken"');const invLabel=inventory.replace("name={row.work_order_display_id}","name={undefined}");const task=tasks.replace('kind="task"','kind="load"');const a=failures(inv,tasks),b=failures(inventory,task),c=failures(invLabel,tasks);if(!a.includes("inventory work-order FK")||!b.includes("task activity drill")||!c.includes("inventory human label"))process.exit(1);console.log("verify-existing-fk-reverse-drills selftest PASS — 3/3 mutations red");process.exit(0);}
