...
export function collectFailures({ service = load(SERVICE_PATH), fromLoad = load(FROM_LOAD_PATH) } = {}) {
  ...
  if (/driver_finance\./.test(serviceCode)) {
    failures.push("broker-advances.service.ts references driver_finance.* in actual code -- a broker advance must never touch a driver liability/advance table");
  }
...
if (process.argv.includes("--selftest")) {
  ...
  for (const [name, from, to] of mutations) {
    ...
    const planted = service.replace(from, to);
...
console.log("verify-broker-advance-never-driver-liability-never-invoice-face: OK — a broker advance never touches driver_finance.*, never touches the invoice face, and applies additively into broker_advance_applied_cents at both receipt time and invoice-mint time");
