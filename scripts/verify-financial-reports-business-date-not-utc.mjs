 ...
 * elsewhere. Five financial-report call sites were found still using the raw UTC pattern as an "as
 * of today" default (ar-aging, ap-aging, cash-flow-overview, the cash-flow route-fix, and the
 * weekly AR-aging-60 scheduled generator) — all fixed. This guard fails if any file under
 * apps/backend/src/reports/ reintroduces the raw pattern.
 *
 * Scoped to apps/backend/src/reports/ deliberately: the pattern is legitimate elsewhere when a true
 * UTC instant is intended (timestamps, logging, test fixtures), and a repo-wide ban would be wrong
 * (the source finding itself found ~54 other repo-wide hits, most non-cutoff, explicitly deferred
 * for individual classification — not banned outright).
 */
...
const DIR = "apps/backend/src/reports";
...
console.log(`${LABEL} OK — ${files.length} report file(s) under ${DIR}, no raw-UTC "today" default.`);
process.exit(0);
