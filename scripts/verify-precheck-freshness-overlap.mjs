...
// ARM 3b — TWO GUARD PRs. Both add a verify step and both touch the append-only CLAIMED-NUMBERS
// registry, but they claim DIFFERENT numbers (Claude odd / Cursor even). This must PASS. Coupling the
// whole scripts/verify-steps/ directory made every guard PR block every other guard PR, which is the
// N^2 treadmill relocated rather than removed — and nearly every quality PR here adds a guard.
const twoGuards = freshnessVerdict({
  behind: 1,
  mainFiles: ["scripts/verify-steps/1806-verify-module-completion.mjs", "scripts/verify-steps/CLAIMED-NUMBERS.json"],
  branchFiles: ["scripts/verify-steps/1795-verify-precheck-freshness-overlap.mjs", "scripts/verify-steps/CLAIMED-NUMBERS.json"],
});
check("two guard PRs claiming different step numbers => allowed", twoGuards.ok === true, JSON.stringify(twoGuards.reason));

// ARM 3c — the real collision the registry exists to prevent: both sides claim the SAME number.
// Must FAIL, or the banded-namespace discipline is unenforced.
const sameNumber = freshnessVerdict({
  behind: 1,
  mainFiles: ["scripts/verify-steps/1795-verify-something-else.mjs", "scripts/verify-steps/CLAIMED-NUMBERS.json"],
  branchFiles: ["scripts/verify-steps/1795-verify-precheck-freshness-overlap.mjs", "scripts/verify-steps/CLAIMED-NUMBERS.json"],
});
