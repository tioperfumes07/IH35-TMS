...
const CHECKS = [
  ...
  { name: "load/driver/vendor scoped history bypasses open-only but keeps void filter", file: F.route, pattern: /else if \(q\.equipment_id \|\| q\.load_id \|\| q\.driver_id \|\| q\.vendor_id\) \{[\s\S]{0,220}where\.push\("w\.voided_at IS NULL"\)/ },
  ...
  { name: "load reverse failure retries exact query", file: F.loadSection, pattern: /<ListErrorState[\s\S]{0,180}onRetry=\{\(\) => void query\.refetch\(\)\}/ },
  { name: "load reverse hides cached rows on failure", file: F.loadSection, pattern: /const rows = query\.isError \? \[\] : \(query\.data\?\.work_orders \?\? \[\]\)/ },
