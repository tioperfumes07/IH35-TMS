...
const files = {
  ...
  integrityDrawer: read("apps/frontend/src/pages/safety/components/IntegrityAlertDetailDrawer.tsx"),
...
function failures(s = files) { return [
  ...
  ["integrity evaluator refreshes the canonical inbox", s.integrityPage.includes("evaluateIntegrityAlerts(operatingCompanyId)") && s.integrityPage.includes('invalidateQueries({ queryKey: ["safety", "integrity-alerts", operatingCompanyId]')],
  ["integrity alert rows open the real selected-record drawer", s.integrityPage.includes('onClick={() => setSelected(row)}') && s.integrityPage.includes("open={Boolean(selected)}") && s.integrityPage.includes("alert={selected}")],
  ["integrity alert drawer renders the canonical record payload", s.integrityDrawer.includes("IntegrityAlertDetailDrawer") && s.integrityDrawer.includes("alert.detection_summary") && s.integrityDrawer.includes("alert.detection_metric") && s.integrityDrawer.includes("alert.source_view") && s.integrityDrawer.includes("alert.created_at")],
  ["integrity alert drawer links related loads and work orders", s.integrityDrawer.includes("alert.related_load_ids") && s.integrityDrawer.includes("alert.related_wo_ids") && s.integrityDrawer.includes('kind="load"') && s.integrityDrawer.includes('kind="work_order"')],
  ...
  ["exact three record targets", s.section.includes('<EntityLinkOrTombstone') && s.section.includes('kind="company_violation"') && s.section.includes('kind="integrity_alert"') && s.section.includes('id={row.id == null ? null : String(row.id)}') && s.section.includes('noun="Company violation"') && s.section.includes('noun="Integrity alert"') && s.section.includes('kind="integrity_anomaly"') && s.entityLink.includes('case "company_violation":') && s.entityLink.includes("/safety/external-fines?record_type=company-violation&violation_id=") && s.entityLink.includes("/safety/integrity-alerts?alert_id=") && s.entityLink.includes("/safety/integrity-reports?anomaly_id=")],
...
if (process.argv.includes("--selftest")) {
  const checks = [
    ...
    failures({...files, integrityPage: files.integrityPage.replace('onClick={() => setSelected(row)}', 'onClick={() => undefined}')}).includes("integrity alert rows open the real selected-record drawer"),
    failures({...files, integrityDrawer: files.integrityDrawer.replace("alert.detection_summary", '"Unavailable"')}).includes("integrity alert drawer renders the canonical record payload"),
    failures({...files, integrityDrawer: files.integrityDrawer.replace('kind="work_order"', 'kind="load"')}).includes("integrity alert drawer links related loads and work orders"),
    ...
    failures({...files, section: files.section.replace('noun="Company violation"', 'noun="Record"')}).includes("exact three record targets"),
