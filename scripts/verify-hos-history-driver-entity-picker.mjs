#!/usr/bin/env node
/**
 * HosHistorySection driver must use EntityPicker (kind=driver),
 * not Combobox over listDrivers. Cursor even claim: 2406.
 */
...
const LABEL = "verify-hos-history-driver-entity-picker";
const TARGET = "apps/frontend/src/pages/compliance/HosHistorySection.tsx";
const SELFTEST = process.argv.includes("--selftest");

export function collectProblems(src) {
  const problems = [];
  const code = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/.*$/gm, "");
  if (!/data-testid=["']hos-history-driver-picker["']/.test(src)) {
    problems.push(`${TARGET}: missing data-testid=hos-history-driver-picker`);
  }
...
if (SELFTEST) {
  ...
  `;
  const good = `
    <div data-testid="hos-history-driver-picker">
...
console.log(`${LABEL} OK — HOS History driver uses EntityPicker`);
