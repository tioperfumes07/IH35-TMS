...
function check(sources) {
  ...
  const expects = [
    ...
    ["kanban", /function cardSecondaryLoadNumber\([^)]*\)[^{]*\{[\s\S]*?return load\.assigned_unit_id \? entityLabel\(load\.load_number/, "Kanban secondary load survives a missing unit label"],
...
if (process.argv.includes("--self-test")) {
  const mutations = [
    ...
    ["kanban", "return load.assigned_unit_id ? entityLabel(load.load_number", "return load.assigned_unit_number ? entityLabel(load.load_number"],
