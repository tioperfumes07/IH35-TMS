...
const SELF = path.join(ROOT, "scripts/verify-account-register-print-letter.mjs");
...
function assertSource() {
  ...
  if (!page.includes("printLetterHtml")) fail("AccountRegisterPage must use printLetterHtml");
  if (/const printList = \(\) => window\.print\(\)/.test(page)) fail("must not window.print() on SPA");
...
function selftest() {
  ...
  // Plant ONLY printList body — never replaceAll helper names (corrupts imports).
