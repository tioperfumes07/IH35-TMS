#!/usr/bin/env node
/**
 * verify-maintenance-warranty-parts-business-dates.mjs (MAINT-MONEY-F6956)
 *
 ...
 *
 * FAIL if either file regresses to the raw-UTC pattern.
 */
...
if (process.argv.includes("--selftest")) {
  ...
  const checks = [
    ["fully-fixed shape produces zero failures", check({ warranty: goodWarranty, parts: goodParts }).length === 0],
    ["one remaining raw-UTC site in warranty.routes.ts is caught", check({ warranty: regressedWarrantyOneSite, parts: goodParts }).some((f) => f.includes(WARRANTY_FILE) && f.includes("occurrence"))],
    ["missing companyBusinessDate import in warranty.routes.ts is caught", check({ warranty: regressedWarrantyNoImport, parts: goodParts }).some((f) => f.includes("no longer imports"))],
    ["one remaining raw-UTC site in parts-inventory.routes.ts is caught", check({ warranty: goodWarranty, parts: regressedPartsOneSite }).some((f) => f.includes(PARTS_FILE) && f.includes("occurrence"))],
...
if (isMain) {
  ...
  console.log(`${LABEL} PASS — maintenance warranty/parts business dates and GL posting dates use companyBusinessDate(), not raw UTC`);
}
