...
const source = fs.readFileSync("apps/frontend/src/components/safety/BackgroundChecksSection.tsx", "utf8");
...
function inspect(value) {
  ...
  const checks = [
    ...
    [/createSafetyBackgroundCheck\(input\.companyId/, "create uses mutable company"],
    ...
    [/queryKey: \["safety", "background-checks", input\.companyId\]/, "wrong company cache can refresh"],
    ...
    [/onSuccess:[\s\S]*companyGenerationRef\.current \+= 1[\s\S]*setSelectedDriverId\(driverId \?\? ""\)[\s\S]*setCheckType\("mvr"\)[\s\S]*setResult\("pass"\)[\s\S]*setCheckedAt\(companyToday\(\)\)[\s\S]*setExpiryDate\(""\)[\s\S]*setNotes\(""\)/, "success does not retire the request and fully reset the draft"],
    [/const closeCreate = \(\) => \{[\s\S]*if \(createMutation\.isPending\) return;[\s\S]*setOpen\(false\)/, "pending create can be dismissed"],
    [/const isCreateDirty =[\s\S]*selectedDriverId !== \(driverId \?\? ""\)[\s\S]*checkType !== "mvr"[\s\S]*result !== "pass"[\s\S]*checkedAt !== companyToday\(\)[\s\S]*Boolean\(expiryDate\)[\s\S]*Boolean\(notes\.trim\(\)\)/, "dirty predicate does not cover every editable field"],
...
if (process.argv.includes("--selftest")) {
  const mutations = [
    ...
    '["safety", "background-checks", input.companyId]',
  ...
  console.log(`verify-background-check-create-company-lifecycle --selftest PASS (${mutations.length}/${mutations.length})`);
} else {
  ...
  console.log("verify-background-check-create-company-lifecycle PASS — create is company-stable with canonical driver linkage");
}
