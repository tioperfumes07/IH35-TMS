...
const commands = [
  ["node", ["scripts/verify-payment-detail-mutation-onerror.mjs", "--selftest"], null],
  ["node", ["scripts/verify-payment-detail-mutation-onerror.mjs"], null],
];

export async function runPaymentDetailMutationOnerrorStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.runAllowFailure(command, args);
    if (status !== 0) {
      throw new Error(`940-payment-detail-mutation-onerror step failed: ${command} ${args.join(" ")}`);
    }
  }
}

const step = {
  name: "940-verify-payment-detail-mutation-onerror",
  run: runPaymentDetailMutationOnerrorStep,
};
...
async function selftest() {
  ...
  console.log("940 verify-step --selftest PASS — guard selftest + live scan executed, planted failure propagated");
}
