...
const commands = [
  ["node", ["scripts/verify-sec-audit-deps-includes-root.mjs", "--selftest"]],
  ["node", ["scripts/verify-sec-audit-deps-includes-root.mjs"]],
];

export async function runSecAuditDepsIncludesRootStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.runAllowFailure(command, args);
    if (status !== 0) throw new Error(`sec-audit-deps-includes-root step failed: ${command} ${args.join(" ")}`);
  }
}

const step = {
  name: "sec-audit-deps-includes-root-contract-and-selftest",
  run: runSecAuditDepsIncludesRootStep,
};
...
async function selftest() {
  ...
  console.log("949 verify-step --selftest PASS");
}
