...
const commands = [
  ["node", ["scripts/verify-settlement-lines-line-type-check-superset.mjs", "--selftest"]],
  ["node", ["scripts/verify-settlement-lines-line-type-check-superset.mjs"]],
];

export async function runSettlementLinesLineTypeCheckSupersetStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.runAllowFailure(command, args);
    if (status !== 0) {
      throw new Error(
        `settlement-lines-line-type-check-superset step failed: ${command} ${args.join(" ")}`,
      );
    }
  }
}
...
const step = {
  name: "settlement-lines-line-type-check-superset-contract-and-selftest",
  run: runSettlementLinesLineTypeCheckSupersetStep,
};
