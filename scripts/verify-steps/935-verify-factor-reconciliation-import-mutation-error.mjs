...
const commands = [
  ["node", ["scripts/verify-factor-reconciliation-import-mutation-error.mjs", "--selftest"], null],
  ["node", ["scripts/verify-factor-reconciliation-import-mutation-error.mjs"], null],
];

export async function runFactorReconciliationImportMutationErrorStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.run(command, args);
    if (status !== 0) {
      throw new Error(`935-factor-reconciliation-import-mutation-error step failed: ${command} ${args.join(" ")}`);
    }
  }
}

const step = {
  name: "935-verify-factor-reconciliation-import-mutation-error",
  run: runFactorReconciliationImportMutationErrorStep,
};
...
async function selftest() {
  ...
  console.log("935 verify-step --selftest PASS — guard selftest + live scan executed, planted failure propagated");
}
