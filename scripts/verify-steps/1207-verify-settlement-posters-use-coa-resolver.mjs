...
const commands = [
  ["node", ["scripts/verify-settlement-posters-use-coa-resolver.mjs", "--selftest"]],
  ["node", ["scripts/verify-settlement-posters-use-coa-resolver.mjs"]],
];

export async function runSettlementPostersUseCoaResolverStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.runAllowFailure(command, args);
    if (status !== 0) throw new Error(`settlement-posters-use-coa-resolver step failed: ${command} ${args.join(" ")}`);
  }
}

const step = {
  name: "settlement-posters-use-coa-resolver-contract-and-selftest",
  run: runSettlementPostersUseCoaResolverStep,
};
...
async function selftest() {
  ...
  console.log("1207 verify-step --selftest PASS");
}
