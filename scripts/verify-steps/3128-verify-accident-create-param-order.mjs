...
const SCRIPT = path.join(HERE, "..", "verify-accident-create-param-order.mjs");

export default {
  name: "verify-accident-create-param-order",
  async run(ctx) {
    const self = spawnSync(process.execPath, [SCRIPT, "--selftest"], { encoding: "utf8" });
    if (self.status !== 0) {
      throw new Error(
        "verify-accident-create-param-order SELFTEST FAIL:\n" +
