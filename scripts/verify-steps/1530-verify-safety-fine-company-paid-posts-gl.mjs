...
//   7. a linkage ledger row ties fine -> expense JE so drill-through exists both ways;
...
export default {
  ...
  run: async () => {
    ...
    if (poster) {
      ...
      // ── 7b. the poster actually WRITES the linkage ledger ────────────────────────────────────────
      if (!/accounting\.civil_fine_postings/.test(poster)) {
        failures.push(
          `${POSTER}: never writes accounting.civil_fine_postings — the posted JE would not be linked ` +
            `back to the fine, breaking forward/reverse drill-through and the idempotency latch.`
        );
      }
    ...
    // ── 7. fine <-> JE linkage ledger shipped in a migration ────────────────────────────────────────
    ...
    if (fs.existsSync(migDir)) {
      for (const f of fs.readdirSync(migDir).filter((n) => n.endsWith(".sql"))) {
        ...
        if (/accounting\.civil_fine_postings/.test(sql) && /expense_je_id/.test(sql)) linkageFound = true;
        if (sql.includes(FLAG_KEY) && /\bfalse\b/i.test(sql)) flagSeededOff = true;
      }
    }
    if (!linkageFound) {
      failures.push(
        `${MIGRATIONS_DIR}: no migration creates accounting.civil_fine_postings with expense_je_id — without ` +
          `it a posted fine has no forward/reverse drill-through to its journal entry, and the poster has ` +
          `no idempotency latch.`
      );
    }
    if (!flagSeededOff) {
      failures.push(
