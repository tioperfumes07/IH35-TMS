...
const commands = [
  ["node", ["scripts/verify-accounting-audit-trail-mutation-onerror.mjs", "--selftest"], null],
  ["node", ["scripts/verify-accounting-audit-trail-mutation-onerror.mjs"], null],
];

export async function runAccountingAuditTrailMutationOnerrorStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.runAllowFailure(command, args);
    if (status !== 0) {
      throw new Error(`944-accounting-audit-trail-mutation-onerror step failed: ${command} ${args.join(" ")}`);
    }
  }
}

const step = {
  name: "944-verify-accounting-audit-trail-mutation-onerror",
  run: runAccountingAuditTrailMutationOnerrorStep,
};
...
async function selftest() {
  ...
  console.log("944 verify-step --selftest PASS — guard selftest + live scan executed, planted failure propagated");
}
