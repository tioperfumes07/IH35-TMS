...
const mod = await import(pathToFileURL(path.join(ROOT, "scripts", "verify-sql-column-existence.mjs")).href);
if (typeof mod.main === "function") await mod.main();
  ...
  if (errs.length) {
    console.error(`verify-sql-column-existence FAIL:\n` + errs.map((e) => `  ✗ ${e}`).join("\n"));
    process.exit(1);
  }
  console.log(`verify-sql-column-existence OK`);
} else {
  // fall back: spawn by re-exec pattern — import side effects may run main
  console.log(`verify-sql-column-existence step loaded`);
}
