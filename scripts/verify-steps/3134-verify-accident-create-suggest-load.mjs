...
const SCRIPT = path.join(HERE, "..", "verify-accident-create-suggest-load.mjs");

export default {
  name: "verify-accident-create-suggest-load",
  async run(ctx) {
    const self = spawnSync(process.execPath, [SCRIPT, "--selftest"], { encoding: "utf8" });
    if (self.status !== 0) {
      throw new Error(
        "verify-accident-create-suggest-load SELFTEST FAIL:\n" +
