...
const commands = [
  ["node", ["scripts/verify-qbo-drift-resolve-mutation-onerror.mjs", "--selftest"], null],
  ["node", ["scripts/verify-qbo-drift-resolve-mutation-onerror.mjs"], null],
];

export async function runQboDriftResolveMutationOnerrorStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.runAllowFailure(command, args);
    if (status !== 0) {
      throw new Error(`938-qbo-drift-resolve-mutation-onerror step failed: ${command} ${args.join(" ")}`);
    }
  }
}

const step = {
  name: "938-verify-qbo-drift-resolve-mutation-onerror",
  run: runQboDriftResolveMutationOnerrorStep,
};
...
async function selftest() {
  ...
  console.log("938 verify-step --selftest PASS — guard selftest + live scan executed, planted failure propagated");
}
