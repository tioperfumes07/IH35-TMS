...
const isDirect = process.argv[1] && path.resolve(process.argv[1]).endsWith("1536-verify-global-catalog-write-authz.mjs");
if (isDirect) main();
...
export default {
  ...
  async run(ctx) {
    ...
    ctx.run("node", ["scripts/verify-steps/1536-verify-global-catalog-write-authz.mjs", "--selftest"]);
    return ctx.run("node", ["scripts/verify-steps/1536-verify-global-catalog-write-authz.mjs"]);
  },
};
