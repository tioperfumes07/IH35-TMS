...
const commands = [
  ["node", ["scripts/verify-coa-role-values-include-settlement-roles.mjs", "--selftest"]],
  ["node", ["scripts/verify-coa-role-values-include-settlement-roles.mjs"]],
];

export async function runCoaRoleValuesIncludeSettlementRolesStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.runAllowFailure(command, args);
    if (status !== 0) throw new Error(`coa-role-values-include-settlement-roles step failed: ${command} ${args.join(" ")}`);
  }
}

const step = {
  name: "coa-role-values-include-settlement-roles-contract-and-selftest",
  run: runCoaRoleValuesIncludeSettlementRolesStep,
};
...
async function selftest() {
  ...
  console.log("1208 verify-step --selftest PASS");
}
