...
const commands = [
  ["node", ["scripts/verify-invoice-detail-mutation-error.mjs", "--selftest"], null],
  ["node", ["scripts/verify-invoice-detail-mutation-error.mjs"], null],
];

export async function runInvoiceDetailMutationErrorStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.runAllowFailure(command, args);
    if (status !== 0) {
      throw new Error(`937-invoice-detail-mutation-error step failed: ${command} ${args.join(" ")}`);
    }
  }
}

const step = {
  name: "937-verify-invoice-detail-mutation-error",
  run: runInvoiceDetailMutationErrorStep,
};
...
async function selftest() {
  ...
  console.log("937 verify-step --selftest PASS — guard selftest + live scan executed, planted failure propagated");
}
