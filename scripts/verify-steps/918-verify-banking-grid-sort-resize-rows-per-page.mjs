...
const commands = [
  ["node", ["scripts/verify-banking-grid-sort-resize-rows-per-page.mjs", "--selftest"], null],
  ["node", ["scripts/verify-banking-grid-sort-resize-rows-per-page.mjs"], null],
];

export async function runBankingGridSortResizeRowsPerPageStep(ctx) {
  for (const [command, args] of commands) {
    const status = ctx.runAllowFailure(command, args);
    if (status !== 0) {
      throw new Error(`918-banking-grid-sort-resize-rows-per-page step failed: ${command} ${args.join(" ")}`);
    }
  }
}

const step = {
  name: "918-banking-grid-sort-resize-rows-per-page",
  run: runBankingGridSortResizeRowsPerPageStep,
};
...
async function selftest() {
  ...
  console.log("918 verify-step --selftest PASS — guard selftest + live scan executed, planted failure propagated");
}
