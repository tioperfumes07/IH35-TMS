...
function failures(source = live) {
  return [
    ["backend exact total", source.route.includes("COUNT(*) OVER()::int AS total_count") && source.route.includes("issues: res.rows, total_count: Number(res.rows[0]?.total_count ?? 0)")],
    ...
    ["home forwards table total", source.home.includes("<InTransitIssuesTable") && source.home.includes("totalCount={triageQuery.data?.total_count")],
...
if (process.argv.includes("--selftest")) {
  const mutations = [
    ...
    { ...live, home: live.home.replaceAll("totalCount={triageQuery.data?.total_count", "totalCount={triageQuery.data?.issues.length") },
  ...
  console.log("verify-maintenance-triage-range SELFTEST PASS — 6/6 mutations red");
  process.exit(0);
}
