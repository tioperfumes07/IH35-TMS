...
const SAMSARA_POSITIONS = "apps/backend/src/integrations/samsara/samsara-positions.service.ts";
...
function audit(driversSrc, ruleExists, activeSetSrc, querySrc, routesSrc, unitsSrc, positionsSrc, ingestionSrc, cronSources) {
  const f = [];
  ...
  if (!/INSERT INTO integrations\.samsara_drivers[\s\S]*ON CONFLICT \(operating_company_id, samsara_driver_id\) DO UPDATE[\s\S]*last_seen_at = GREATEST\(/.test(positionsSrc))
    f.push(`${SAMSARA_POSITIONS}: observed current drivers must upsert entity-scoped monotonic last_seen_at`);
  if (!/pairCurrentDriver\(client, operatingCompanyId, unitId, stat\.id, stat\.current_driver, stat\.captured_at\)/.test(positionsSrc))
    f.push(`${SAMSARA_POSITIONS}: freshness must use the physical position capture time`);
...
function main() {
  ...
  const positionsSrc = readFileSync(SAMSARA_POSITIONS, "utf8");
  const ingestionSrc = readFileSync(INGESTION_TENANTS, "utf8");
  const cronSources = INGESTION_CRONS.map((file) => readFileSync(file, "utf8"));
  const failures = audit(driversSrc, ruleExists, activeSetSrc, querySrc, routesSrc, unitsSrc, positionsSrc, ingestionSrc, cronSources);
  if (failures.length) {
  ...
  if (selftest) {
    ...
    if (audit(m1, ruleExists, activeSetSrc, querySrc, routesSrc, unitsSrc, positionsSrc, ingestionSrc, cronSources).length === 0) { console.error("SELFTEST FAIL: dropping is_sample_data filter did not trip"); process.exit(1); }
    const m2 = driversSrc.replaceAll(`filters.push("deactivated_at IS NULL")`, `filters.push("1=1")`);
    if (audit(m2, ruleExists, activeSetSrc, querySrc, routesSrc, unitsSrc, positionsSrc, ingestionSrc, cronSources).length === 0) { console.error("SELFTEST FAIL: dropping deactivated filter did not trip"); process.exit(1); }
    if (audit(driversSrc, false, activeSetSrc, querySrc, routesSrc, unitsSrc, positionsSrc, ingestionSrc, cronSources).length === 0) { console.error("SELFTEST FAIL: missing rule doc did not trip"); process.exit(1); }
    const mutations = [
    ...
    for (const mutation of mutations) {
      if (audit(driversSrc, ruleExists, mutation, querySrc, routesSrc, unitsSrc, positionsSrc, ingestionSrc, cronSources).length === 0) {
    ...
    if (audit(listMutations[0], ruleExists, activeSetSrc, querySrc, routesSrc, unitsSrc, positionsSrc, ingestionSrc, cronSources).length === 0) {
      console.error("SELFTEST FAIL: planted active-driver list mutation escaped"); process.exit(1);
    }
    if (audit(driversSrc, ruleExists, activeSetSrc, querySrc, routesSrc, listMutations[1], positionsSrc, ingestionSrc, cronSources).length === 0) {
      console.error("SELFTEST FAIL: planted active-unit list mutation escaped"); process.exit(1);
    }
    const freshnessMutations = [
      positionsSrc.replace("last_seen_at = GREATEST(", "last_seen_at = EXCLUDED.last_seen_at + (") ,
      positionsSrc.replace("stat.current_driver, stat.captured_at", "stat.current_driver, new Date().toISOString()"),
    ];
    for (const mutation of freshnessMutations) {
      if (audit(driversSrc, ruleExists, activeSetSrc, querySrc, routesSrc, unitsSrc, mutation, ingestionSrc, cronSources).length === 0) {
        console.error("SELFTEST FAIL: planted observed-driver freshness mutation escaped");
        process.exit(1);
      }
    }
    const ingestionMutation = ingestionSrc.replace("usmca_cfg.is_enabled = true", "usmca_cfg.is_enabled = false");
    if (audit(driversSrc, ruleExists, activeSetSrc, querySrc, routesSrc, unitsSrc, positionsSrc, ingestionMutation, cronSources).length === 0) {
