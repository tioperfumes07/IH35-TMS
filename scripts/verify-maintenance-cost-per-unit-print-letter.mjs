...
const SELF = path.join(ROOT, "scripts/verify-maintenance-cost-per-unit-print-letter.mjs");
const PAGE = path.join(ROOT, "apps/frontend/src/pages/reports/MaintenanceCostPerUnitPage.tsx");
const HELPER = path.join(ROOT, "apps/frontend/src/lib/openPrintableDocument.ts");

function fail(msg) {
  console.error(`FAIL verify-maintenance-cost-per-unit-print-letter: ${msg}`);
  process.exit(1);
}

function assertSource() {
  if (!fs.existsSync(PAGE)) fail("missing MaintenanceCostPerUnitPage");
...
if (process.argv.includes("--selftest")) selftest();
  ...
  assertSource();
  console.log("PASS: verify-maintenance-cost-per-unit-print-letter");
}
