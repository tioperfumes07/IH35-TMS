...
const BASELINE = new Set([
  ...
  "apps/backend/src/accounting/posting-engine.service.ts::accounting.journal_entries",
...
/** Paths whose writes are imports of real books, never samples. */
export function isImportPath(file) {
  return /qbo-sync\/|qbo-.*-puller|\/seed\/|csv-seed-import/.test(file);
}
...
export function untaggedInserts(src, tables = SAMPLE_TAGGED_TABLES) {
  ...
  for (const table of tables) {
    ...
    while ((m = re.exec(clean)) !== null) {
      if (!/\bis_sample_data\b/i.test(m[1])) out.push(table);
    }
  }
  return out;
}
