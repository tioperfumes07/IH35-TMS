...
for (const [label, step] of [["invoice pickup sort", invoiceStep], ["QBO sync staleness", qboStep]]) {
  ...
  test(`${label} wrapper throws when the planted-failure selftest returns nonzero`, () => {
    let invocation = 0;
    assert.throws(
      () => step.run({ run: () => (++invocation === 1 ? 0 : 1) }),
      /selftest failed/
    );
  });
  test(`${label} wrapper runs primary and selftest on success`, () => {
    const calls = [];
    step.run({ run(command, args) { calls.push([command, args]); return 0; } });
