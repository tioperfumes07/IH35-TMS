...
const SELF = path.join(ROOT, "scripts/verify-management-report-package-print-letter.mjs");
const PAGE = path.join(ROOT, "apps/frontend/src/pages/reports/ManagementReportPackagePage.tsx");
const HELPER = path.join(ROOT, "apps/frontend/src/lib/openPrintableDocument.ts");

function fail(msg) {
  console.error(`FAIL verify-management-report-package-print-letter: ${msg}`);
  process.exit(1);
}

function assertSource() {
  if (!fs.existsSync(PAGE)) fail("missing ManagementReportPackagePage");
  ...
  if (!page.includes("printLetterHtml")) fail("ManagementReportPackagePage must use printLetterHtml");
...
function selftest() {
  ...
  console.log("PASS: verify-management-report-package-print-letter --selftest");
}

if (process.argv.includes("--selftest")) selftest();
else {
  assertSource();
  console.log("PASS: verify-management-report-package-print-letter");
}
