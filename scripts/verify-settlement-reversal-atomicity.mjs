...
const UNIT_TEST =
  "apps/backend/src/accounting/settlement-posting/__tests__/settlement-bill-payment-reversal.test.ts";
...
function inspect(service, poster, bills, governance, dbTest, unitTest) {
  ...
  for (const token of [
    ...
    if (!service.includes(token))
      violations.push(`settlement reversal missing invariant: ${token}`);
  }
  ...
  for (const token of [
    ...
    if (!poster.includes(token))
      violations.push(`canonical posting reversal missing invariant: ${token}`);
  }
  if (!/resolveReversalDate\s*\(\s*originalDate/.test(poster)) {
    violations.push(
      "canonical posting reversal missing invariant: resolveReversalDate(originalDate)",
    );
  }
  ...
  for (const token of [
    ...
    if (!bills.includes(token))
      violations.push(`canonical bill unwind missing invariant: ${token}`);
  }
  for (const token of [
    "companyBusinessDate()",
    "reverseSettlementBillPaymentInClientTx",
    ...
    if (!governance.includes(token))
      violations.push(
        `outer settlement cancellation missing invariant: ${token}`,
      );
  }
  ...
  for (const token of [
    ...
    if (!dbTest.includes(token))
      violations.push(`DB behavior proof missing: ${token}`);
  }
  if (!/["']nothing_to_reverse["'][\s\S]{0,100}["']reversed["']/.test(dbTest)) {
    violations.push(
      'DB behavior proof missing: "nothing_to_reverse", "reversed"',
    );
  }
  for (const token of [
    "PERIOD_LOCKED",
    ...
    if (!unitTest.includes(token))
      violations.push(`focused behavior proof missing: ${token}`);
  }
  return violations;
}
...
function selftest() {
  ...
  const goodPoster = `
    ...
    await ensureOpenPeriod(client
    export async function reversePostedSourceTransactionInClientTx`;
  const goodBills = `export async function voidBillPaymentInClientTx export async function voidBillInClientTx
    reversePostedSourceTransactionInClientTx UPDATE banking.bank_accounts SET paid_cents = $2 revoked_at = now()`;
  ...
  const goodDb = `Promise.all([ "nothing_to_reverse", "reversed" absolute_residual_cents resolveReversalJournalEntryIds
    revoked_at IS NULL current_balance_cents row.status === "open"`;
  ...
  if (planted.length < 10) {
    console.error(
      `[${LABEL}] --selftest FAILED: planted partial-success workflow was not fully rejected`,
      planted,
    );
    process.exit(1);
  }
  console.log(
    `[${LABEL}] --selftest PASS (planted violations=${planted.length}; good=0)`,
  );
}
...
console.log(
  `[${LABEL}] OK — settlement reversal is atomic, fail-loud, idempotent, date-coherent, and reconciled.`,
);
