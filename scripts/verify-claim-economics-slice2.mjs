...
export function computeFailures(sources) {
  ...
  // 4) ClaimCreateModal — trailer via EntityPicker kind=trailer (mdata.equipment FK + inline create);
  // owner-locked ask-default; deductible via MoneyInput (not a raw number input — QBO-parity money entry).
  if (!/EntityPicker[\s\S]*?kind=["']trailer["']/.test(claimCreate)) {
    errors.push("ClaimCreateModal.tsx: trailer field must use EntityPicker kind=trailer (mdata.equipment FK)");
  }
  if (!/nestedInDrawer/.test(claimCreate.split('kind="trailer"')[1]?.split("EntityPicker")[0] ?? claimCreate)) {
    errors.push("ClaimCreateModal.tsx: trailer EntityPicker must set nestedInDrawer inside ParityDrawer shell");
  }
  if (/<select[\s\S]{0,200}?value=\{form\.trailer_id\}/.test(claimCreate)) {
    errors.push("ClaimCreateModal.tsx: trailer field regressed to a bare <select> — must use EntityPicker kind=trailer");
  }
...
<label data-testid="claim-create-trailer-field">
  <EntityPicker kind="trailer" nestedInDrawer value={form.trailer_id} />
</label>
