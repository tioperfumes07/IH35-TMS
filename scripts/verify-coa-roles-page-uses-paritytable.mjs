 ...
 * per-row account input + datalist and the Save button (upsertCoaRole mutation) keep their
...
function assertMigrated(src) {
  ...
  if (!src.includes("coa-role-account-options-")) {
    errors.push(`${PAGE}: must keep the per-row account datalist options`);
  }
  return errors;
}
  ...
  const good = `
    ...
    const columns = [
      ...
      { key: "account_id", label: "Account", render: (row) => (
        <input list={\`coa-role-account-options-\${row.role}\`} /> ) },
