...
if (process.argv.includes("--selftest")) {
  const good = 'mutation.mutateAsync({ kind: "idea", body }); void Promise.resolve(onSubmit(body)).then(() => setValue(""), () => undefined);';
  const bad = 'mutation.mutate({ kind: "idea", body });\n            onSubmit(body);\n            setValue("");';
  const goodOk = collectNoteSaveProblems(good).length === 0;
  const badHits = collectNoteSaveProblems(bad);
  if (!goodOk || badHits.length < 2) {
