...
const block = source.match(/app\.post\("\/api\/v1\/maintenance\/warranty\/claims\/:id\/archive"[\s\S]*?\n  \}\);/)?.[0] ?? "";
const checks = [
  ...
  ["honest not-found response", /if \(!archived\) return reply\.code\(404\)/],
];
function failures(text) { return checks.flatMap(([label, pattern]) => pattern.test(text) ? [] : [label]); }
const problems = failures(block);
if (problems.length) { console.error(`verify-warranty-claim-archive-failure-truth FAILED:\n${problems.map((p) => ` - ${p}`).join("\n")}`); process.exit(1); }
if (process.argv.includes("--selftest")) {
  const mutations = [
    ...
    ["if (!archived) return reply.code(404)", "if (!archived) return reply.code(200)"],
  ];
  ...
  console.log(`verify-warranty-claim-archive-failure-truth --selftest PASS (${mutations.length}/${mutations.length} planted defects red)`); process.exit(0);
}
console.log("verify-warranty-claim-archive-failure-truth PASS — archive success and audit require one active company claim row");
