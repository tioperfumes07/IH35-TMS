...
const CHAIN_REQUIRED = [
  ...
  },
  {
    ...
    needles: [
      ...
      "mdata.units",
      "u.id = w.unit_id",
      "u.owner_company_id = w.operating_company_id",
      "mdata.loads",
      "l.id = w.load_id",
      "l.operating_company_id = w.operating_company_id",
      "mdata.vendors",
      "v.id = w.vendor_id",
      "v.operating_company_id = w.operating_company_id",
  ...
  },
  {
    ...
    needles: [
      ...
      "p.effective_date <= c.accident_date",
      "p.expiry_date >= c.accident_date",
      "c.amount_paid_cents > 0",
      "accounting.expenses",
      "e.id = c.expense_id",
      "accounting.bills",
      "b.id = c.bill_id",
      "COALESCE(e.total_amount_cents, b.amount_cents, NULLIF(c.amount_claimed_cents, 0)) > 0",
      ...
      "SUM(rp.amount_cents) = LEAST(",
      "bad_rp.claim_id = c.id",
      "balance.debit_or_credit = 'debit'",
      "balance.debit_or_credit = 'credit'",
    ],
  },
...
function selftest() {
  ...
  const mutations = [
    ...
    ["accident probe loses its JE posting line", (s) => ({
      ...s,
      [BACKEND]: s[BACKEND].replace(
        /(key: "scenario\.accident"[\s\S]*?)AND jep\.source_transaction_id = b\.id::text/,
        "$1",
      ),
    })],
