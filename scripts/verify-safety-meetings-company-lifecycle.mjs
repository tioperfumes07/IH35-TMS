...
function inspect(source) {
  ...
  if (!["lifecycleGenerationRef.current += 1", "createMutation.reset()", 'setCreateOpen(false)', 'setTopic("")', "setMeetingDate(companyToday())", "setRequiredAttendees([])", "setAttendeePick(null)"].every((token) => closeCreateBody.includes(token)) || !source.includes('<Modal variant="drawer" open={createOpen} onClose={closeCreate}')) errors.push("meeting dismiss does not retire generation and reset complete creator state");
...
if (process.argv.includes("--selftest")) {
  ...
  if (missed.length) {
    console.error(`verify-safety-meetings-company-lifecycle SELFTEST FAIL — ${missed.length}/8 mutation(s) survived`);
    process.exit(1);
  }
  console.log("verify-safety-meetings-company-lifecycle selftest PASS — 8/8 planted defects rejected");
  process.exit(0);
}
