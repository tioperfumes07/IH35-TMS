...
const hasTriggerWoDetail=(source)=>
  source.includes("row.trigger_wo_id")&&(
    /kind\s*=\s*["']work_order["']/.test(source)||
    source.includes("/maintenance/work-orders/${row.trigger_wo_id}")
  );
const failures=(source=ui)=>[
  ["real estimate API",source.includes("listSevereRepairEstimates(operatingCompanyId")],
  ["trigger WO detail link",hasTriggerWoDetail(source)],
...
if(process.argv.includes("--selftest")){
  const planted=ui.replace(/kind\s*=\s*["']work_order["']/g,'kind="unit"').replace("/maintenance/work-orders/${row.trigger_wo_id}","/maintenance/severe-repairs");
  if(!failures(planted).includes("trigger WO detail link"))process.exit(1);
  console.log("verify-maintenance-severe-repair-connectivity selftest PASS — WO-detail mutation red");
  process.exit(0);
}
