...
const checks = [
  ...
  ["page", /createMaintenanceVendor\(\{[\s\S]{0,120}operating_company_id: companyId[\s\S]{0,800}mdata_vendor_id: draft\.mdata_vendor_id/, "maintenance vendor create submits company plus the selected AP vendor FK"],
  ...
  ["routes", /withCompany\(user\.uuid, parsed\.data\.operating_company_id[\s\S]{0,120}const values: unknown\[\] = \[parsed\.data\.operating_company_id\][\s\S]{0,100}const filters = \["operating_company_id = \$1::uuid"\]/, "reverse list seeds the selected-company predicate"],
  ["routes", /if \(parsed\.data\.mdata_vendor_id\) \{[\s\S]{0,100}values\.push\(parsed\.data\.mdata_vendor_id\)[\s\S]{0,100}filters\.push\(`linked_vendor_id = \$\$\{values\.length\}::uuid`\)/, "reverse list binds the AP vendor FK as a UUID query parameter"],
  ["routes", /FROM catalogs\.maintenance_vendors[\s\S]{0,120}WHERE \$\{filters\.join\(" AND "\)\}[\s\S]{0,180}values/, "reverse list executes the composed company and vendor predicates"],
