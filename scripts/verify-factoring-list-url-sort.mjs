#!/usr/bin/env node
/**
 * verify-factoring-list-url-sort.mjs — BANK-SORT-ROLLOUT-ACCT CI guard
 *
 * Locks: FactoringListPage (/accounting/factoring) — every visible DATA column header is
 * ASC/DESC sortable and sort persists in the URL (?sort=&dir=) via useUrlSort + ParityTable
 * controlled-sort props (same contract as Bills/Expenses).
 *
 * Usage:
 *   node scripts/verify-factoring-list-url-sort.mjs
 *   node scripts/verify-factoring-list-url-sort.mjs --selftest
 */
...
const LABEL = "verify-factoring-list-url-sort";
const PAGE_FILE = "apps/frontend/src/pages/accounting/FactoringListPage.tsx";
...
export function factoringListUrlSortErrors({ hookSrc, pageSrc }) {
  const failures = [];
...
function selftest() {
  ...
  const planted = [
    ...
    ["missing sortable", { ...good, pageSrc: goodPage.replace('sortable: true }, { key: "status"', '}, { key: "status"') }, "missing sortable: true"],
  ];
  const goodErrors = factoringListUrlSortErrors(good);
  const missed = planted.filter(([, fixture, needle]) => !factoringListUrlSortErrors(fixture).some((f) => f.includes(needle)));
...
const failures = factoringListUrlSortErrors({
...
console.log(`${LABEL} — OK (FactoringListPage — every data column sortable + URL-persisted via useUrlSort)`);
