...
function failures(candidate) {
  const route = candidate.slice(candidate.indexOf('/api/v1/maintenance/fault-rules"'));
  const checks = [
    ["creator limiter", /fault-rules"[\s\S]{0,160}rateLimit:\s*\{\s*max:\s*60,\s*timeWindow:\s*"1 minute"/],
