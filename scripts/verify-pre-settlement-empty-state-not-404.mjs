...
export function collectFailures(src = loadSource()) {
  ...
  if (!/if \(!result\) return reply\.send\(\{ settlement: null, lines: \[\] \}\);/.test(src)) {
    failures.push("the by-driver route no longer returns the honest 200 { settlement: null, lines: [] } empty payload for this state");
  }
