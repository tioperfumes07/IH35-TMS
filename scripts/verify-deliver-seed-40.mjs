 ...
 * loads.ts: the 8-load owner hand-list is present and never touched, the real delivered_at
 * evidence is passed through (never invented/defaulted), and both real state-machine transitions
 * (dispatched -> in_transit -> delivered_pending_docs) fire through the real PATCH route — never
 * a raw SQL UPDATE on mdata.loads.status.
 */
...
export function collectProblems(root = ROOT) {
  ...
  if (!/OWNER_HAND_LOADS\.has\(x\.load_number\)/.test(src)) {
...
if (process.argv.includes("--selftest")) {
  ...
  const cases = [
    ...
    { name: "regression: hold list no longer excludes candidates", content: GOOD.replace("!OWNER_HAND_LOADS.has(x.load_number) && ", ""), expectProblems: 1 },
