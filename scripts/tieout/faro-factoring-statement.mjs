...
async function main() {
  ...
  try {
    ...
    const faceRes = await client.query(
      `SELECT COALESCE(SUM(total_cents), 0)::bigint AS face_cents,
    ...
    const { face_cents, ar_cents } = faceRes.rows[0];
    ...
    const { reserve_cents, fee_cents, cash_cents, wire_cents } = advRes.rows[0];
