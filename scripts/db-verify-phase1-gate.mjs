...
try {
  ...
  if (expectedCoverage !== 97) {
    fail(`audit coverage expected list should be 97, got ${expectedCoverage}`);
  }

  console.log(`PASS: Phase 1 Gate verification complete. ${migrationFiles.length} migrations present, all tables present, all catalogs pre-populated, all RLS enabled, audit coverage at 97 events.`);
} catch (error) {
