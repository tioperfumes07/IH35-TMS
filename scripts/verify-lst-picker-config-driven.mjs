...
export function contractErrors(src) {
  ...
  // Owner 2026-09-03: Equipment / load type UI removed from Book Load (duplicates Trailer type).
  // Column mdata.loads.catalog_load_type_id stays for legacy rows; wizard must not select or write it.
  if (/createKind="load_type"/.test(src.bookLoad) || /Equipment\s*\/\s*load type/.test(src.bookLoad)) {
    errors.push("P44-FK: Book Load must not render Equipment/load type (duplicates Trailer type; owner 2026-09-03)");
  }
  if (/catalog_load_type_id:\s*values\.catalog_load_type_id/.test(src.bookLoad)) {
    errors.push("P44-FK: Book Load create payload must not write catalog_load_type_id");
  }
  ...
  if (!/catalog_load_type_id = \$9::uuid/.test(src.bookService) && !/catalog_load_type_id = \$10::uuid/.test(src.bookService)) {
    errors.push("P44-FK: Book Load create service must still accept catalog_load_type_id for legacy null writes");
  }
  ...
  if (!/catalog_load_type_id:\s*str\(load\.catalog_load_type_id\)/.test(src.editMapping)) {
    errors.push("P44-FK: edit hydrate must still read catalog_load_type_id for legacy loads");
  }
  if (/\["catalog_load_type_id", "catalog_load_type_id"/.test(src.editMapping)) {
    errors.push("P44-FK: edit patch must not write catalog_load_type_id after Equipment/load type removal");
  }
...
function selftest() {
  const p44 = {
    ...
    bookLoad: 'detention_reason_id: values.detention_reason_id || undefined, pickup_time_type_id: stop.pickup_time_type_id || undefined, lumper_provider_id: stop.lumper_provider_id, load_trailer_equipment_id: values.load_trailer_equipment_id || undefined, createKind="lumper_provider", const options = rows.map((row) => ({ value: row.id }));',
    ...
    bookService: "detention_expected_y_n, detention_reason_id, detention_expected_hours, time_window_type, pickup_time_type_id, appointment_start_at, catalog_load_type_id = $10::uuid,",
    ...
    editMapping: 'catalog_load_type_id: str(load.catalog_load_type_id), additional_charge_id: str(line.additional_charge_id), lumper_provider_id: str(s.lumper_provider_id), load_trailer_equipment_id: str(load.load_trailer_equipment_id),',
    ...
    dispatchFlagDrawer: 'createKind="dispatch_flag_color"; dispatch_flag_color_id',
    dispatchFlagMigration: "loads_dispatch_flag_color_same_company_fk; ALTER COLUMN dispatch_flag_color_id SET NOT NULL",
    accessorialEditor: 'value={row.additional_charge_id || null}; createKind="additional_charge"',
    additionalChargeMigration: "load_charge_lines_additional_charge_same_company_fk; line_kind = 'accessorial' AND additional_charge_id IS NOT NULL",
    lumperProviderMigration: "load_stops_lumper_provider_required; enforce_load_stop_lumper_provider_company",
    trailerEquipmentPicker: 'createKind="load_trailer_equipment"',
    trailerEquipmentMigration: "loads_trailer_equipment_same_company_fk; ALTER COLUMN load_trailer_equipment_id SET NOT NULL",
    accidentDrawer: 'createKind="accident_type"; accident_type_id: accidentTypeId',
    accidentTypeMigration: "accident_reports_type_same_company_fk; ALTER COLUMN accident_type_id SET NOT NULL",
    complaintsTab: 'createKind="complaint_type"; complaint_type_id: form.complaint_type_id',
    complaintsRoute: "complaint_type_id: z.string().uuid(); $12::uuid",
    complaintTypeMigration: "complaints_complaint_type_same_company_fk; FOREIGN KEY (complaint_type_id, operating_company_id)",
    cargoClaimSurface: 'createKind="cargo_claim_reason"; claim_reason_id: form.claimReasonId',
    incidentsRoute: "claim_reason_id: z.string().uuid(); claim_reason_id,",
    cargoClaimReasonMigration: "incidents_claim_reason_same_company_fk; incidents_cargo_claim_reason_required",
    bookLoadCustomer: 'createKind="customer"; customer_id',
    invoiceCustomer: 'createKind="customer"; customer_id',
    customerConsumerMigration: "loads_customer_same_company_fk; invoices_customer_same_company_fk",
    recordExpenseForm: 'createKind="vendor"; vendorUuid',
    expenseRoutes: "expense_vendor_not_in_company; operating_company_id = $2::uuid",
    expenseVendorMigration: "expenses_vendor_same_company_fk; FOREIGN KEY (vendor_uuid, operating_company_id)",
  };
