...
/** @matrix-built modules=maintenance,accounting cols=connectivity */
...
async function liveCheck() {
  if (!process.env.DATABASE_URL && !process.env.DATABASE_DIRECT_URL) {
    console.error("verify-capitalized-repair-registers-fixed-asset: FAIL — DATABASE_URL not set or the database is unreachable. A live money guard that cannot connect is a FAIL, never a pass (ROUND 29.9-B).");
    process.exit(1);
  }
