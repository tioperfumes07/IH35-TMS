...
const KNOWN_ORPHAN_WRITERS = [
  // Deprecated route file, retained per archive-never-delete; its event has no consumer and the file
  // is not reachable from the live router.
  "apps/backend/src/banking/manual-je.routes.deprecated.ts",
  // maintenance.qbo.bill.sync / maintenance.qbo.expense.sync — QuickBooks WRITE-BACK, which is OFF by
  // owner ruling ("the only thing we do not do is sync to QuickBooks", 2026-08-03). Deliberately left
  // pointing at the inert table rather than given a consumer: a QBO push must not become reachable as
  // a side effect of an outbox cleanup.
  "apps/backend/src/maintenance/work-orders.routes.ts",
  // 2026-08-03: 11 -> 2. Repointed onto outbox.events with real consumers: load-distribution,
  // dispatch-load-dispatched.handler, factoring/packet-assemble, fuel/planner, dispatch/intransit-issues,
  // liabilities, maintenance/triage, dispatch/dispatch-refinements. book-load's speculative seven-event
  // fan-out was removed outright (every outcome it named is delivered by a consumed path).
];

/**
 * Event types that have a REGISTERED handler and therefore must never be enqueued onto the dead
 * table — writing them there is strictly worse than having no handler, because the code looks
 * complete end-to-end while delivering nothing.
 */
...
/** True when `eventType` appears inside a bounded window after an INSERT into the orphaned table. */
export function writesHandledEventToOrphan(src, eventType) {
  const re = new RegExp(
    `INSERT\\s+INTO\\s+${escapeRegExp(ORPHAN_TABLE)}[\\s\\S]{0,600}?${escapeRegExp(eventType)}`,
    "i"
  );
...
export function analyse(files) {
  ...
  for (const [file, raw] of Object.entries(files)) {
    ...
    // An event type WITH a handler, enqueued onto the dead table, is the worst case: the chain reads
    // as complete end-to-end and delivers nothing. This is how no driver ever received a dispatch
    // message — twilio.whatsapp.send has had a working handler the whole time.
    for (const handled of HANDLED_EVENT_TYPES) {
      if (writesHandledEventToOrphan(raw, handled)) {
        problems.push(
          `${file}: enqueues "${handled}" onto ${ORPHAN_TABLE}, but that event type HAS a registered ` +
            `handler. It will never run — the table is drained by nothing. Use enqueueOutboxEvent() ` +
            `(apps/backend/src/outbox/enqueue-outbox-event.ts).`
        );
      }
    }
...
function selftest() {
  ...
  // THE DRIVER-NOTIFICATION CHAIN: a handled event type on the dead table.
  t("a HANDLED event type on the orphan table FAILS",
    analyse({ ...base, "apps/backend/src/banking/manual-je.routes.deprecated.ts":
      `INSERT INTO ${ORPHAN_TABLE} (event_type) VALUES ("twilio.whatsapp.send")` }).length === 1);

  t("a handled event type on outbox.events passes",
    analyse({ ...base, "apps/backend/src/x.ts":
      'INSERT INTO outbox.events (event_type) VALUES ("twilio.whatsapp.send")' }).length === 0);
...
if (process.argv.includes("--selftest")) {
  selftest();
  console.log(`${LABEL} selftest OK — 9 cases (4 pass-shapes, 5 fail-shapes incl. the real orphaned-override state, the handled-event-on-dead-table case, and the ratchet arm)`);
  process.exit(0);
}
