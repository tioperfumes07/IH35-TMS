...
const createSection = (s) => s.slice(s.indexOf("export async function createOfficeIntransitIssue"));
function failures(s, p, l, d, u) {
  const q = section(s); const resolve = resolveSection(s); const create = createSection(s); const out = [];
  ...
  if (!/const issue = insertRes\.rows\[0\];[\s\S]*if \(!issue\) return \{ ok: false as const, error: "create_failed" as const \};[\s\S]*return \{ ok: true as const, issue \};/.test(create)) out.push("office create can return success without a persisted issue identity");
...
if (process.argv.includes("--selftest")) {
  ...
  const mutations = [
    ...
    [service.replace("if (!issue) return { ok: false as const, error: \"create_failed\" as const };", "void issue;"), page, load, driver, unit],
