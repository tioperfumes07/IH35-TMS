...
LABEL. Codex and all coders (Cursor / Cascade / Devin / Codex Coder) have FULL Neon access and merge authority.**
...
- **Coders MERGE ON GREEN — every lane, including financial/migration/`accounting.*`·`catalogs.*`·`mdata.*`.** No
  ...
  hold.
- **FULL NEON ACCESS.** Codex and the coders apply migrations and flip posting flags **themselves** on Neon
...
(squash); verify deploy by health-SHA ancestry; never `git add -A` (untracked `.Codex/worktrees`).
...
the hire contract) · audit item **0008-f RESOLVED** · `00_LOCKED_DECISIONS.md` §9.5 · `docs/AGENTS.md`.
