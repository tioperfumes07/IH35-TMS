...
one ranked finding per PR. **Local fail-fast:** `scripts/money-pr-local-gate.mjs` (first husky pre-push step; Rule 29 suite) — do not push until it PASSes; run it explicitly (do not rely on husky); never `--no-verify`; one push; never rebase while `build-typecheck` runs.
