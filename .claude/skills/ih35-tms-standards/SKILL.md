...
one ranked finding per PR. **Local fail-fast:** `scripts/money-pr-local-gate.mjs` (first husky pre-push step) — do not push until it PASSes; one push; never rebase while `build-typecheck` runs.
